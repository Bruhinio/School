...
const Btn = ({children,onPress,variant="filled",color,size="md",disabled,full,style:sx={}})=>{
  ...
  return(
    <button onClick={()=>{if(!disabled){Sounds.play("click");onPress?.();}}} disabled={disabled}
...
const Sounds = {
  ...
  play(type="click") {
    try {
      ...
      const configs = {
        click:  { freq:600, type:"sine",    dur:0.06, vol:0.08 },
        success:{ freq:880, type:"sine",    dur:0.18, vol:0.12 },
        error:  { freq:220, type:"sawtooth",dur:0.18, vol:0.10 },
        notify: { freq:740, type:"sine",    dur:0.22, vol:0.15 },
        login:  { freq:523, type:"sine",    dur:0.35, vol:0.12 },
        alert:  { freq:440, type:"triangle",dur:0.30, vol:0.18 },
      };
      const cfg = configs[type] || configs.click;
      o.type = cfg.type;
      o.frequency.setValueAtTime(cfg.freq, ctx.currentTime);
      if(type==="login") o.frequency.exponentialRampToValueAtTime(cfg.freq*1.5, ctx.currentTime+0.2);
      if(type==="notify") o.frequency.exponentialRampToValueAtTime(cfg.freq*1.2, ctx.currentTime+0.1);
...
const useSound = () => (type="click") => Sounds.play(type);
...
const notifyAvisoWhatsApp = (aviso, teachers=[], students=[]) => {
  const msg = `🏫 *${aviso.type==="accident"?"🚨 URGENTE":aviso.type==="general"?"📢 Aviso":"📋 Aviso"} — SchoolLMS*\n\n*${aviso.title}*\n\n${aviso.body}\n\n_${aviso.time||"Hoy"}_`;
  const recipients = [...teachers.filter(t=>t.contact), ...students.filter(s=>s.parentContact)];
  if(recipients.length === 0){ alert("No hay contactos con número de WhatsApp registrado."); return; }
  recipients.forEach((r,i) => setTimeout(()=>sendWhatsApp({phone:r.contact||r.parentContact,message:msg}), i*800));
};

// ─── HOLIDAY THEMES ───────────────────────────────────────────────────────────
const HOLIDAY_THEMES = {
  none:     { label:"Normal",        emoji:"🏫", bg:"#F2F2F7", accent:"#007AFF", header:"#007AFF", pattern:"" },
  navidad:  { label:"Navidad",       emoji:"🎄", bg:"#F0FFF4", accent:"#C41E3A", header:"#C41E3A", pattern:"❄️" },
  halloween:{ label:"Halloween",     emoji:"🎃", bg:"#1a0a2e", accent:"#FF6B00", header:"#2D1B69", pattern:"🦇", dark:true },
  independencia:{ label:"Independencia", emoji:"🇲🇽", bg:"#F5FFF5", accent:"#006847", header:"#006847", pattern:"🦅" },
  diaMadres:{ label:"Día de las Madres", emoji:"💐", bg:"#FFF0F5", accent:"#E91E8C", header:"#E91E8C", pattern:"🌸" },
  primavera:{ label:"Primavera",     emoji:"🌸", bg:"#FFFEF0", accent:"#FF9800", header:"#4CAF50", pattern:"🌺" },
  diadeMuertos:{ label:"Día de Muertos", emoji:"💀", bg:"#1a0a00", accent:"#FF6B00", header:"#4A0080", pattern:"🌼", dark:true },
  amorAmistad:{ label:"Amor y Amistad", emoji:"💝", bg:"#FFF0F5", accent:"#E91E8C", header:"#E91E8C", pattern:"💕" },
};
let _globalTheme = "none";
const getTheme = () => HOLIDAY_THEMES[_globalTheme] || HOLIDAY_THEMES.none;
const setGlobalTheme = (t) => { _globalTheme = t; };
// Load from localStorage
try { _globalTheme = localStorage.getItem("schoolTheme") || "none"; } catch {}

// ─── BELL NOTIFICATION BUTTON ─────────────────────────────────────────────────
const BellNotification = ({ avisos=[], accent=C.indigo, onAddAviso, canAdd=false, teachers=[], students=[] }) => {
  ...
  return (
    <div style={{position:"relative"}}>
      <button onClick={()=>{setOpen(!open);sound("click");}}
        style={{width:36,height:36,borderRadius:"50%",background:open?`${accent}20`:"transparent",
          border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:20,position:"relative"}}>
        🔔
        {unread>0&&<span style={{position:"absolute",top:2,right:2,background:C.red,color:"#fff",
      ...
      {open&&(
        <div style={{position:"fixed",top:56,right:8,width:Math.min(340,window.innerWidth-16),
          ...
          <div style={{padding:"14px 16px 10px",borderBottom:`1px solid ${C.sep}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div style={{fontSize:15,fontWeight:700,color:C.lbl,fontFamily:SF}}>🔔 Avisos</div>
            <div style={{display:"flex",gap:8}}>
              {canAdd&&<button onClick={()=>setShowForm(!showForm)}
                style={{background:`${accent}15`,border:"none",borderRadius:8,padding:"4px 10px",
                  color:accent,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SF}}>+ Nuevo</button>}
              <button onClick={()=>setOpen(false)}
                style={{background:C.fill3,border:"none",borderRadius:"50%",width:24,height:24,
                  cursor:"pointer",color:C.lbl2,fontSize:13}}>✕</button>
            </div>
          </div>
          {showForm&&canAdd&&(
            <div style={{padding:"12px 14px",borderBottom:`1px solid ${C.sep}`,flexShrink:0}}>
              <select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value}))}
                style={{width:"100%",background:C.fill4,border:"none",borderRadius:8,padding:"8px 10px",fontSize:13,color:C.lbl,fontFamily:SF,marginBottom:8,outline:"none"}}>
                <option value="general">📢 General</option>
                <option value="accident">🚨 Urgente / Accidente</option>
                <option value="board">📋 Tablón</option>
                <option value="administrative">🏫 Administrativo</option>
              </select>
              <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
                placeholder="Título del aviso…" style={{width:"100%",background:C.fill4,border:"none",
                  borderRadius:8,padding:"8px 10px",fontSize:13,color:C.lbl,fontFamily:SF,marginBottom:6,outline:"none"}}/>
              <textarea value={form.body} onChange={e=>setForm(f=>({...f,body:e.target.value}))}
                placeholder="Descripción…" rows={2} style={{width:"100%",background:C.fill4,border:"none",
                  borderRadius:8,padding:"8px 10px",fontSize:13,color:C.lbl,fontFamily:SF,resize:"none",outline:"none",marginBottom:8}}/>
              <div style={{display:"flex",gap:6}}>
                <button onClick={submit}
                  style={{flex:1,background:accent,border:"none",borderRadius:8,padding:"8px",
                    color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:SF}}>Publicar</button>
                {form.type==="accident"&&(
                  <button onClick={()=>notifyAvisoWhatsApp({...form,time:today()},teachers,students)}
                    style={{background:"#25D366",border:"none",borderRadius:8,padding:"8px 10px",
                      color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:SF}}>📱 WA</button>
                )}
              </div>
            </div>
          )}
          <div style={{overflowY:"auto",flex:1}}>
            {avisos.length===0&&(
              <div style={{padding:"24px",textAlign:"center",color:C.lbl3,fontSize:14,fontFamily:SF}}>Sin avisos</div>
            )}
            {avisos.map((a,i)=>{
              const typeColor={general:C.blue,accident:C.red,board:C.purple,administrative:C.orange}[a.type]||C.blue;
              const typeIcon={general:"📢",accident:"🚨",board:"📋",administrative:"🏫"}[a.type]||"📣";
              return(
                <div key={a.id} style={{padding:"10px 14px",borderBottom:i<avisos.length-1?`1px solid ${C.sep}`:"none",
                  background:a.type==="accident"?`${C.red}05`:"transparent"}}>
                  <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                    <span style={{fontSize:16,flexShrink:0}}>{typeIcon}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:700,color:typeColor,fontFamily:SF}}>{a.title}</div>
                      <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,marginTop:2,lineHeight:1.4}}>{a.body}</div>
                      <div style={{fontSize:11,color:C.lbl3,fontFamily:SF,marginTop:4}}>{a.time||"Reciente"}</div>
                    </div>
                    {!a.read&&<div style={{width:8,height:8,borderRadius:"50%",background:typeColor,flexShrink:0,marginTop:4}}/>}
                  </div>
                </div>
              );
            })}
...
const DeveloperApp=({state,setState,onLogout})=>{
  ...
  // Unassigned students
  const unassignedStudents = state.students.filter(s => !s.group || s.group.trim() === "");
  const [assignModal, setAssignModal] = useState(null); // student to assign
  ...
  const assignStudentGroup = async () => {
    if (!assignModal || !assignGroup) return;
    const grp = state.groups.find(g => g.id === assignGroup || g.name === assignGroup);
    const groupName = grp ? grp.name : assignGroup;
    const grpMatch = groupName.match(/(\d+)[°]?([A-Z])/i);
    try {
      if (typeof assignModal.id === "string")
        await updateDoc(doc(db,"students",assignModal.id), {
          group: groupName,
          grade: grpMatch ? parseInt(grpMatch[1]) : assignModal.grade,
          section: grpMatch ? grpMatch[2].toUpperCase() : assignModal.section,
        });
    } catch {}
    setState(s=>({...s, students: s.students.map(st => st.id===assignModal.id
      ? {...st, group:groupName, grade:grpMatch?parseInt(grpMatch[1]):st.grade, section:grpMatch?grpMatch[2].toUpperCase():st.section}
      : st)}));
    setAssignModal(null); setAssignGroup("");
  };

  const saveStudentProfile = async () => {
    if (!studentProfileModal) return;
    const updates = { ...editStudentForm };
    if (studentPhoto) updates.photo = studentPhoto;
    try {
      if (typeof studentProfileModal.id === "string")
        await updateDoc(doc(db,"students",studentProfileModal.id), updates);
    } catch {}
    setState(s=>({...s, students: s.students.map(st => st.id===studentProfileModal.id ? {...st,...updates} : st)}));
    setStudentProfileModal(null); setStudentPhoto(null);
  };

  const DEV_TABS = [
    {id:"students", icon:"🎒", label:"Alumnos"},
    {id:"pending",  icon:"⏳", label:"Sin Grupo", badge: unassignedStudents.length},
  ...
  return(
    <div style={{background:C.bg2, minHeight:"100vh", fontFamily:SF, display:"flex", flexDirection:"column"}}>
      <style>{`
        @keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
        @media(min-width:768px){
          .dev-layout{flex-direction:row!important;}
          .dev-sidebar{width:220px!important;min-height:100vh!important;flex-direction:column!important;padding:24px 12px!important;gap:4px!important;position:sticky!important;top:0!important;height:100vh!important;overflow-y:auto!important;}
          .dev-sidebar-btn{flex-direction:row!important;justify-content:flex-start!important;padding:10px 14px!important;border-radius:12px!important;border-bottom:none!important;width:100%!important;}
          .dev-main{flex:1!important;max-width:100%!important;padding:24px!important;}
          .dev-header{padding:28px 24px 20px!important;}
        }
      `}</style>

      {/* Layout wrapper */}
      <div className="dev-layout" style={{display:"flex", flexDirection:"column", minHeight:"100vh"}}>

        {/* Sidebar / Top nav */}
        <div className="dev-sidebar" style={{background:devColor, display:"flex", flexDirection:"row",
          overflowX:"auto", padding:"0 8px", gap:0, scrollbarWidth:"none", flexShrink:0}}>

          {/* Header — visible only on desktop via CSS */}
          <div style={{display:"none"}} className="dev-header-wrap">
            <div style={{padding:"24px 12px 16px"}}>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:SF,marginBottom:4}}>Desarrollador</div>
              <div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:SFD,letterSpacing:"-0.3px"}}>Admin</div>
            </div>
          </div>

          {/* Mobile header */}
          <div style={{display:"flex",alignItems:"center",padding:"12px 8px",borderBottom:"1px solid rgba(255,255,255,0.08)",width:"100%",justifyContent:"space-between",flexShrink:0}}
            className="dev-mobile-header">
            <span style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.7)",fontFamily:SF}}>🛠 Admin</span>
            <button onClick={onLogout} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:8,padding:"5px 12px",color:"rgba(255,255,255,0.7)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:SF}}>Salir</button>
          </div>

          {/* Nav items */}
          {DEV_TABS.map(t=>(
            <button key={t.id} className="dev-sidebar-btn"
              onClick={()=>setTab(t.id)}
              style={{flexShrink:0, padding:"12px 14px", background:"none",
                border:"none", borderBottom: tab===t.id ? "2px solid #fff" : "2px solid transparent",
        ...
        {/* Main content */}
        <div className="dev-main" style={{flex:1, padding:"16px 16px 80px", animation:"fadeUp 0.3s ease", maxWidth:800, width:"100%", margin:"0 auto"}}>

          {/* ── ALUMNOS TAB ─────────────────────────────────────────────────── */}
          {tab==="students"&&(
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF}}>🎒 Alumnos</div>
                <Btn onPress={()=>setShowSForm(!showSForm)} size="sm" color={devColor}>{showSForm?"Cancelar":"+ Registrar"}</Btn>
              </div>
              {showSForm&&(
                <Card style={{padding:16,marginBottom:14}}>
                  ...
                  <div style={{background:C.fill4,borderRadius:12,padding:"10px 14px",marginBottom:10}}>
                    ...
                    <select value={newS.group} onChange={e=>setNewS(s=>({...s,group:e.target.value}))}
                      style={{width:"100%",background:"transparent",border:"none",fontSize:15,color:C.lbl,fontFamily:SF,outline:"none"}}>
                      <option value="">Sin asignar (asignar después)</option>
                      {state.groups.map(g=><option key={g.id} value={g.name}>{g.name}</option>)}
                    </select>
                  </div>
              ...
              {unassignedStudents.length > 0 && (
                <div style={{background:`${C.orange}12`,border:`1.5px solid ${C.orange}30`,borderRadius:12,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:20}}>⚠️</span>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700,color:C.orange,fontFamily:SF}}>{unassignedStudents.length} alumno{unassignedStudents.length>1?"s":""} sin grupo</div>
                    <div style={{fontSize:12,color:C.lbl2,fontFamily:SF}}>Ve a la pestaña "Sin Grupo" para asignarlos</div>
                  </div>
                  <button onClick={()=>setTab("pending")} style={{background:C.orange,border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SF}}>Ver →</button>
                </div>
              )}
              <Sec title={`Alumnos registrados — ${state.students.length}`}>
                {state.students.length === 0 && <div style={{padding:"20px 16px",textAlign:"center",color:C.lbl3,fontSize:14,fontFamily:SF}}>Sin alumnos registrados</div>}
                {state.students.map((s,i)=>(
                  <div key={s.id}>
                    <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",cursor:"pointer"}}
                      onClick={()=>{setStudentProfileModal(s);setEditStudentForm({name:s.name,parentEmail:s.parentEmail,parentContact:s.parentContact||"",group:s.group||""});}}>
                      {s.photo
                        ? <img src={s.photo} alt="" style={{width:42,height:42,borderRadius:"50%",objectFit:"cover",border:`2px solid ${s.color}`}}/>
                        : <Ava initials={s.avatar} color={s.color} size={42}/>}
                      <div style={{flex:1}}>
                        <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>{s.name}</div>
                        <div style={{...fmt.caption,color:s.group?C.lbl2:C.orange,fontFamily:SF,marginTop:1}}>{s.group||"⚠️ Sin grupo"}</div>
                        <div style={{fontFamily:"'SF Mono','Menlo',monospace",fontSize:11,color:C.green,marginTop:2,opacity:0.85}}>{s.key}</div>
                      </div>
                      <svg width="7" height="12" viewBox="0 0 7 12"><path d="M1 1l5 5-5 5" stroke={C.g3} strokeWidth="1.5" strokeLinecap="round" fill="none"/></svg>
                    </div>
          ...
          {tab==="pending"&&(
            <>
              <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF,marginBottom:14}}>⏳ Alumnos sin grupo asignado</div>
              {unassignedStudents.length === 0 ? (
                <Card style={{padding:32,textAlign:"center"}}>
                  <div style={{fontSize:40,marginBottom:8}}>✅</div>
                  <div style={{fontSize:15,fontWeight:600,color:C.green,fontFamily:SF}}>¡Todos los alumnos tienen grupo!</div>
                </Card>
              ) : (
                <Card>
                  {unassignedStudents.map((s,i)=>(
                    <div key={s.id}>
                      <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px"}}>
                        {s.photo ? <img src={s.photo} alt="" style={{width:42,height:42,borderRadius:"50%",objectFit:"cover"}}/> : <Ava initials={s.avatar} color={s.color} size={42}/>}
                        <div style={{flex:1}}>
                          <div style={{fontSize:14,fontWeight:600,color:C.lbl,fontFamily:SF}}>{s.name}</div>
                          <div style={{fontSize:12,color:C.lbl3,fontFamily:SF,marginTop:1}}>Sin grupo · {s.parentEmail}</div>
                        </div>
                        <button onClick={()=>{setAssignModal(s);setAssignGroup("");}}
                          style={{background:`${C.blue}15`,border:`1px solid ${C.blue}30`,borderRadius:8,padding:"6px 12px",color:C.blue,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SF}}>
                          Asignar grupo
                        </button>
                      </div>
                      {i<unassignedStudents.length-1&&<Div indent={70}/>}
                    </div>
                  ))}
                </Card>
              )}
          ...
          {tab==="teachers"&&(
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF}}>👩‍🏫 Docentes</div>
                <Btn onPress={()=>setShowTForm(!showTForm)} size="sm" color={devColor}>{showTForm?"Cancelar":"+ Registrar"}</Btn>
              </div>
              {showTForm&&(
                <Card style={{padding:16,marginBottom:14}}>
              ...
              <Sec title={`Docentes registrados — ${state.teachers.length}`}>
                {state.teachers.length === 0 && <div style={{padding:"20px 16px",textAlign:"center",color:C.lbl3,fontSize:14,fontFamily:SF}}>Sin docentes registrados</div>}
              ...
              <Modal open={!!editTeacher} onClose={()=>setEditTeacher(null)} title={`Editar: ${editTeacher?.name?.split(" ")[0]}`}>
                <Input label="📚 Materias (separar con coma)" placeholder="Matemáticas, Álgebra" value={editForm.subjects} onChange={v=>setEditForm(f=>({...f,subjects:v}))}/>
                <Input label="👥 Grupos asignados (separar con coma)" placeholder="3°A, 3°B" value={editForm.groups} onChange={v=>setEditForm(f=>({...f,groups:v}))}/>
          ...
          {tab==="groups"&&(
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF}}>🏫 Grupos</div>
                <Btn onPress={()=>setShowGForm(!showGForm)} size="sm" color={devColor}>{showGForm?"Cancelar":"+ Crear"}</Btn>
              </div>
              {showGForm&&(
                <Card style={{padding:16,marginBottom:14}}>
              ...
              {state.groups.map((g)=>{
                ...
                const studs=state.students.filter(s=>s.group===g.name||g.students?.includes(s.id));
                const [open,setOpen]=useState(false);
                return(
                  <Card key={g.id} style={{marginBottom:10}}>
                    <div style={{padding:"14px 16px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>setOpen(!open)}>
                        <div style={{width:46,height:46,borderRadius:12,background:`${devColor}12`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          <span style={{fontSize:13,fontWeight:800,color:devColor,fontFamily:SF}}>{g.name}</span>
                        </div>
                        <div style={{flex:1}}>
                          <div style={{fontSize:15,fontWeight:700,color:C.lbl,fontFamily:SF}}>{g.subject||"Sin materia"} — {g.name}</div>
                          <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,marginTop:2}}>{teacher?.name||"Sin docente"} · {studs.length} alumnos</div>
                        </div>
                        <span style={{fontSize:12,color:C.lbl3}}>{open?"▲":"▼"}</span>
                      </div>
                      {open&&studs.length>0&&(
                        <div style={{marginTop:12,borderTop:`1px solid ${C.sep}`,paddingTop:12}}>
                          <div style={{fontSize:11,fontWeight:700,color:C.lbl2,textTransform:"uppercase",letterSpacing:"0.05em",fontFamily:SF,marginBottom:8}}>Alumnos del grupo</div>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
                            {studs.map(s=>(
                              <button key={s.id} onClick={()=>{setStudentProfileModal(s);setEditStudentForm({name:s.name,parentEmail:s.parentEmail,parentContact:s.parentContact||"",group:s.group||""});}}
                                style={{background:C.fill4,border:`1px solid ${C.g4}`,borderRadius:12,padding:"10px 8px",cursor:"pointer",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                                {s.photo
                                  ? <img src={s.photo} alt="" style={{width:44,height:44,borderRadius:"50%",objectFit:"cover",border:`2px solid ${s.color}`}}/>
                                  : <Ava initials={s.avatar} color={s.color} size={44}/>}
                                <div style={{fontSize:11,fontWeight:600,color:C.lbl,fontFamily:SF,lineHeight:1.3}}>{s.name.split(" ")[0]} {s.name.split(" ")[1]||""}</div>
                                <div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>Ver perfil →</div>
                              </button>
                            ))}
                          </div>
                        </div>
                      ...
                      {open&&studs.length===0&&(
                        <div style={{marginTop:12,padding:"12px",textAlign:"center",color:C.lbl3,fontSize:13,fontFamily:SF,borderTop:`1px solid ${C.sep}`}}>Sin alumnos en este grupo</div>
                      )}
                    </div>
                  </Card>
                );
          ...
          {tab==="cycles"&&(
            <>
              <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF,marginBottom:14}}>🔄 Ciclos Escolares</div>
              <Sec title={`Ciclos — ${state.cycles.length}`}>
          ...
          {tab==="temas"&&(
            <>
              <div style={{fontSize:17,fontWeight:700,color:C.lbl,fontFamily:SF,marginBottom:6}}>🎨 Temas Festivos</div>
              <div style={{fontSize:13,color:C.lbl2,fontFamily:SF,marginBottom:16}}>Cambia el tema visual de toda la app para celebrar fechas especiales.</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10}}>
                {Object.entries(HOLIDAY_THEMES).map(([key,theme])=>{
                  const active = _globalTheme === key;
                  return(
                    <button key={key} onClick={()=>{
                      setGlobalTheme(key);
                      try { localStorage.setItem("schoolTheme",key); } catch {}
                      Sounds.play("success");
                      setState(s=>({...s,_theme:key}));
                    }}
                      style={{background:theme.dark?"#1a0a2e":active?`${devColor}10`:"#fff",
                        border:`2px solid ${active?devColor:C.g5}`,
                      ...
                      <div style={{fontSize:12,fontWeight:active?700:500,
                        color:theme.dark?"#fff":active?devColor:C.lbl,fontFamily:SF}}>{theme.label}</div>
                      {active&&<div style={{fontSize:10,color:theme.dark?"#aef":devColor,fontWeight:700,marginTop:4,fontFamily:SF}}>✓ ACTIVO</div>}
                    </button>
                  );
                })}
              </div>
              <Card style={{padding:14,marginTop:16}}>
                <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,lineHeight:1.6}}>
                  💡 <strong>Cómo funciona:</strong> Al activar un tema, los colores de toda la app cambian para todos los roles. Se guarda automáticamente.
                </div>
              </Card>
            </>
          )}
      ...
      {assignModal&&(
        <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:500,padding:"20px 20px 40px",animation:"slideUp 0.25s ease"}}>
            <div style={{fontSize:16,fontWeight:700,color:C.lbl,fontFamily:SF,marginBottom:4}}>Asignar Grupo</div>
            <div style={{fontSize:13,color:C.lbl2,fontFamily:SF,marginBottom:16}}>{assignModal.name}</div>
            <div style={{background:C.fill4,borderRadius:12,padding:"10px 14px",marginBottom:16}}>
              <div style={{fontSize:11,color:C.lbl2,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:4,fontFamily:SF}}>Selecciona un grupo</div>
              <select value={assignGroup} onChange={e=>setAssignGroup(e.target.value)}
                ...
                {state.groups.map(g=><option key={g.id} value={g.id}>{g.name} {g.subject?`· ${g.subject}`:""}</option>)}
              </select>
            </div>
            <div style={{display:"flex",gap:10}}>
              <Btn onPress={()=>setAssignModal(null)} full variant="ghost">Cancelar</Btn>
              <Btn onPress={assignStudentGroup} full color={devColor} style={{opacity:assignGroup?1:0.4}}>Asignar</Btn>
            </div>
          </div>
        </div>
      )}
      ...
      {studentProfileModal&&(
        <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:500,padding:"20px 20px 40px",animation:"slideUp 0.25s ease",maxHeight:"85vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:16,fontWeight:700,color:C.lbl,fontFamily:SF}}>Perfil del Alumno</div>
              <button onClick={()=>setStudentProfileModal(null)} style={{background:C.fill3,border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:15,color:C.lbl2}}>✕</button>
            </div>
            {/* Photo */}
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginBottom:16}}>
              <div style={{position:"relative",marginBottom:8}}>
                {(studentPhoto||studentProfileModal.photo)
                  ? <img src={studentPhoto||studentProfileModal.photo} alt="" style={{width:80,height:80,borderRadius:"50%",objectFit:"cover",border:`3px solid ${studentProfileModal.color}`}}/>
                  : <Ava initials={studentProfileModal.avatar} color={studentProfileModal.color} size={80}/>}
                <button onClick={()=>photoRef.current?.click()}
                  style={{position:"absolute",bottom:0,right:0,width:26,height:26,borderRadius:"50%",background:devColor,border:"2px solid #fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>
                  📷
                </button>
              </div>
              <input ref={photoRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
                const f=e.target.files?.[0]; if(!f)return;
                const r=new FileReader(); r.onload=ev=>setStudentPhoto(ev.target.result); r.readAsDataURL(f);
              }}/>
              <div style={{fontSize:11,color:C.lbl3,fontFamily:SF}}>Toca 📷 para cambiar foto</div>
            </div>
            ...
            <div style={{background:C.fill4,borderRadius:12,padding:"10px 14px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.lbl2,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:4,fontFamily:SF}}>Grupo</div>
              <select value={editStudentForm.group||""} onChange={e=>setEditStudentForm(f=>({...f,group:e.target.value}))}
                style={{width:"100%",background:"transparent",border:"none",fontSize:15,color:C.lbl,fontFamily:SF,outline:"none"}}>
            ...
            {/* QR */}
            <div style={{background:C.fill4,borderRadius:12,padding:14,marginBottom:14,textAlign:"center"}}>
              <div style={{fontSize:11,color:C.lbl2,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",fontFamily:SF,marginBottom:8}}>Código QR de Asistencia</div>
              <QRCode value={studentProfileModal.key} size={120}/>
            </div>
            <div style={{display:"flex",gap:10}}>
              <Btn onPress={()=>{setStudentProfileModal(null);setStudentPhoto(null);}} full variant="ghost">Cancelar</Btn>
              <Btn onPress={saveStudentProfile} full color={devColor}>Guardar</Btn>
            </div>
            <div style={{marginTop:12}}>
              <Btn onPress={()=>deleteStudentFb(studentProfileModal)} full variant="ghost" style={{color:C.red,borderColor:`${C.red}30`}}>🗑 Eliminar alumno</Btn>
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          <NavBar title="Gestión Escolar" large accent={C.indigo}
            right={<div style={{display:"flex",alignItems:"center",gap:8}}>
              <BellNotification avisos={state.avisos||[]} accent={C.indigo} canAdd
                onAddAviso={(a)=>{
                  const aviso={id:Date.now(),fromName:"Directora",fromRole:"Directora",type:a.type,title:a.title,body:a.body,time:todayStr,read:false};
                  setState(s=>({...s,avisos:[aviso,...(s.avisos||[])]}));
                  Sounds.play("notify");
                }}
                teachers={state.teachers} students={state.students}/>
              <button onClick={()=>setCycleModal(true)} style={{background:`${C.indigo}10`,border:`1px solid ${C.indigo}25`,borderRadius:20,padding:"5px 14px",color:C.indigo,fontSize:13,fontWeight:600,fontFamily:SF,cursor:"pointer"}}>{activeCycle?.name}</button>
            </div>}/>

          {/* Layout: sidebar vertical en desktop, tabs arriba en móvil */}
          <div className="dir-layout" style={{display:"flex",flexDirection:"column"}}>
            <style>{`
              @media(min-width:768px){
                .dir-layout{flex-direction:row!important;min-height:calc(100vh - 56px);}
                .dir-sidebar{width:190px!important;flex-direction:column!important;padding:12px 8px!important;gap:2px!important;border-right:1px solid #E5E5EA!important;background:#fff!important;position:sticky!important;top:56px!important;height:calc(100vh - 56px)!important;overflow-y:auto!important;flex-shrink:0!important;}
                .dir-tab-btn{flex-direction:row!important;justify-content:flex-start!important;padding:10px 12px!important;border-radius:10px!important;border-bottom:none!important;border-right:none!important;gap:8px!important;width:100%!important;}
                .dir-content{flex:1!important;padding:20px!important;max-width:760px!important;}
                .dir-mobile-scroll{display:none!important;}
              }
            `}</style>

            {/* Mobile tab scroll */}
            <div className="dir-mobile-scroll" style={{display:"flex",gap:6,padding:"12px 16px 8px",overflowX:"auto",scrollbarWidth:"none",background:"#fff",borderBottom:`1px solid ${C.sep}`}}>
              {[
                ...
                {id:"aprobaciones",icon:"✅",label:"Aprobaciones",badge:state.pendingContent.length},
              ].map(t=>(
                <button key={t.id} onClick={()=>{setMgmtTab(t.id);Sounds.play("click");}}
                  style={{flexShrink:0,padding:"6px 14px",borderRadius:20,border:mgmtTab===t.id?`1.5px solid ${C.indigo}`:`1px solid ${C.g4}`,
                    cursor:"pointer",fontFamily:SF,fontSize:13,fontWeight:600,
                    background:mgmtTab===t.id?C.indigo:"#fff",
                    color:mgmtTab===t.id?"#fff":C.lbl2,position:"relative"}}>
                  {t.label}
                  {t.badge>0&&<span style={{position:"absolute",top:-5,right:-5,background:C.red,color:"#fff",fontSize:9,fontWeight:700,borderRadius:"50%",width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center"}}>{t.badge}</span>}
                </button>
              ))}
            </div>

            {/* Vertical sidebar */}
            <div className="dir-sidebar" style={{display:"none",flexDirection:"column",padding:"8px",gap:2,background:"#fff"}}>
              <div style={{padding:"8px 12px 14px",borderBottom:`1px solid ${C.sep}`,marginBottom:6}}>
                <div style={{fontSize:11,color:C.lbl3,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",fontFamily:SF}}>Gestión</div>
              </div>
              {[
                {id:"resumen",icon:"📊",label:"Resumen"},
                {id:"asistencia",icon:"📋",label:"Asistencia"},
                ...
                <button key={t.id} className="dir-tab-btn"
                  onClick={()=>{setMgmtTab(t.id);Sounds.play("click");}}
                  style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,
                    padding:"10px 8px",background:mgmtTab===t.id?`${C.indigo}12`:"transparent",
                    border:"none",borderRadius:10,cursor:"pointer",fontFamily:SF,
                    color:mgmtTab===t.id?C.indigo:C.lbl2,fontWeight:mgmtTab===t.id?700:500,
                    fontSize:13,position:"relative",transition:"all 0.15s"}}>
                  <span style={{fontSize:18}}>{t.icon}</span>
                  <span>{t.label}</span>
                  {t.badge>0&&<span style={{position:"absolute",top:6,right:6,background:C.red,color:"#fff",fontSize:9,fontWeight:700,borderRadius:"50%",width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center"}}>{t.badge}</span>}
                </button>
              ))}
            </div>

            {/* Main content */}
            <div className="dir-content" style={{flex:1,padding:"16px 16px 100px"}}>
                ...
                <>
                  ...
                  <Card style={{marginBottom:14,overflow:"hidden"}}>
                    ...
                    {(()=>{
                      ...
                      const clickableA=[["Salón 1°A",28,40],["Salón 1°B",28,82],["Dirección",28,124],["Sala Doc.",28,166],["Biblioteca",28,208]];
                      const clickableB=[["Salón 2°A",243,40],["Salón 2°B",243,82],["Salón 3°A",243,124],["Salón 3°B",243,166],["Lab. Cien.",243,208]];
                      const hasGroup=(l)=>!!roomToGroup[l];
                      const RoomRect=({label,x,y})=>{
                        const clickable=hasGroup(label);
                        const grpName=roomToGroup[label];
                        const grp=state.groups.find(g=>g.name===grpName);
                        const occupants=grp?state.students.filter(s=>grp.students?.includes(s.id)||s.group===grpName):[];
                        const presentCount=occupants.filter(s=>s.attendance?.find(a=>a.date===todayStr&&a.s==="present")).length;
                        return(
                          <g key={label} style={{cursor:clickable?"pointer":"default"}} onClick={()=>clickable&&setSelectedRoom(label)}>
                            <rect x={x-12} y={y-14} width="80" height="30" rx="4"
                              fill={selectedRoom===label?`${C.indigo}30`:"white"}
                              stroke={selectedRoom===label?C.indigo:clickable?"#C5CAE9":"#ddd"}
                              strokeWidth={selectedRoom===label?"2":"1"}/>
                            <text x={x+28} y={y+3} textAnchor="middle" fill={clickable?"#1a1a7e":"#666"} fontSize="7.5"
                              fontWeight={clickable?"700":"400"}>{label}</text>
                            {clickable&&grp&&(
                              <circle cx={x+62} cy={y-8} r="4" fill={presentCount>0?C.green:C.g3}/>
                            )}
                          </g>
                        );
                      };
                      return(
                        <svg width="100%" viewBox="0 0 320 280" style={{display:"block",background:"#F8F9FC"}}>
                          <rect x="105" y="90" width="110" height="100" rx="5" fill="#E8F4FD" stroke="#B8D4E8" strokeWidth="1.2"/>
                          <text x="160" y="143" textAnchor="middle" fill="#5A8FA8" fontSize="9" fontWeight="500">Patio Central</text>
                          <rect x="8" y="24" width="95" height="215" rx="7" fill="#EEF2FF" stroke={C.indigo} strokeWidth="1.4" opacity="0.6"/>
                          <text x="55" y="17" textAnchor="middle" fill={C.indigo} fontSize="8" fontWeight="700">EDIFICIO A</text>
                          {clickableA.map(([l,x,y])=><RoomRect key={l} label={l} x={x} y={y}/>)}
                          <rect x="217" y="24" width="95" height="215" rx="7" fill="#F0FFF4" stroke={C.green} strokeWidth="1.4" opacity="0.6"/>
                          <text x="264" y="17" textAnchor="middle" fill={C.green} fontSize="8" fontWeight="700">EDIFICIO B</text>
                          {clickableB.map(([l,x,y])=><RoomRect key={l} label={l} x={x} y={y}/>)}
                          <rect x="80" y="248" width="160" height="24" rx="5" fill="#E3F2FD" stroke="#90CAF9" strokeWidth="1.2"/>
                          <text x="160" y="263" textAnchor="middle" fill="#1565C0" fontSize="7" fontWeight="600">ENTRADA PRINCIPAL</text>
                          <AlertDot active={hasAccident} x={55} y={124} color={C.red}/>
                          <AlertDot active={hasBoardAlert} x={264} y={124} color={C.purple}/>
                          <AlertDot active={hasGeneral} x={160} y={138} color={C.orange}/>
                          <AlertDot active={hasAdmin} x={55} y={166} color={C.blue}/>
                          <circle cx="105" cy="272" r="4" fill={C.green}/><text x="113" y="276" fill="#666" fontSize="6.5">Presentes hoy</text>
                          <circle cx="180" cy="272" r="4" fill={C.g3}/><text x="188" y="276" fill="#666" fontSize="6.5">Sin registro</text>
                        </svg>
                      );
                    })()}
                    {selectedRoom&&(()=>{
                      ...
                      const studs=grp?state.students.filter(s=>grp.students?.includes(s.id)||s.group===grpName):[];
                        ...
                        <div style={{borderTop:`0.5px solid ${C.sep}`,background:"#fff"}}>
                          <div style={{padding:"12px 14px 8px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                            <div>
                              ...
                              <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF}}>{grp?`${grp.subject} — Grupo ${grpName}`:"Sin grupo asignado"}</div>
                            </div>
                          ...
                          {teacher?(
                            <div style={{padding:"8px 14px",borderBottom:`0.5px solid ${C.sep}`}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                ...
                                <span style={{fontSize:11,fontWeight:600,borderRadius:6,padding:"2px 8px",fontFamily:SF,color:teacherPresent?C.green:C.g1,background:teacherPresent?`${C.green}15`:C.fill4}}>{teacherPresent?`Presente ${teacherAtt.time}`:"Sin registro"}</span>
                              </div>
                            </div>
                          ):(
                          ...
                          <div style={{padding:"8px 0 4px"}}>
                            <div style={{fontSize:10,fontWeight:700,color:C.lbl3,textTransform:"uppercase",letterSpacing:"0.06em",fontFamily:SF,padding:"0 14px",marginBottom:5}}>Alumnos — {studs.length} inscritos</div>
                            ...
                            {studs.map((s,i)=>{
                              ...
                              const dotColor=todayRec?.s==="present"?C.green:todayRec?.s==="justified"?C.orange:todayRec?.s==="absent"?C.red:C.g3;
                              const statusLabel=todayRec?.s==="present"?"Presente":todayRec?.s==="justified"?"Justificada":todayRec?.s==="absent"?"Falta":"Sin registro";
                              return(
                                <div key={s.id}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
                    {[[state.students.length,"Alumnos","🎒",C.blue],[state.teachers.length,"Docentes","👩‍🏫",C.green],[state.groups.length,"Grupos","🏫",C.purple]].map(([n,l,e,c])=>(
                      <Card key={l} style={{padding:"14px 10px",textAlign:"center"}}>
                        <div style={{fontSize:22,marginBottom:4}}>{e}</div>
                        <div style={{fontSize:22,fontWeight:700,color:c,fontFamily:SF}}>{n}</div>
                        <div style={{fontSize:11,color:C.lbl2,fontFamily:SF}}>{l}</div>
                      </Card>
                    ))}
                  </div>

                  {/* Actividades próximas */}
                  {(state.actividades||[]).filter(a=>a.date>=todayStr).slice(0,3).length>0&&(
                    <Sec title="📅 Próximas actividades">
                      {(state.actividades||[]).filter(a=>a.date>=todayStr).sort((a,b)=>a.date.localeCompare(b.date)).slice(0,3).map((a,i,arr)=>{
                        ...
                        return(
                          <div key={a.id}>
                            <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px"}}>
                              <div style={{width:36,height:36,borderRadius:9,background:`${typeColor}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
                                {({trabajo:"📝",exposicion:"🎤",efemeride:"📅",examen:"📋",evento:"🎉",otro:"📌"})[a.type]||"📌"}
                              </div>
                              <div style={{flex:1}}>
                                <div style={{fontSize:14,fontWeight:600,color:C.lbl,fontFamily:SF}}>{a.title}</div>
                                <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,marginTop:1}}>📅 {a.date}</div>
                              </div>
                              <span style={{fontSize:10,fontWeight:600,color:typeColor,background:`${typeColor}12`,borderRadius:6,padding:"2px 8px",fontFamily:SF}}>{a.type}</span>
                            </div>
                            {i<arr.length-1&&<Div indent={64}/>}
                          </div>
                        );
                      })}
                    </Sec>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;-webkit-font-smoothing:antialiased;}
        ...
        @media(min-width:768px){
          ...
          .dev-layout{flex-direction:row!important;}
          .dev-sidebar{width:200px!important;min-height:100vh;flex-direction:column!important;padding:20px 10px!important;gap:4px!important;position:sticky!important;top:0!important;height:100vh!important;overflow-y:auto!important;flex-shrink:0!important;}
          .dev-sidebar-btn{flex-direction:row!important;justify-content:flex-start!important;padding:10px 14px!important;border-radius:10px!important;border-bottom:none!important;width:100%!important;border:none!important;gap:8px!important;}
          .dev-mobile-header{display:none!important;}
          .dev-main{flex:1!important;padding:24px!important;max-width:760px!important;}
          .tab-bar-mobile{display:none!important;}
          .dir-layout{flex-direction:row!important;min-height:calc(100vh - 56px)!important;}
          .dir-sidebar{display:flex!important;flex-direction:column!important;width:190px!important;flex-shrink:0!important;background:#fff!important;border-right:1px solid #E5E5EA!important;padding:12px 8px!important;gap:2px!important;position:sticky!important;top:56px!important;height:calc(100vh - 56px)!important;overflow-y:auto!important;}
          .dir-tab-btn{flex-direction:row!important;justify-content:flex-start!important;padding:10px 12px!important;border-radius:10px!important;gap:8px!important;width:100%!important;}
          .dir-mobile-scroll{display:none!important;}
          .dir-content{flex:1!important;padding:20px!important;max-width:800px!important;}
        }
        @media(min-width:1024px){
          .app-shell{max-width:100%;}
          .dev-main{max-width:960px!important;}
          .dir-content{max-width:960px!important;}
        }
