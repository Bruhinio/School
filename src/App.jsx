...
const DeveloperApp=({state,setState,onLogout})=>{
  ...
  // Add a subject+teacher to a group and auto-update any already-assigned students
  const addSubjectToGroup=(gId)=>{
    if(!newSubjInput.trim())return;
    const entry={subject:newSubjInput.trim(),teacherId:newSubjTeacher?Number(newSubjTeacher):null};
    setState(prev=>{
      ...
      const updStudents=prev.students.map(st=>{
        ...
        if(st.subjects?.[entry.subject])return st; // already has it
        return{...st,subjects:{...(st.subjects||{}),[entry.subject]:{grade:8,tasks:[]}}};
      });
      return{...prev,
        groups:prev.groups.map(g=>g.id===gId?{...g,subjects:newSubjects,subject:g.subject||entry.subject}:g),
  ...
  const assignStudentToGroup=async(student,grpId)=>{
    ...
    setState(prev=>({...prev,
      students:prev.students.map(st=>st.id===student.id
        ?{...st,group:grp.name,subjects:{...autoSubjs,...(st.subjects||{})}}
        :st),
