...
// ANIMAL_OFFSETS: vertical shift (dy) so accessories align with each animal's head
const ANIMAL_OFFSETS={
  whale:{dy:-4},giraffe:{dy:-6},elephant:{dy:-2},hippo:{dy:2},
  bear:{dy:0},lion:{dy:-2},fox:{dy:-4},rabbit:{dy:-8},
  owl:{dy:-2},cat:{dy:-4},default:{dy:0},
};

const MascotSVG=({subject,outfit,size=80,animal=null})=>{
  const m=resolveMascot(animal, subject);
  const animalKey=animal||(m?.animalId)||"default";
  const dy=(ANIMAL_OFFSETS[animalKey]||ANIMAL_OFFSETS.default).dy;

  // ── Accessories — each wrapped in <g transform={`translate(0,${dy})`}> ────
  const Birrete=()=>(<g transform={`translate(0,${dy})`}><rect x="28" y="10" width="44" height="8" rx="3" fill="#1a1a2e"/><rect x="34" y="2" width="32" height="9" rx="2" fill="#1a1a2e"/><line x1="63" y1="4" x2="76" y2="14" stroke="#FFD700" strokeWidth="2.5" strokeLinecap="round"/><circle cx="77" cy="15" r="4" fill="#FFD700"/></g>);
  const GafasNerd=()=>(<g transform={`translate(0,${dy})`}><circle cx="37" cy="42" r="9" fill="none" stroke="#222" strokeWidth="2.5"/><circle cx="63" cy="42" r="9" fill="none" stroke="#222" strokeWidth="2.5"/><line x1="46" y1="42" x2="54" y2="42" stroke="#222" strokeWidth="2.5"/><line x1="28" y1="41" x2="22" y2="39" stroke="#222" strokeWidth="2.5" strokeLinecap="round"/><line x1="72" y1="41" x2="78" y2="39" stroke="#222" strokeWidth="2.5" strokeLinecap="round"/><circle cx="37" cy="42" r="6" fill="#E3F2FD" opacity="0.4"/><circle cx="63" cy="42" r="6" fill="#E3F2FD" opacity="0.4"/></g>);
  const CapaHeroe=()=>(<g transform={`translate(0,${dy})`}><rect x="36" y="56" width="28" height="6" rx="2" fill="#FFD700"/><path d="M36 62 Q30 72 32 90 L50 84 L68 90 Q70 72 64 62 Z" fill="#E53935"/><path d="M36 62 Q43 68 50 65 Q57 68 64 62" fill="#B71C1C" opacity="0.7"/></g>);
  const Corona=()=>(<g transform={`translate(0,${dy})`}><path d="M28 20 L28 8 L38 15 L50 4 L62 15 L72 8 L72 20 Z" fill="#FFD700"/><line x1="28" y1="20" x2="72" y2="20" stroke="#F9A825" strokeWidth="2"/><circle cx="50" cy="5" r="4" fill="#E53935"/><circle cx="28" cy="9" r="3" fill="#4CAF50"/><circle cx="72" cy="9" r="3" fill="#4CAF50"/><circle cx="39" cy="13" r="2.5" fill="#2196F3"/><circle cx="61" cy="13" r="2.5" fill="#2196F3"/></g>);
  const GafasLab=()=>(<g transform={`translate(0,${dy})`}><rect x="20" y="36" width="60" height="14" rx="6" fill="#4FC3F7" opacity="0.5" stroke="#0277BD" strokeWidth="2"/><line x1="50" y1="36" x2="50" y2="50" stroke="#0277BD" strokeWidth="1.5"/><line x1="20" y1="43" x2="12" y2="41" stroke="#0277BD" strokeWidth="2.5" strokeLinecap="round"/><line x1="80" y1="43" x2="88" y2="41" stroke="#0277BD" strokeWidth="2.5" strokeLinecap="round"/></g>);
  const BataBlanca=()=>(<g transform={`translate(0,${dy})`}><path d="M28 55 L20 95 L80 95 L72 55 Q60 62 50 60 Q40 62 28 55Z" fill="white" stroke="#B0BEC5" strokeWidth="1.5"/><path d="M50 60 L50 95" stroke="#CFD8DC" strokeWidth="1"/><path d="M38 57 L36 75" stroke="#CFD8DC" strokeWidth="1.2"/><path d="M62 57 L64 75" stroke="#CFD8DC" strokeWidth="1.2"/><circle cx="50" cy="70" r="2" fill="#90A4AE"/><circle cx="50" cy="78" r="2" fill="#90A4AE"/><circle cx="50" cy="86" r="2" fill="#90A4AE"/></g>);
  const SombreroPaja=()=>(<g transform={`translate(0,${dy})`}><ellipse cx="50" cy="20" rx="38" ry="7" fill="#F9A825"/><path d="M26 20 Q28 4 50 2 Q72 4 74 20" fill="#F9A825"/><ellipse cx="50" cy="19" rx="24" ry="4" fill="#F57F17" opacity="0.45"/><path d="M30 18 Q50 24 70 18" stroke="#E65100" strokeWidth="1" fill="none" opacity="0.5"/></g>);
  const TrajeEspacial=()=>(<g transform={`translate(0,${dy})`}><circle cx="50" cy="38" r="32" fill="#E3F2FD" stroke="#1565C0" strokeWidth="3" opacity="0.88"/><ellipse cx="45" cy="28" rx="10" ry="7" fill="white" opacity="0.45"/><circle cx="50" cy="38" r="32" fill="none" stroke="#90CAF9" strokeWidth="1.5" opacity="0.5"/><rect x="34" y="68" width="32" height="6" rx="3" fill="#B0BEC5"/></g>);
  const SombreroVaquero=()=>(<g transform={`translate(0,${dy})`}><ellipse cx="50" cy="22" rx="40" ry="8" fill="#6D4C41"/><path d="M24 22 Q26 4 50 2 Q74 4 76 22" fill="#795548"/><ellipse cx="50" cy="21" rx="22" ry="5" fill="#5D4037" opacity="0.5"/><path d="M26 20 Q50 28 74 20" stroke="#4E342E" strokeWidth="1.5" fill="none"/></g>);
  const CascoAntiguo=()=>(<g transform={`translate(0,${dy})`}><path d="M18 28 Q16 6 50 4 Q84 6 82 28 L82 32 L18 32 Z" fill="#B0BEC5"/><rect x="44" y="4" width="12" height="22" rx="3" fill="#CFD8DC"/><ellipse cx="50" cy="32" rx="32" ry="6" fill="#90A4AE"/><path d="M18 32 L10 50 L20 50 L24 32" fill="#B0BEC5"/><path d="M82 32 L90 50 L80 50 L76 32" fill="#B0BEC5"/></g>);
  const LupaArqueologo=()=>(<g transform={`translate(0,${dy})`}><circle cx="68" cy="30" r="18" fill="none" stroke="#37474F" strokeWidth="3"/><circle cx="68" cy="30" r="14" fill="#E3F2FD" opacity="0.55"/><circle cx="63" cy="25" r="4" fill="white" opacity="0.4"/><line x1="54" y1="44" x2="38" y2="62" stroke="#37474F" strokeWidth="4" strokeLinecap="round"/></g>);
  const Pergamino=()=>(<g transform={`translate(0,${dy})`}><rect x="62" y="20" width="26" height="38" rx="4" fill="#FFF9C4" stroke="#F9A825" strokeWidth="1.5"/><ellipse cx="75" cy="20" rx="13" ry="4" fill="#FFE082"/><ellipse cx="75" cy="58" rx="13" ry="4" fill="#FFE082"/><line x1="67" y1="29" x2="84" y2="29" stroke="#F9A825" strokeWidth="1.5"/><line x1="67" y1="36" x2="84" y2="36" stroke="#F9A825" strokeWidth="1.5"/><line x1="67" y1="43" x2="80" y2="43" stroke="#F9A825" strokeWidth="1.5"/><line x1="67" y1="50" x2="78" y2="50" stroke="#F9A825" strokeWidth="1.2"/></g>);
  const Pluma=()=>(<g transform={`translate(0,${dy})`}><path d="M72 8 Q88 20 78 38 Q68 50 60 60" fill="#FFF8E1" stroke="#FFA000" strokeWidth="1.5"/><path d="M72 8 Q62 22 60 60" fill="#FFE082" opacity="0.65"/><path d="M60 60 L56 76" stroke="#5D4037" strokeWidth="2.5" strokeLinecap="round"/><path d="M76 12 Q82 18 78 26" stroke="#FFE0B2" strokeWidth="1" fill="none"/></g>);
  const SombreroTeatro=()=>(<g transform={`translate(0,${dy})`}><ellipse cx="50" cy="24" rx="36" ry="7" fill="#6A1B9A"/><path d="M28 24 Q30 6 50 4 Q70 6 72 24" fill="#4A148C"/><ellipse cx="50" cy="23" rx="20" ry="4.5" fill="#7B1FA2" opacity="0.5"/><circle cx="40" cy="14" r="3" fill="#FFD700"/><circle cx="60" cy="14" r="3" fill="#FFD700"/><circle cx="50" cy="10" r="3.5" fill="#FF6F00"/></g>);
  const LibroMagico=()=>(<g transform={`translate(0,${dy})`}><rect x="60" y="18" width="30" height="38" rx="4" fill="#7B1FA2"/><rect x="62" y="20" width="26" height="34" rx="2" fill="#CE93D8"/><line x1="75" y1="20" x2="75" y2="54" stroke="#7B1FA2" strokeWidth="2"/><path d="M64 29 Q75 23 86 29" stroke="#FFD700" strokeWidth="1.5" fill="none"/><circle cx="75" cy="39" r="5" fill="#FFD700" opacity="0.85"/><path d="M72 37 L75 34 L78 37 L77 41 L73 41 Z" fill="#FFF176"/></g>);
  const EstrellaAutor=()=>(<g transform={`translate(0,${dy})`}><path d="M50 4 L55 16 L68 16 L58 24 L62 37 L50 29 L38 37 L42 24 L32 16 L45 16 Z" fill="#FFD700" stroke="#F9A825" strokeWidth="1"/><path d="M50 4 L53 12 L60 12 L54 17 L57 25 L50 20 L43 25 L46 17 L40 12 L47 12 Z" fill="#FFF176" opacity="0.6"/></g>);
...
const FloatingMascot = ({ subject, progress, style={}, animal=null }) => {
  const m = resolveMascot(animal, subject);
...
const TeacherApp=({state,setState,teacherId,onLogout})=>{
  ...
  const myGroups=state.groups.filter(g=>
    String(g.teacherId)===String(teacher?.id) ||
    (g.subjects||[]).some(s=>s.teacherId&&String(s.teacherId)===String(teacher?.id))
  );
  // Per-teacher: which subjects they teach in each group (string-safe)
  const myGroupSubjects=(g)=>(g.subjects||[]).filter(s=>s.teacherId&&String(s.teacherId)===String(teacher?.id));
  ...
  const groupStudents=selectedGroup?state.students.filter(s=>(selectedGroup.students||[]).includes(s.id)||s.group===selectedGroup.name):[];
              ...
              {subjectsForTeacher.map((s,i)=>{
                ...
                return(
                  <div key={i} onClick={()=>{setSelectedGroupSubject(s.subject);setClassTab("board");}}
                    style={{background:"#fff",borderRadius:16,marginBottom:10,overflow:"hidden",
                      boxShadow:"0 2px 10px rgba(0,0,0,0.07)",cursor:"pointer",
                      border:`1.5px solid ${m.color}25`,transition:"transform 0.12s,box-shadow 0.12s"}}
                    onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow=`0 6px 20px ${m.color}30`;}}
                    onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.07)";}}>
                    <div style={{background:`linear-gradient(135deg,${m.color},${m.color}bb)`,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:46,height:46,borderRadius:14,background:"rgba(255,255,255,0.22)",
                        display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0}}>
                        {m.emoji}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:17,fontWeight:700,color:"#fff",fontFamily:SFD,letterSpacing:"-0.3px"}}>{s.subject}</div>
                      ...
                      <svg width="8" height="14" viewBox="0 0 8 14" fill="none">
                        <path d="M1 1L7 7L1 13" stroke="rgba(255,255,255,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </div>
                  </div>
                );
                  ...
                  return(
                    <div key={i}>
                      <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",opacity:0.65}}>
                        <div style={{width:36,height:36,borderRadius:10,background:`${m.color}18`,
                          display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{m.emoji}</div>
                        <div style={{flex:1}}>
                  ...
                  <Btn onPress={()=>{
                    ...
                    groupStudents.forEach(s=>{
                      ...
                      if(typeof s.id==="string"){
                        ...
                        updateDoc(doc(db,"students",s.id),parcialData).catch(()=>{});
                      }
                    });
                        ...
                        {myGroupSubjects(g).length>0&&(
                          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:10}}>
                            {myGroupSubjects(g).map((s,i)=>{
                              const m=MASCOTS[s.subject]||MASCOTS["Matemáticas"];
                              return(
...
const StudentApp=({state,setState,studentId,onLogout})=>{
  ...
  useEffect(()=>{if(profilePic)localStorage.setItem(`lms_pic_${studentId}`,profilePic);},[profilePic,studentId]);
    ...
    return(
      <div style={{background:T.bg,minHeight:"100vh",fontFamily:SF}}>
        ...
        <div style={{background:`linear-gradient(135deg,${m.color},${m.color}bb)`}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px 10px"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:"rgba(255,255,255,0.75)",fontFamily:SF}}>{m.name} · Nv.{Math.floor(progress/20)+1}</div>
            </div>
            <div style={{animation:"mascotBob 2s ease-in-out infinite"}}>
        ...
        <div className="page-pad">
          {subjTab==="tablon"&&(
            <>
              <button onClick={()=>{setShopFrom("subject");setShopSubject(selectedSubject);}}
                style={{width:"100%",background:`linear-gradient(135deg,${m.color},${m.color}cc)`,color:"#fff",border:"none",borderRadius:12,padding:"11px",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:SF,display:"flex",alignItems:"center",justifyContent:"center",gap:7,boxShadow:`0 4px 14px ${m.color}35`,marginBottom:14}}>
                👕 Tienda de Accesorios · {currentTokens} 🪙
              </button>
              <Card style={{padding:14,marginBottom:12,background:`${m.color}08`}}>
                <div style={{height:10,background:C.fill3,borderRadius:5,overflow:"hidden",marginBottom:8}}>
                  <div style={{height:"100%",width:`${progress}%`,borderRadius:5,background:`linear-gradient(90deg,${m.color},${m.color}bb)`,transition:"width 0.6s ease"}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <span style={{fontSize:12,color:C.lbl2,fontFamily:SF}}>{tasksDone} tareas completadas</span>
                  <span style={{fontSize:12,color:"#9A7108",fontWeight:600,fontFamily:SF}}>{currentTokens} 🪙</span>
                </div>
              </Card>
              {allActivities.length===0&&<div style={{textAlign:"center",padding:"40px 16px",color:C.lbl2,fontSize:15,fontFamily:SF}}><div style={{fontSize:40,marginBottom:10}}>🎉</div>Sin actividades publicadas aún</div>}
              {allActivities.map(c=>{
                const doneKey=`done_${c.id}`;
                const done=!!(completedTasks[doneKey]);
                return(
                  <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"#fff",borderRadius:12,marginBottom:8,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
                    <div style={{width:40,height:40,borderRadius:12,background:done?`${C.green}18`:`${m.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{done?"✅":c.type==="tarea"?"📝":c.type==="examen"?"📋":"⚡"}</div>
                    <div style={{flex:1}}>
                      <div style={{...fmt.callout,fontWeight:600,fontFamily:SF,color:done?C.lbl2:C.lbl,textDecoration:done?"line-through":"none"}}>{c.title}</div>
                      <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF}}>{done?"¡Completado! +2 🪙":`Entrega: ${c.date||"—"}`}</div>
                    </div>
                    {!done&&<button onClick={()=>{setCompletedTasks(t=>({...t,[doneKey]:true,[selectedSubject]:(t[selectedSubject]||0)+1}));setTokens(p=>(p!==null?p:currentTokens)+2);}} style={{background:`${m.color}18`,color:m.color,border:`1px solid ${m.color}40`,borderRadius:10,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SF,whiteSpace:"nowrap"}}>✓ +2🪙</button>}
                  </div>
                );
              })}
            </>
                      ...
                      return(
                        <Card key={c.id} style={{marginBottom:8}}>
                          <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10}}>
                            <div style={{width:38,height:38,borderRadius:10,background:`${m.color}12`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{c.type==="tarea"?"📝":c.type==="examen"?"📋":"⚡"}</div>
                            <div style={{flex:1}}>
                              <div style={{fontSize:14,fontWeight:600,color:C.lbl,fontFamily:SF}}>{c.title}</div>
                              <div style={{fontSize:11,color:C.lbl2,fontFamily:SF,marginTop:2}}>{c.points||10} pts · {c.date||"—"}</div>
                            </div>
          ...
          {subjTab==="calificaciones"&&(
            <Card style={{padding:16,marginBottom:14}}>
              ...
              <div style={{marginTop:14,paddingTop:12,borderTop:`1px solid ${C.sep}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                {sData.grade!=null?(<div style={{background:`${sData.grade>=9?C.green:sData.grade>=7?m.color:C.orange}20`,borderRadius:12,padding:"6px 16px"}}><span style={{fontSize:22,fontWeight:900,color:sData.grade>=9?C.green:sData.grade>=7?m.color:C.orange,fontFamily:SFD}}>{sData.grade.toFixed(1)}</span></div>):(<span style={{fontSize:12,color:C.orange,fontWeight:700,background:`${C.orange}12`,borderRadius:8,padding:"5px 12px",fontFamily:SF}}>Pendiente</span>)}
              </div>
            </Card>
          )}
        </div>
  ...
  return(
    <div className="app-layout" style={{background:T.bg,minHeight:"100vh",fontFamily:SF}}>
      ...
      {(tab==="subjects"||tab==="medals")&&mySubjects.map((subj,i)=>(
        <FloatingMascot key={subj} subject={subj} animal={subjectMascotMap[subj]||null} progress={getMascotProgress(subj)} style={{zIndex:10+(i*2)}}/>
      ))}
      ...
      {tab==="feed"&&(
        <div style={{position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",zIndex:2}}>
            {/* Class activities section — shown before general feed */}
            {myContent.length>0&&(
              <div style={{padding:"8px 16px 0"}}>
                ...
                {myContent.slice().reverse().map(c=>{
                  ...
                  const done=!!(completedTasks[doneKey]);
                  return(
                    <div key={c.id} style={{background:"#fff",borderRadius:14,marginBottom:10,
                      ...
                      <div style={{background:`linear-gradient(135deg,${typeColor}15,${typeColor}05)`,
                        ...
                        <span style={{fontSize:10,fontWeight:700,color:typeColor,background:`${typeColor}15`,
                          borderRadius:8,padding:"2px 8px",fontFamily:SF,flexShrink:0}}>{typeLabel}</span>
                      </div>
                      ...
                      <div style={{padding:"8px 14px 10px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                        ...
                        {done
                          ?<span style={{fontSize:11,fontWeight:700,color:C.green,background:`${C.green}12`,borderRadius:8,padding:"3px 9px",fontFamily:SF}}>✅ Completado</span>
                          :<button onClick={()=>{setCompletedTasks(t=>({...t,[doneKey]:true,[c.subject||""]:(t[c.subject||""]||0)+1}));setTokens(p=>(p!==null?p:currentTokens)+2);}}
                            style={{background:`${typeColor}18`,color:typeColor,border:`1px solid ${typeColor}40`,
                              borderRadius:8,padding:"4px 12px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SF}}>
                            ✓ Marcar +2🪙
                          </button>
                        }
                      </div>
                  ...
                  <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:14}}>
                    <div style={{position:"relative",flexShrink:0}}>
                      <div style={{width:56,height:56,borderRadius:16,background:`${m.color}12`,
                        ...
                        <MascotSVG subject={subj} animal={subjectMascotMap[subj]||null} outfit={equipped} size={50}/>
                      </div>
                    ...
                    <div style={{textAlign:"right"}}>
                      {sData.grade!=null
                        ?<><div style={{fontSize:20,fontWeight:700,color:sData.grade>=9?C.green:sData.grade>=7?T.accent:C.orange,fontFamily:SF}}>{Number(sData.grade).toFixed(1)}</div><div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>/ 10</div></>
                        :<span style={{fontSize:11,color:C.orange,fontWeight:600,fontFamily:SF}}>—</span>
                      }
                    </div>
                    <svg width="8" height="13" viewBox="0 0 8 13" fill="none"><path d="M1 1L7 6.5L1 12" stroke={C.g3} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </div>
                  ...
                  <tbody>
                    {mySubjects.map((subj,i)=>{
                      ...
                      const avgParciales=allFilled
                        ...
                        : 0;
                      const finalGrade=allFilled?avgParciales:0;
                      return(
                        <tr key={subj} style={{borderTop:i>0?`0.5px solid ${C.sep}`:"none"}}>
                          ...
                          <td style={{textAlign:"center",padding:"10px 4px",verticalAlign:"middle"}}>
                            <div style={{background:`${allFilled&&finalGrade>=9?C.green:allFilled&&finalGrade>=7?T.accent:allFilled?C.orange:C.fill3}18`,
                              borderRadius:8,padding:"3px 6px",display:"inline-block"}}>
                              <span style={{fontSize:14,fontWeight:800,
                                color:allFilled&&finalGrade>=9?C.green:allFilled&&finalGrade>=7?T.accent:allFilled?C.orange:C.lbl3,fontFamily:SF}}>
                                {allFilled?finalGrade.toFixed(1):"0"}
                              </span>
                            </div>
                          </td>
                        </tr>
...
const NotifToastLayer = () => {
  ...
  return (
    <div className="notif-layer">
      {notifs.map(n => (
        <div key={n.id} style={{
          background:n.urgent?"linear-gradient(135deg,#FF3B30,#FF6B35)":"linear-gradient(135deg,#007AFF,#34C759)",
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,fontFamily:"-apple-system,sans-serif",opacity:0.9,lineHeight:1.4}}>{n.text}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function App(){
  ...
  const fbSetState = useCallback((updater) => {
    setState(prev => {
      ...
      // Sync attendance and student subjects
      ...
      // Sync approvedContent: save newly approved items to Firestore
