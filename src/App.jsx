...
const DeveloperApp=({state,setState,onLogout})=>{
  ...
  const addSubjectToGroup=(gId)=>{
    ...
    const entry={subject:newSubjInput.trim(),teacherId:newSubjTeacher?Number(newSubjTeacher):null,mascot:newSubjMascot||null};
    setState(prev=>{
  ...
  const removeSubjectFromGroup=(gId,subjName)=>{
    setState(prev=>{
      ...
      return{...prev,groups:prev.groups.map(g=>g.id===gId?{...g,subjects:newSubjects}:g)};
    });
  };
  ...
  const assignStudentToGroup=async(student,grpId)=>{
    ...
    if(typeof grp.id==="string"){
      try{
        const {doc:fiDoc,updateDoc:fiUpd}=await import("firebase/firestore").catch(()=>({}));
        if(fiDoc&&fiUpd) await fiUpd(fiDoc(db,"groups",grp.id),{students:newStudents});
      }catch{}
    }
                  ...
                  {isExp&&(
                    <div style={{borderTop:`0.5px solid ${C.sep}`,padding:"12px 16px"}}>
                      ...
                      {groupSubjects.map((subj,si)=>{
                        const tch=state.teachers.find(t=>Number(t.id)===Number(subj.teacherId));
                            ...
                            <div style={{padding:"8px 14px",borderBottom:`0.5px solid ${C.sep}`}}>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                ...
                                <div style={{flex:1}}>
                                  <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>{teacher.name}</div>
                                  <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF}}>
                                    {[...new Set((state.groups||[]).flatMap(g=>(g.subjects||[]).filter(s=>Number(s.teacherId)===Number(teacher.id)).map(s=>s.subject)))].join(", ")||(teacher.subjects||[]).join(", ")||"Sin materias"}
                                  </div>
                                </div>
...
const TeacherApp=({state,setState,teacherId,onLogout})=>{
  ...
  // Include groups where this teacher is assigned to ANY subject (not just top-level teacherId)
  const myGroups=state.groups.filter(g=>
    g.teacherId===teacher?.id ||
    (g.subjects||[]).some(s=>Number(s.teacherId)===Number(teacher?.id))
  );
  // Per-teacher: which subjects they teach in each group
  const myGroupSubjects=(g)=>(g.subjects||[]).filter(s=>Number(s.teacherId)===Number(teacher?.id));
            ...
            <>
              ...
              <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
                {allSubjects.filter(s=>!subjectsForTeacher.find(ms=>ms.subject===s.subject)).map((s,i,arr)=>{
                  const otherTeacher = state.teachers.find(t=>Number(t.id)===Number(s.teacherId));
                  const m = MASCOTS[s.subject]||MASCOTS["Matemáticas"];
                  return(
                    <div key={i}>
  ...
  if(selectedSubject){
    ...
    return(
      <div style={{background:T.bg,minHeight:"100vh",fontFamily:SF,position:"relative",overflow:"hidden"}}>
        <FloatingMascot subject={selectedSubject} animal={subjectMascotMap[selectedSubject]||null} progress={progress}/>
            ...
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <div style={{position:"relative",flexShrink:0}}>
                <div style={{width:mascotPx+28,height:mascotPx+28,borderRadius:20,background:`${m.color}18`,
                  ...
                  <MascotSVG subject={selectedSubject} animal={subjectMascotMap[selectedSubject]||null} outfit={equipped} size={mascotPx+16}/>
                </div>
