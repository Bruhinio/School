...
// Returns { isMobile, isTablet, isDesktop, bp }
...
// Usage: const R = useR(); then R.sp, R.fs.body, R.avatar.md, etc.
const useR = () => {
  ...
  return tokens[bp] || tokens.mobile;
};
...
const TabBar=({tabs,active,onChange,accent=C.blue})=>{
  const R = useR();
  if (R.tabH === 0) return null; // desktop: hidden, sidebar takes over
  return(
    <div className="tab-bar-bottom">
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>{SFX.play("click");onChange(t.id);}} style={{flex:1,display:"flex",flexDirection:"column",
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              {Object.entries(HOLIDAY_THEMES).map(([key,ht])=>{
                const [currentThemeKey,setCurrentThemeKey]=useState(_globalHolidayTheme);
            ...
            {(()=>{
              const HolidayGrid=()=>{
                const [currentKey,setCurrentKey]=useState(_globalHolidayTheme);
                return(
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                    {Object.entries(HOLIDAY_THEMES).map(([key,ht])=>(
                      <div key={key}
                        onClick={()=>{ SFX.play("click"); setGlobalHolidayTheme(key); setCurrentKey(key); pushNotification({title:"🎨 Tema aplicado",text:`Tema "${ht.name}" activado en toda la plataforma.`}); }}
                        style={{
                          background:currentKey===key?ht.gradient||devColor+"20":`linear-gradient(135deg,${ht.bg}ee,${ht.bg}aa)`,
                          border:`2px solid ${currentKey===key?ht.accent:"transparent"}`,
                          borderRadius:16,padding:"16px 12px",cursor:"pointer",
                          textAlign:"center",transition:"all 0.3s",
                          boxShadow:currentKey===key?"0 4px 16px rgba(0,0,0,0.2)":"0 1px 4px rgba(0,0,0,0.08)"
                        }}>
                        <div style={{fontSize:32,marginBottom:8}}>{ht.emoji}</div>
                        <div style={{fontWeight:700,fontSize:13,fontFamily:SF,color:currentKey===key?"#fff":C.lbl,marginBottom:4}}>{ht.name}</div>
                        {currentKey===key&&(
                          <div style={{fontSize:10,fontWeight:700,color:"#fff",background:"rgba(255,255,255,0.25)",borderRadius:8,padding:"2px 8px",display:"inline-block",fontFamily:SF}}>✓ Activo</div>
                        )}
    ...
    <>
      ...
      {screen==="app"&&(
        <div className="app-shell">
          {/* Sidebar — visible only on desktop ≥1024px */}
          {role&&<SidebarNav role={role} state={state} onLogout={logout}/>}
          {/* Main content */}
