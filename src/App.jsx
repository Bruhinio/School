...
const Feed=({state,setState,userId,userName,userAvatar,userColor,userRole,accent=C.blue,newsItems=[],urgentCount=0})=>{
  ...
  const deletePost=(id)=>setState(s=>({...s,posts:s.posts.filter(p=>p.id!==id)}));
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"13px 14px 8px"}}>
                ...
                {canModify(post)&&(
                  <div style={{display:"flex",gap:4,marginLeft:4}}>
                    <button onClick={()=>startEdit(post)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:"2px 5px",borderRadius:6,color:C.lbl3}} title="Editar">✏️</button>
                    <button onClick={()=>deletePost(post.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:"2px 5px",borderRadius:6,color:C.red}} title="Eliminar">🗑️</button>
                  </div>
                )}
              </div>
...
const ChatPanel=({state,setState,myUserId,myName,myAvatar,myColor,role,accent=C.blue})=>{
  ...
  if(view==="list"||view==="new"){
    ...
    return(
      <div style={{background:"#fff",minHeight:"100vh",fontFamily:SF_local,display:"flex",flexDirection:"column"}}>
  ...
  if(view==="chat"&&selectedChat){
    ...
    return(
      <div style={{background:"#f0f2f5",
        height:"calc(100dvh - 68px - var(--tab-h,84px))",
        maxHeight:"calc(100dvh - 68px - var(--tab-h,84px))",
        fontFamily:SF_local,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* Messenger Header */}
        ...
        <div style={{flexShrink:0,background:"#fff",borderTop:"1px solid #e4e6eb",
          ...
          {(msgText.trim()||attachImages.length>0||attachFiles.length>0)?(
            <button onClick={sendMessage}
              style={{width:38,height:38,borderRadius:"50%",background:MSN_BLUE,border:"none",
                cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",
                boxShadow:"0 2px 10px rgba(0,132,255,0.4)",transition:"transform 0.1s"}}
            ...
            <button style={{width:38,height:38,borderRadius:"50%",background:"none",border:"none",
              cursor:"pointer",fontSize:24,flexShrink:0,display:"flex",alignItems:"center",
              justifyContent:"center",color:MSN_BLUE}}>👍</button>
          )}
        </div>
      </div>
    );
    ...
    <div style={{position:"fixed",top:78,right:16,zIndex:500}}>
      <button onClick={()=>setDirBellOpen(o=>!o)}
        style={{width:38,height:38,borderRadius:"50%",
          background:dirBellOpen?C.fill3:`linear-gradient(135deg,${C.indigo},${C.indigo}bb)`,
          border:"none",cursor:"pointer",display:"flex",alignItems:"center",
...
const TeacherApp=({state,setState,teacherId,onLogout})=>{
  ...
  return(
    <div className="app-layout" style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
      ...
      {tab==="classes"&&(
        <div>
          <NavBar title="Mis Clases" large accent={ACCENT} right={<BellBtn/>}/>
          <div style={{padding:"0 16px 130px"}}>
            {myGroups.length===0&&(
      ...
      {tab==="attendance"&&(
        <div>
          <NavBar title="Asistencia" large accent={ACCENT} right={<BellBtn/>}/>
          <div className="page-pad">
            {myGroups.length===0?(
      ...
      {tab==="settings"&&(
        <div>
          <NavBar title="Ajustes" large accent={ACCENT} right={<BellBtn/>}/>
          <div className="page-pad">
            {/* Profile photo */}
