...
const ChatPanel=({state,setState,myUserId,myName,myAvatar,myColor,role,accent=C.blue})=>{
  ...
  const getContacts=()=>{
    ...
    if(role==="director"){
      ...
      const myStudents=state.students.filter(s=>myGroupIds.some(gid=>{const g=state.groups.find(x=>x.id===gid);return(g?.students||[]).some(id=>String(id)===String(s.id));}));
  ...
  const getOrCreateChat=(contactId,contactName)=>{
    ...
    if(!chats.find(c=>c.id===chatId)){
      ...
      setState(s=>({...s,chats:[newChat,...(s.chats||[])],chatMessages:{...s.chatMessages,[chatId]:[]}}));
    }
    return chatId;
  };
  ...
  const sendMessage=()=>{
    ...
    const msg={id:Date.now(),from:myUserId,fromName:myName,
      ...
      type:"text",time:new Date().toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"}),
      date:new Date().toISOString().slice(0,10)};
    const newMsgs=[...(allMessages[selectedChatId]||[]),msg];
    const previewText=msg.text||(msg.images?.length?"📷 Foto":"📎 Archivo");
    setState(s=>({
      ...s,
      chatMessages:{...s.chatMessages,[selectedChatId]:newMsgs},
      chats:(s.chats||[]).map(c=>c.id===selectedChatId?{...c,lastMsg:previewText,lastTime:"Ahora",unread:{...Object.fromEntries((c.participants||[]).filter(p=>p!==myUserId).map(p=>[p,(c.unread?.[p]||0)+1]))},[myUserId]:0}:c)
    }));
  ...
  if(view==="chat"&&selectedChat){
    ...
    return(
      <div style={{background:"#f0f2f5",minHeight:"100vh",fontFamily:SF_local,display:"flex",flexDirection:"column"}}>
        {/* Header */}
        <div style={{background:`linear-gradient(135deg,${accent},${accent}cc)`,position:"sticky",top:0,zIndex:100}}>
        ...
        {/* Messages */}
        <div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:6,paddingBottom:120}}>
        ...
        {/* Poll form */}
        {showPoll&&(
          <div style={{position:"fixed",bottom:70,left:0,right:0,zIndex:200,padding:"0 12px 8px"}}>
            <div style={{background:"#fff",borderRadius:16,padding:16,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>
        ...
        {(attachImages.length>0||attachFiles.length>0)&&(
          <div style={{position:"fixed",bottom:65,left:0,right:0,zIndex:150,padding:"8px 12px",background:"rgba(255,255,255,0.95)",backdropFilter:"blur(10px)",borderTop:`0.5px solid ${C.sep}`}}>
        ...
        {/* Input bar */}
        <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:200,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(12px)",borderTop:`0.5px solid ${C.sep}`,padding:"8px 12px 20px",display:"flex",gap:8,alignItems:"flex-end"}}>
          <input ref={imgRef} type="file" accept="image/*" multiple style={{display:"none"}} onChange={addImg}/>
          <input ref={fileRef} type="file" multiple style={{display:"none"}} onChange={addFile}/>
          <button onClick={()=>imgRef.current?.click()}
            style={{width:36,height:36,borderRadius:"50%",background:`${accent}12`,border:"none",cursor:"pointer",fontSize:18,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>📷</button>
          <button onClick={()=>fileRef.current?.click()}
            style={{width:36,height:36,borderRadius:"50%",background:`${accent}12`,border:"none",cursor:"pointer",fontSize:18,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>📎</button>
          <button onClick={()=>setShowPoll(s=>!s)}
            style={{width:36,height:36,borderRadius:"50%",background:showPoll?accent:`${accent}12`,border:"none",cursor:"pointer",fontSize:18,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>📊</button>
          <div style={{flex:1,background:C.fill4,borderRadius:20,display:"flex",alignItems:"flex-end",padding:"0 4px 0 12px",minHeight:36}}>
            <textarea value={msgText} onChange={e=>setMsgText(e.target.value)}
              onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}}
              placeholder="Mensaje…" rows={1}
              style={{flex:1,background:"transparent",border:"none",padding:"9px 0",fontSize:14,color:C.lbl,fontFamily:SF_local,outline:"none",resize:"none",lineHeight:"18px",maxHeight:90,overflowY:"auto"}}/>
          </div>
          <button onClick={sendMessage} disabled={!msgText.trim()&&attachImages.length===0&&attachFiles.length===0}
            style={{width:36,height:36,borderRadius:"50%",background:(msgText.trim()||attachImages.length>0||attachFiles.length>0)?accent:C.fill3,
              border:"none",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>
            <svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M12.5 1.5L7 7M12.5 1.5L8.5 12.5L7 7M12.5 1.5L1.5 5.5L7 7" stroke={(msgText.trim()||attachImages.length>0||attachFiles.length>0)?"#fff":C.g3} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </button>
        </div>
      </div>
    );
      ...
      {tab==="classes"&&(
        <div>
          ...
          <div style={{padding:"0 16px 100px"}}>
            ...
            {myGroups.map(g=>{
              const studs=state.students.filter(s=>(g.students||[]).some(id=>String(id)===String(s.id)));
              ...
              return(
                <Card key={subj} style={{marginBottom:10}} onPress={()=>{setSelectedSubject(subj);localStorage.setItem(newActKey,String(Math.floor(Date.now()/1000)));}}>
                  <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:14}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      {(()=>{
                        const manualF=student?.[`final_${subj}`]!=null?Number(student[`final_${subj}`]):null;
                        const pList=getStudentParciales(subj);
                        const allP=pList.every(({g})=>g!=null);
                        const avgP=allP?pList.reduce((a,{g})=>a+(g||0),0)/pList.length:null;
                        const displayG=manualF??avgP??sData.grade;
                        return displayG!=null
                          ?<><div style={{fontSize:20,fontWeight:700,color:displayG>=9?C.green:displayG>=7?T.accent:C.orange,fontFamily:SF}}>{Number(displayG).toFixed(1)}</div><div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>/ 10</div></>
