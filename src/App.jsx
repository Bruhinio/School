...
const TeacherApp=({state,setState,teacherId,onLogout})=>{
  ...
  const [sidebarOpen,setSidebarOpen]=useState(true);
  const [selectedGroup,setSelectedGroup]=useState(null);
  const [selectedGroupSubject,setSelectedGroupSubject]=useState(null); // subject string selected inside a group
  ...
  if(selectedGroup){
    const COLOR = selectedGroupSubject
      ? (MASCOTS[selectedGroupSubject]?.color||C.blue)
      : C.blue;
...
const getShopItems=(subject)=>MASCOT_SHOP[subject]||MASCOT_SHOP._default.map(i=>({...i,id:i.id+"_"+subject.slice(0,3)}));
...
const StudentApp=({state,setState,studentId,onLogout})=>{
  ...
  if(selectedSubject){
    ...
    const [subjTab,setSubjTab]=React.useState("tablon");
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontFamily:SF}}>
                  ...
                  <tbody>
                    {mySubjects.map((subj,i)=>{
                      ...
                      const finalGrade=subjectData[subj]?.grade||null;
                      return(
                        <tr key={subj} style={{borderTop:i>0?`0.5px solid ${C.sep}`:"none"}}>
                          ...
                          <td style={{textAlign:"center",padding:"10px 4px",verticalAlign:"middle"}}>
                            {finalGrade!=null?(
                              ...
                              <span style={{fontSize:10,color:C.orange,fontFamily:SF,fontWeight:600,background:`${C.orange}12`,borderRadius:6,padding:"3px 7px"}}>Pendiente</span>
                            )}
                          </td>
                        </tr>
                      );
