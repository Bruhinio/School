...
const StudentApp=({state,setState,studentId,onLogout})=>{
  ...
  if(selectedActivity){
    ...
    const submitTarea=()=>{
      ...
      setState(s=>({...s,approvedContent:s.approvedContent.map(c=>
        String(c.id)===String(act.id)?{...c,submissions:[...(c.submissions||[]).filter(x=>String(x.studentId)!==String(student?.id)),sub]}:c
      )}));
      setSelectedActivity({...act,submissions:[...(act.submissions||[]).filter(x=>String(x.studentId)!==String(student?.id)),sub]});
    ...
    return(
      <div style={{background:T.bg,minHeight:"100vh",fontFamily:SF}}>
        <div style={{background:`linear-gradient(135deg,${typeColor},${typeColor}cc)`,padding:"14px 16px 18px"}}>
          ...
          {/* Status badge */}
          {existingSub&&<div style={{background:"rgba(255,255,255,0.95)",borderRadius:10,padding:"8px 14px",display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:16}}>✅</span>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:700,color:C.green,fontFamily:SF}}>Entregado</div>
              {existingSub.score!=null&&<div style={{fontSize:11,color:C.lbl2,fontFamily:SF}}>Calificación: {existingSub.score}/10</div>}
            </div>
          </div>}
        </div>
        ...
        <div className="page-pad">
          {subjTab==="tablon"&&(
            <>
              ...
              {allActivities.length===0&&<div style={{textAlign:"center",padding:"40px 16px",color:C.lbl2,fontSize:15,fontFamily:SF}}><div style={{fontSize:40,marginBottom:10}}>📝</div>Las tareas y actividades aparecen en la pestaña "Tareas"</div>}
              {allActivities.map(c=>{
                const isTaskType=["tarea","actividad","examen","cuestionario"].includes(c.type);
                const doneKey=`done_${c.id}`;
                const done=!!(completedTasks[doneKey]);
                const typeColor={tarea:C.blue,examen:C.orange,actividad:C.purple,cuestionario:C.teal}[c.type]||m.color;
                return(
                  <div key={c.id} onClick={isTaskType?()=>{setSelectedActivity(c);setQuizAnswers({});setQuizSubmitted(false);}:undefined}
                    style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"#fff",borderRadius:12,marginBottom:8,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",cursor:isTaskType?"pointer":"default"}}>
                    <div style={{width:40,height:40,borderRadius:12,background:done?`${C.green}18`:`${typeColor}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>
                      {done?"✅":c.type==="tarea"?"📝":c.type==="examen"?"📋":c.type==="cuestionario"?"❓":"⚡"}
                    </div>
                    <div style={{flex:1}}>
