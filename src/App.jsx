        ...
        <div style={{marginBottom:16}}>
          ...
          <textarea value={localFeedback} onChange={e=>setLocalFeedback(e.target.value)}
            ...
            style={{width:"100%",background:C.fill4,border:"none",borderRadius:12,padding:"12px 14px",
              fontSize:14,color:C.lbl,fontFamily:SF,resize:"none",outline:"none",boxSizing:"border-box",lineHeightconst TeacherApp=({state,setState,teacherId,onLogout})=>{
          ...
          <div style={{display:"flex",alignItems:"center",padding:"12px 16px 0",gap:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:18,fontWeight:700,color:"#fff",fontFamily:SFD,lineHeight:1.2}}>
                {myGroupSubjects(selectedGroup).length>0
                  ? myGroupSubjects(selectedGroup).map(s=>s.subject).join(" · ")
                  : (selectedGroup.subject||"Mi Clase")}
              </div>
                ...
                <Card key={g.id} style={{marginBottom:12,overflow:"hidden"}} onPress={()=>setSelectedGroup(g)}>
                  <div style={{background:`linear-gradient(135deg,${C.blue},${C.blue}bb)`,padding:"14px 16px"}}>
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                      <div>
                        <div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:SFD,letterSpacing:"-0.4px"}}>
                          {myGroupSubjects(g).length>0 ? myGroupSubjects(g).map(s=>s.subject).join(" · ") : (g.subject||"Clase")}
                        </div>
