...
// ─── TEACHER APP ──────────────────────────────────────────────────────────────
// ─── GRADE MODAL ─────────────────────────────────────────────────────────────
const SubmissionRow = ({s, act, gradeDraft, publishedGrades, setGradeModal, isLast}) => {
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:submitted?4:0}}>
          ...
          <div style={{flex:1,cursor:submitted?"pointer":"default"}} onClick={()=>submitted&&setExpanded(e=>!e)}>
            ...
            <div style={{fontSize:11,color:submitted?C.green:C.lbl3,fontFamily:SF}}>
              {submitted?"✅ Entregó"+(sub?.date?` · ${sub.date}`:"")+(sub?.score!=null?` · ${sub.score}/10`:""):"⏳ Sin entrega"}
            </div>
          </div>
          {savedGrade&&(
            ...
            {sub.answers&&(
              <div style={{marginTop:4}}>
                ...
                {(act.quiz||[]).map((q,qi)=>{
                  ...
                  return(
                    <div key={qi} style={{fontSize:11,color:C.lbl2,fontFamily:SF,padding:"2px 0",display:"flex",alignItems:"flex-start",gap:5}}>
                      <span>{isRight?"✅":"❌"}</span>
                      <span style={{flex:1}}>{q.q} → {q.a?.[chosen]||"—"}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
        ...
        <div style={{padding:"0 16px 100px"}}>
          ...
          <Card style={{marginBottom:12}}>
            ...
            {studs.map((s,i)=>(
              <SubmissionRow key={s.id} s={s} act={act} gradeDraft={gradeDraft} publishedGrades={publishedGrades} setGradeModal={setGradeModal} isLast={i===studs.length-1}/>
            ))}
          </Card>
  ...
  if(selectedActivity){
    ...
    const submitTarea=async()=>{
      ...
      setState(s=>({...s,approvedContent:s.approvedContent.map(c=>
        String(c.id)===String(act.id)?{...c,submissions:newSubs}:c
      )}));
      setSelectedActivity({...act,submissions:newSubs});
      ...
      if (act.id && typeof act.id === "string") {
        try {
          const {updateDoc:upd,doc:fdoc} = await import("firebase/firestore");
          await upd(fdoc(db,"approvedContent",act.id), {submissions:newSubs});
        } catch(e){}
      }
    };
