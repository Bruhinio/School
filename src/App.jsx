...
const pushNotification = (msg) => {
  _notifQueue = [..._notifQueue, { id: Date.now(), ...msg }];
  ...
  setTimeout(() => {
    _notifQueue = _notifQueue.filter(n => n.id !== msg.id);
    _notifListeners.forEach(fn => fn([..._notifQueue]));
  }, 5000);
};
      ...
      <div className="page-pad" style={{animation:"fadeUp 0.3s ease"}}>
        ...
        {tab==="groups"&&(()=>{
          ...
          const addSubjectToGroup=(gId)=>{
            ...
            const newEntry={subject:newSubjInput.trim(),teacherId:newSubjTeacher?Number(newSubjTeacher):null};
            setState(prev=>({...prev,
              groups:prev.groups.map(g=>g.id===gId?{...g,
                subjects:[...(g.subjects||[]),newEntry],
                subject:g.subject||newSubjInput.trim()
              }:g)
            }));
            setNewSubjInput("");setNewSubjTeacher("");
          };
          const removeSubjectFromGroup=(gId,subjName)=>{
            setState(prev=>({...prev,
              groups:prev.groups.map(g=>g.id===gId?{...g,
                subjects:(g.subjects||[]).filter(s=>s.subject!==subjName)
              }:g)
            }));
          };
          return(
          <>
            ...
            <div style={{...fmt.footnote,color:C.lbl2,textTransform:"uppercase",letterSpacing:"0.05em",fontFamily:SF,marginBottom:8,paddingLeft:4}}>
              {state.groups.length} grupos — toca para gestionar materias
            </div>
                ...
                <Card key={g.id} style={{marginBottom:10,overflow:"hidden"}}>
                  ...
                  <div onClick={()=>setExpandedGroup(isExp?null:g.id)}
                    ...
                    <div style={{width:44,height:44,borderRadius:12,background:`${devColor}15`,
                      ...
                      <span style={{fontSize:14,fontWeight:800,color:devColor,fontFamily:SF}}>{g.name}</span>
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:11,color:C.lbl2,fontFamily:SF,marginTop:2}}>
                        {studs.length} alumnos · {groupSubjects.length} materia{groupSubjects.length!==1?"s":""}
                      </div>
                    </div>
                    <span style={{fontSize:16,color:C.lbl2,transition:"transform 0.2s",
                  ...
                  {isExp&&(
                    <div style={{borderTop:`0.5px solid ${C.sep}`,padding:"12px 16px"}}>
                      <div style={{fontSize:12,fontWeight:700,color:C.lbl2,textTransform:"uppercase",
                        letterSpacing:"0.05em",fontFamily:SF,marginBottom:10}}>📚 Materias del Grupo</div>
                      {groupSubjects.length===0&&(
                        <div style={{textAlign:"center",padding:"12px 0",color:C.lbl2,fontSize:13,fontFamily:SF}}>
                          Sin materias. Agrega abajo.
                        </div>
                      )}
                      {groupSubjects.map((subj,si)=>{
                        const tch=state.teachers.find(t=>t.id===subj.teacherId||t.id===Number(subj.teacherId));
                        return(
                          <div key={si} style={{display:"flex",alignItems:"center",gap:10,
                            padding:"8px 12px",background:C.fill4,borderRadius:10,marginBottom:6}}>
                            <span style={{fontSize:16}}>📖</span>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:13,fontWeight:600,color:C.lbl,fontFamily:SF}}>{subj.subject}</div>
                      ...
                      {/* Add subject row */}
                      <div style={{marginTop:10,padding:"10px 12px",background:`${devColor}08`,borderRadius:10,
                        border:`1px dashed ${devColor}30`}}>
                        <div style={{fontSize:11,fontWeight:700,color:devColor,fontFamily:SF,marginBottom:8}}>
                          ➕ Agregar Materia
                        </div>
                        <input value={newSubjInput} onChange={e=>setNewSubjInput(e.target.value)}
                          placeholder="Nombre de materia (ej: Matemáticas)"
                          style={{width:"100%",background:"#fff",border:`1px solid ${C.g4}`,borderRadius:8,
                            padding:"8px 12px",fontSize:13,color:C.lbl,fontFamily:SF,outline:"none",
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <select value={newSubjTeacher} onChange={e=>setNewSubjTeacher(e.target.value)}
                            style={{flex:1,background:"#fff",border:`1px solid ${C.g4}`,borderRadius:8,
                              padding:"8px 10px",fontSize:13,color:C.lbl,fontFamily:SF,outline:"none"}}>
                            <option value="">Sin docente</option>
                            {state.teachers.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
                          </select>
                          <Btn onPress={()=>addSubjectToGroup(g.id)} color={devColor} size="sm"
        ...
        {tab==="assign"&&(()=>{
          const unassigned=state.students.filter(s=>!s.group||!state.groups.find(g=>g.students?.includes(s.id)));
          return(
            <div>
              <Card style={{padding:14,marginBottom:16,background:`${C.orange}10`,border:`1px solid ${C.orange}20`}}>
                <div style={{fontSize:13,fontWeight:700,color:C.orange,fontFamily:SF,marginBottom:4}}>📋 Asignación de Grupos</div>
                <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,lineHeight:1.5}}>
                  Los alumnos recién registrados deben ser asignados a un grupo antes de poder acceder a materias y actividades. Aquí puedes ver los alumnos pendientes y asignarlos.
                </div>
              </Card>
              {unassigned.length===0?(
                <div style={{textAlign:"center",padding:"32px 16px"}}>
                  <div style={{fontSize:40,marginBottom:10}}>✅</div>
                  <div style={{fontSize:16,fontWeight:600,color:C.lbl,fontFamily:SF,marginBottom:4}}>¡Todos asignados!</div>
                  <div style={{fontSize:13,color:C.lbl2,fontFamily:SF}}>Todos los alumnos tienen grupo asignado.</div>
                </div>
              ):(
                <>
                  <div style={{...fmt.footnote,color:C.red,textTransform:"uppercase",letterSpacing:"0.05em",fontFamily:SF,marginBottom:8,paddingLeft:4}}>
                  ...
                  {unassigned.map(s=>{
                    ...
                    const assign=async()=>{
                      if(!selGroup)return;
                      const grp=state.groups.find(g=>g.id===selGroup||g.name===selGroup);
                      if(!grp)return;
                      const newStudents=[...(grp.students||[]),s.id];
                      if(typeof grp.id==="string"){
                        try{ const {doc:fiDoc,updateDoc:fiUpd}=await import("firebase/firestore").catch(()=>({}));
                          if(fiDoc&&fiUpd) await fiUpd(fiDoc(window.__db||{},"groups",grp.id),{students:newStudents}); }catch{}
                      }
                      // Auto-assign subjects from group definition
                      const autoSubjs=(grp.subjects||[]).reduce((acc,{subject})=>({
                        ...acc,[subject]:{grade:8,tasks:[]}
                      }),{});
                      setState(prev=>({...prev,
                        students:prev.students.map(st=>st.id===s.id?{...st,group:grp.name,subjects:{...autoSubjs,...st.subjects}}:st),
                        groups:prev.groups.map(g=>g.id===grp.id?{...g,students:newStudents}:g)
                      }));
                      SFX.play("success");
                      pushNotification({title:"✅ Alumno asignado",text:`${s.name} fue asignado al grupo ${grp.name}.`});
                    };
                    return(
                      <Card key={s.id} style={{marginBottom:10,padding:14}}>
                        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                          ...
                          <div style={{flex:1}}>
                            <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>{s.name}</div>
                            <div style={{fontSize:11,fontWeight:600,color:C.red,background:`${C.red}12`,borderRadius:5,padding:"2px 8px",display:"inline-block",fontFamily:SF,marginTop:3}}>⚠️ Sin grupo</div>
                          </div>
                        </div>
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <select value={selGroup} onChange={e=>setSelGroup(e.target.value)}
                            style={{flex:1,background:C.fill4,border:`1px solid ${C.g4}`,borderRadius:10,padding:"9px 12px",fontSize:14,color:C.lbl,fontFamily:SF,outline:"none"}}>
                            <option value="">Seleccionar grupo…</option>
                            {state.groups.map(g=><option key={g.id} value={g.id}>{g.name} — {g.subject||"Sin materia"}</option>)}
                          </select>
                          <Btn onPress={assign} color={C.green} disabled={!selGroup}>Asignar</Btn>
                        </div>
                      </Card>
                    );
                  })}
              ...
              <div style={{...fmt.footnote,color:C.lbl2,textTransform:"uppercase",letterSpacing:"0.05em",fontFamily:SF,margin:"20px 0 8px",paddingLeft:4}}>
                Todos los alumnos
              </div>
              <Sec>
                {state.students.map((s,i)=>{
                  const grp=state.groups.find(g=>g.students?.includes(s.id));
                  const isAssigned=!!grp;
                  return(
                    <div key={s.id}>
                      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px"}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>{s.name}</div>
                          <div style={{fontSize:11,color:C.lbl2,fontFamily:SF,marginTop:2}}>{grp?grp.name:s.group||"Sin grupo"}</div>
                        </div>
                        <span style={{fontSize:11,fontWeight:600,borderRadius:6,padding:"3px 9px",fontFamily:SF,
                          color:isAssigned?C.green:C.red,background:isAssigned?`${C.green}12`:`${C.red}12`}}>
...
const DirectorApp=({state,setState,onLogout})=>{
  ...
  // ── Vista principal ───────────────────────────────────────────────────────
  return(
    <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF,display:"flex",flexDirection:"row"}}>
      ...
      <AppSidebar
        ...
        onLogout={onLogout}
        navItems={[
          {id:"feed",icon:"📋",label:"Tablón",active:tab==="feed",onClick:()=>setTab("feed")},
          {id:"management",icon:"📊",label:"Gestión",active:tab==="management",onClick:()=>setTab("management"),
            subItems:[
              {id:"resumen",icon:"📈",label:"Resumen",active:mgmtTab==="resumen",onClick:()=>setMgmtTab("resumen")},
              {id:"asistencia",icon:"📅",label:"Asistencia",active:mgmtTab==="asistencia",onClick:()=>setMgmtTab("asistencia")},
              {id:"personal",icon:"👩‍🏫",label:"Personal",active:mgmtTab==="personal",onClick:()=>setMgmtTab("personal")},
              {id:"alumnos",icon:"🎒",label:"Alumnos",active:mgmtTab==="alumnos",onClick:()=>setMgmtTab("alumnos")},
              {id:"grupos",icon:"👥",label:"Grupos",active:mgmtTab==="grupos",onClick:()=>setMgmtTab("grupos")},
              {id:"avisos",icon:"📢",label:"Avisos",active:mgmtTab==="avisos",onClick:()=>setMgmtTab("avisos"),badge:(state.avisos||[]).filter(a=>!a.read).length},
              {id:"actividades",icon:"🗓️",label:"Actividades",active:mgmtTab==="actividades",onClick:()=>setMgmtTab("actividades")},
              {id:"aprobaciones",icon:"✅",label:"Aprobaciones",active:mgmtTab==="aprobaciones",onClick:()=>setMgmtTab("aprobaciones"),badge:state.pendingContent.length},
            ]},
          {id:"settings",icon:"⚙️",label:"Ajustes",active:tab==="settings",onClick:()=>setTab("settings")},
        ]}
      />
  ...
  return(
    <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF,display:"flex",flexDirection:"row"}}>
      <AppSidebar
        ...
        navItems={[
          {id:"feed",icon:"📋",label:"Tablón",active:tab==="feed",onClick:()=>setTab("feed")},
          {id:"classes",icon:"🏫",label:"Mis Clases",active:tab==="classes",onClick:()=>setTab("classes"),badge:urgentCount||0},
          {id:"ai",icon:"🤖",label:"Asistente IA",active:tab==="ai",onClick:()=>setTab("ai")},
          {id:"settings",icon:"⚙️",label:"Ajustes",active:tab==="settings",onClick:()=>setTab("settings")},
        ]}
      />
      <div style={{flex:1,minWidth:0,minHeight:"100vh",overflowX:"hidden"}}>
...
const StudentApp=({state,setState,studentId,onLogout})=>{
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:SF,display:"flex",flexDirection:"row"}}>
      <AppSidebar
        open={sidebarOpen} onToggle={()=>setSidebarOpen(o=>!o)}
        ...
        onLogout={onLogout}
        navItems={[
          {id:"feed",icon:"📋",label:"Tablón",active:tab==="feed",onClick:()=>setTab("feed"),badge:urgentCount||0},
          {id:"subjects",icon:"📚",label:"Materias",active:tab==="subjects",onClick:()=>{setSelectedSubject(null);setTab("subjects");}},
          {id:"medals",icon:"🏆",label:"Logros",active:tab==="medals",onClick:()=>setTab("medals")},
          {id:"settings",icon:"⚙️",label:"Ajustes",active:tab==="settings",onClick:()=>setTab("settings")},
        ]}
      />
...
export default function App(){
  ...
  const login=(r,id)=>{SFX.play("login");setRole(r);setUserId(id);setScreen("app");};
  const logout=()=>{
    ...
    <BpProvider>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-font-smoothing:antialiased;}
        ...
        /* ── PAGE PADDING ────────────────────────────────────── */
        .page-pad{
          padding:var(--sp-sm) var(--sp) var(--page-pb);
        }
        ...
        /* ── SIDEBAR LAYOUT ──────────────────────────────────── */
        /* On mobile, the sidebar collapses to icon-only (60px) by default */
        @media(max-width:639px){
          /* Sidebar is overlay on mobile */
          .sidebar-layout{ display:flex; flex-direction:row; }
        }
