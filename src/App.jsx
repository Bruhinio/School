                ...
                <>
                  ...
                  {unassigned.map(s=>{
                    ...
                    const assign=async()=>{
                      ...
                      setState(prev=>({...prev,
                        students:prev.students.map(st=>st.id===s.id?{...st,group:grp.name}:st),
  ...
  return(
    <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF,display:"flex",flexDirection:"row"}}>
      ...
      <div style={{
        ...
        background:"linear-gradient(180deg,#3730a3 0%,#4f46e5 60%,#6366f1 100%)",
        display:"flex",flexDirection:"column",
        position:"sticky",top:0,height:"100vh",overflowY:"auto",
        boxShadow:"2px 0 16px rgba(79,70,229,0.18)"
      }}>
        {/* Logo */}
        <div style={{padding:"24px 18px 16px",borderBottom:"1px solid rgba(255,255,255,0.12)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:40,height:40,borderRadius:12,background:"rgba(255,255,255,0.18)",
              display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>🏫</div>
            <div>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SFD,lineHeight:1.2}}>Instituto</div>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SFD,lineHeight:1.2}}>Educativo</div>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"16px 18px",borderBottom:"1px solid rgba(255,255,255,0.1)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:600,color:"#fff",fontFamily:SF,
                overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>Directora Gómez</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.65)",fontFamily:SF}}>{activeCycle?.name||"Ciclo activo"}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"12px 10px",flex:1}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",
            letterSpacing:"0.1em",marginBottom:6,paddingLeft:8,fontFamily:SF}}>Navegación</div>
          {[
            {id:"feed",    icon:"📋", label:"Tablón"},
            {id:"management",icon:"📊", label:"Gestión"},
            ...
            return(
              <button key={t.id} onClick={()=>setTab(t.id)}
                ...
                <span style={{fontSize:14,fontWeight:active?700:500,color:"#fff",fontFamily:SF}}>{t.label}</span>
                {active&&<div style={{marginLeft:"auto",width:6,height:6,borderRadius:"50%",background:"#fff",flexShrink:0}}/>}
              </button>
            );
          })}

          {/* Gestión sub-tabs shown inline when management is active */}
          {tab==="management"&&(
            <div style={{marginTop:6,paddingLeft:8,borderLeft:"2px solid rgba(255,255,255,0.15)",marginLeft:12}}>
              {[
                {id:"resumen",    icon:"📈", label:"Resumen"},
                {id:"asistencia", icon:"📅", label:"Asistencia",  badge:(Object.values(state.teacherAttendance?.[todayStr]||{}).length||0)>0?null:null},
                {id:"personal",   icon:"👩‍🏫", label:"Personal"},
                {id:"alumnos",    icon:"🎒", label:"Alumnos"},
                {id:"grupos",     icon:"👥", label:"Grupos"},
                {id:"avisos",     icon:"📢", label:"Avisos",      badge:(state.avisos||[]).filter(a=>!a.read).length||null},
                {id:"actividades",icon:"🗓️", label:"Actividades"},
                {id:"aprobaciones",icon:"✅", label:"Aprobaciones", badge:state.pendingContent.length||null},
              ].map(t=>{
                const active = mgmtTab===t.id;
                return(
                  <button key={t.id} onClick={()=>setMgmtTab(t.id)}
                    style={{width:"100%",display:"flex",alignItems:"center",gap:8,
                      padding:"7px 10px 7px 12px",borderRadius:8,border:"none",cursor:"pointer",
                    ...
                    <span style={{fontSize:12,fontWeight:active?700:400,color:active?"#fff":"rgba(255,255,255,0.75)",fontFamily:SF,flex:1}}>{t.label}</span>
                    {t.badge>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:9,fontWeight:700,
                      borderRadius:8,padding:"1px 5px",fontFamily:SF,flexShrink:0}}>{t.badge}</span>}
                  </button>
                );
              })}
            </div>
        ...
        {/* Logout */}
        <div style={{padding:"12px 10px 24px",borderTop:"1px solid rgba(255,255,255,0.1)"}}>
          <button onClick={()=>{SFX.play("click");onLogout();}}
            style={{width:"100%",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.18)",
              borderRadius:12,padding:"10px 14px",color:"rgba(255,255,255,0.85)",fontSize:13,
    ...
    <BpProvider>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-font-smoothing:antialiased;}
        ...
        :root{
          --sp:16px;          /* page horizontal padding */
          --sp-sm:10px;       /* small gap */
          --sp-xs:6px;        /* tiny gap */
          --radius:14px;      /* card radius */
          --radius-sm:10px;
          --fs-title:22px;    /* page title */
          --fs-head:17px;     /* section heading */
          --fs-body:15px;     /* body text */
          --fs-sub:13px;      /* secondary label */
          --fs-cap:12px;      /* caption */
          --fs-xs:11px;       /* tiny labels */
          --tab-h:58px;       /* bottom tab bar height */
          --nav-h:54px;       /* top nav height */
          --sidebar-w:0px;    /* sidebar width (desktop only) */
          --card-cols:1;      /* card grid columns */
          --avatar-lg:64px;
          --avatar-md:42px;
          --avatar-sm:34px;
        }
        ...
        @media(min-width:1024px){
          .main-area{ padding-bottom:0; max-width:900px; margin:0 auto; }
        }
        ...
        @media(min-width:1024px){
          .tab-bar-bottom{ display:none !important; }
        }
        ...
        @media(min-width:1024px){
          .stats-row{ grid-template-columns:repeat(6,1fr); gap:14px; }
        }
        ...
        /* ── PAGE PADDING ────────────────────────────────────── */
        .page-pad{
          padding:var(--sp-sm) var(--sp) calc(var(--tab-h) + 16px);
        }
        @media(min-width:1024px){
        ...
        @media(min-width:1024px){
          .mgmt-tabs{ flex-wrap:wrap; overflow:visible; }
        }
        ...
        @media(min-width:640px){
          .school-map-svg{ min-height:300px; }
        }
        @media(min-width:1024px){
          .school-map-svg{ min-height:380px; }
        }

        /* ── PROFILE CARD (centering on wide screens) ──────── */
        @media(min-width:1024px){
          .profile-header{
            display:flex;
