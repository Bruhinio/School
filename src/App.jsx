    ...
    <>
      ...
      <div style={{position:"fixed",top:58,right:10,width:300,maxHeight:"70vh",
        ...
        <div style={{overflowY:"auto",maxHeight:"calc(70vh - 58px)"}}>
          ...
          {newsItems.map((n,i)=>{
            const isUnread=!bellReadIds.includes(n.id);
            return(
            <div key={n.id}>
              <div onClick={()=>{markOneFeedBellRead(n.id);setSelNews(selNews===n.id?null:n.id);}}
                style={{padding:"9px 12px",cursor:"pointer",borderLeft:`4px solid ${n.color}`,
                  background:isUnread?`${C.blue}10`:selNews===n.id?C.fill4:"transparent",transition:"background 0.1s"}}>
                <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                  <span style={{fontSize:18,flexShrink:0,marginTop:1}}>{n.icon}</span>
                  <div style={{flex:1,minWidth:0}}>
                    {isUnread&&<span style={{fontSize:9,fontWeight:800,color:C.blue,fontFamily:SF}}>● NUEVO  </span>}
                    ...
                    <div style={{fontSize:12,fontWeight:isUnread?700:600,color:C.lbl,fontFamily:SF,lineHeight:1.35,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:selNews===n.id?"normal":"nowrap",marginTop:n.urgent?2:0}}>{n.title}</div>
...
const ChatPanel=({state,setState,myUserId,myName,myAvatar,myColor,role,accent=C.blue})=>{
  // Fix for iOS: set --vh to real viewport height
  useEffect(()=>{
  ...
  if(view==="chat"&&selectedChat){
    ...
    return(
      <div style={{background:"#f0f2f5",height:"calc(var(--vh,1vh)*100 - var(--tab-h,56px))",maxHeight:"-webkit-fill-available",fontFamily:SF_local,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* Header */}
        ...
        <div style={{flexShrink:0,background:"rgba(255,255,255,0.98)",borderTop:`0.5px solid ${C.sep}`,
          padding:"10px 12px",paddingBottom:"max(12px,env(safe-area-inset-bottom,12px))",
        ...
        <>
          ...
          <div style={{position:"absolute",top:42,right:0,width:300,maxHeight:"70vh",
            ...
            <div style={{background:`linear-gradient(135deg,${C.blue},${C.blue}bb)`,padding:"11px 13px",
              display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SF}}>📣 Avisos {unreadBellCount>0&&`(${unreadBellCount} nuevos)`}</div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              {newsItems.map((n,i)=>{
                ...
                <div key={n.id}>
                  <div onClick={()=>{markOneBellRead(n.id);setSelBell(selBell===n.id?null:n.id);}}
                    style={{padding:"9px 12px",cursor:"pointer",borderLeft:`4px solid ${n.color}`,
                      background:isUnread?`${C.blue}10`:selBell===n.id?C.fill4:"transparent",
                      transition:"background 0.15s"}}>
                    <div style={{display:"flex",gap:7,alignItems:"flex-start"}}>
                      <span style={{fontSize:17,flexShrink:0}}>{n.icon}</span>
                      <div style={{flex:1,minWidth:0}}>
                        {isUnread&&<div style={{fontSize:9,fontWeight:800,color:C.blue,fontFamily:SF,marginBottom:2}}>● NUEVO</div>}
                        {n.urgent&&<div style={{fontSize:9,fontWeight:800,color:C.red,fontFamily:SF}}>🚨 URGENTE</div>}
                        <div style={{fontSize:12,fontWeight:isUnread?700:600,color:C.lbl,fontFamily:SF,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:selBell===n.id?"normal":"nowrap"}}>{n.title}</div>
