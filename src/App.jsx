...
// Per-animal head anchors & adjustments
const ANIMAL_HEADS={
  owl:      {x:50,y:26,scale:1,hatY:-34,glassesY:4,bodyY:22,sideX:30}, 
  bear:     {x:50,y:26,scale:1.05,hatY:-36,glassesY:5,bodyY:24,sideX:32},
  whale:    {x:36,y:36,scale:0.9,hatY:-28,glassesY:8,bodyY:28,sideX:50},
  elephant: {x:50,y:28,scale:1.1,hatY:-38,glassesY:6,bodyY:26,sideX:34},
  giraffe:  {x:50,y:18,scale:0.85,hatY:-30,glassesY:2,bodyY:14,sideX:28},
  lion:     {x:50,y:28,scale:1.08,hatY:-36,glassesY:6,bodyY:26,sideX:32},
  fox:      {x:50,y:26,scale:0.95,hatY:-32,glassesY:4,bodyY:22,sideX:30},
  rabbit:   {x:50,y:30,scale:0.92,hatY:-40,glassesY:8,bodyY:26,sideX:30},
  dragon:   {x:50,y:26,scale:1.02,hatY:-34,glassesY:4,bodyY:22,sideX:31},
  cat:      {x:50,y:26,scale:0.96,hatY:-33,glassesY:4,bodyY:22,sideX:30},
  default:  {x:50,y:26,scale:1,hatY:-34,glassesY:4,bodyY:22,sideX:30},
};
...
const MascotSVG=({subject,outfit,size=80,animal=null})=>{
  ...
  const {x:hx,y:hy,scale:sc,hatY,glassesY,bodyY,sideX}=head;
  const T=(children)=><g transform={`translate(${hx},${hy}) scale(${sc})`}>{children}</g>;

  // ── ACCESORIOS PERSONALIZADOS POR ANIMAL ────────────────────────────────
  
  // BÚHO - académico, sabio
  const OwlBirrete=()=>T(<><rect x="-20" y={hatY} width="40" height="7" rx="2" fill="#1a1a2e"/><rect x="-14" y={hatY-8} width="28" height="9" rx="2" fill="#1a1a2e"/><line x1="12" y1={hatY-6} x2="23" y2={hatY+3} stroke="#FFD700" strokeWidth="2.5" strokeLinecap="round"/><circle cx="24" cy={hatY+4} r="3.5" fill="#FFD700"/></>);
  const OwlGafas=()=>T(<><circle cx="-14" cy={glassesY} r="9" fill="none" stroke="#1a1a2e" strokeWidth="2.5"/><circle cx="14" cy={glassesY} r="9" fill="none" stroke="#1a1a2e" strokeWidth="2.5"/><line x1="-5" y1={glassesY} x2="5" y2={glassesY} stroke="#1a1a2e" strokeWidth="2.5"/><circle cx="-14" cy={glassesY} r="6" fill="#E3F2FD" opacity="0.3"/><circle cx="14" cy={glassesY} r="6" fill="#E3F2FD" opacity="0.3"/></>);
  const OwlCapa=()=>T(<><rect x="-15" y={bodyY} width="30" height="5" rx="2" fill="#FFD700"/><path d={`M-15 ${bodyY+5} Q-19 ${bodyY+16} -17 ${bodyY+32} L0 ${bodyY+25} L17 ${bodyY+32} Q19 ${bodyY+16} 15 ${bodyY+5} Z`} fill="#2c3e50"/></>);
  const OwlLibro=()=>T(<><rect x={sideX-10} y="-10" width="28" height="36" rx="3" fill="#1a1a2e"/><rect x={sideX-8} y="-8" width="24" height="32" rx="2" fill="#ecf0f1"/><line x1={sideX+4} y1="-8" x2={sideX+4} y2="24" stroke="#34495e" strokeWidth="2"/></>);

  // OSO - fuerte, aventurero
  const BearCasco=()=>T(<><ellipse cx="0" cy={hatY+6} rx="30" ry="8" fill="#795548"/><path d={`M-24 ${hatY+6} Q-22 ${hatY-14} 0 ${hatY-16} Q22 ${hatY-14} 24 ${hatY+6}`} fill="#8d6e63"/><circle cx="-8" cy={hatY-6} r="3" fill="#FFD700"/><circle cx="8" cy={hatY-6} r="3" fill="#FFD700"/></>);
  const BearGafas=()=>T(<><rect x="-28" y={glassesY-3} width="56" height="16" rx="8" fill="#ff6b35" opacity="0.4" stroke="#c0392b" strokeWidth="3"/><line x1="0" y1={glassesY-3} x2="0" y2={glassesY+13} stroke="#c0392b" strokeWidth="2"/></>);
  const BearCapa=()=>T(<><rect x="-16" y={bodyY} width="32" height="6" rx="3" fill="#8d6e63"/><path d={`M-16 ${bodyY+6} Q-21 ${bodyY+18} -19 ${bodyY+36} L0 ${bodyY+28} L19 ${bodyY+36} Q21 ${bodyY+18} 16 ${bodyY+6} Z`} fill="#c0392b"/><path d={`M-16 ${bodyY+6} Q-8 ${bodyY+12} 0 ${bodyY+10} Q8 ${bodyY+12} 16 ${bodyY+6}`} fill="#e74c3c" opacity="0.5"/></>);
  const BearHacha=()=>T(<><line x1={sideX+5} y1="-15" x2={sideX+5} y2="25" stroke="#5d4037" strokeWidth="4" strokeLinecap="round"/><path d={`M${sideX-2} -18 L${sideX-8} -12 L${sideX+2} -6 L${sideX+12} -12 Z`} fill="#95a5a6"/><path d={`M${sideX-2} -18 L${sideX+2} -6 L${sideX+12} -12 Z`} fill="#ecf0f1"/></>);

  // BALLENA - marino, explorador
  const WhaleSombrero=()=>T(<><ellipse cx="0" cy={hatY+8} rx="32" ry="7" fill="#3498db"/><path d={`M-26 ${hatY+8} Q-24 ${hatY-12} 0 ${hatY-14} Q24 ${hatY-12} 26 ${hatY+8}`} fill="#2980b9"/><circle cx="0" cy={hatY-8} r="4" fill="#e74c3c"/><path d={`M-12 ${hatY+5} L-8 ${hatY-3} L8 ${hatY-3} L12 ${hatY+5}`} fill="#ecf0f1" opacity="0.5"/></>);
  const WhaleGafas=()=>T(<><circle cx="-16" cy={glassesY+2} r="11" fill="none" stroke="#2c3e50" strokeWidth="3"/><circle cx="16" cy={glassesY+2} r="11" fill="none" stroke="#2c3e50" strokeWidth="3"/><line x1="-5" y1={glassesY+2} x2="5" y2={glassesY+2} stroke="#2c3e50" strokeWidth="3"/><circle cx="-16" cy={glassesY+2} r="8" fill="#4FC3F7" opacity="0.4"/><circle cx="16" cy={glassesY+2} r="8" fill="#4FC3F7" opacity="0.4"/></>);
  const WhaleChaleco=()=>T(<><path d={`M-18 ${bodyY} L-24 ${bodyY+38} L24 ${bodyY+38} L18 ${bodyY} Q8 ${bodyY+6} 0 ${bodyY+4} Q-8 ${bodyY+6} -18 ${bodyY}Z`} fill="#e67e22" stroke="#d35400" strokeWidth="1.5"/><circle cx="0" cy={bodyY+12} r="2.5" fill="#ecf0f1"/><circle cx="0" cy={bodyY+22} r="2.5" fill="#ecf0f1"/></>);
  const WhaleAncla=()=>T(<><line x1={sideX+6} y1="-12" x2={sideX+6} y2="18" stroke="#34495e" strokeWidth="4"/><path d={`M${sideX-4} 18 L${sideX+6} 28 L${sideX+16} 18`} fill="none" stroke="#34495e" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/><circle cx={sideX+6} cy="18" r="5" fill="none" stroke="#34495e" strokeWidth="3"/><line x1={sideX-2} y1="-12" x2={sideX+14} y2="-12" stroke="#34495e" strokeWidth="3" strokeLinecap="round"/></>);

  // ELEFANTE - sabio, poderoso
  const ElephantCorona=()=>T(<><path d={`M-22 ${hatY+8} L-22 ${hatY-6} L-10 ${hatY+2} L0 ${hatY-12} L10 ${hatY+2} L22 ${hatY-6} L22 ${hatY+8} Z`} fill="#FFD700"/><circle cx="0" cy={hatY-11} r="4" fill="#e74c3c"/><circle cx="-21" cy={hatY-5} r="3" fill="#27ae60"/><circle cx="21" cy={hatY-5} r="3" fill="#27ae60"/><circle cx="-10" cy={hatY+3} r="2.5" fill="#3498db"/><circle cx="10" cy={hatY+3} r="2.5" fill="#3498db"/></>);
  const ElephantGafas=()=>T(<><rect x="-30" y={glassesY-2} width="60" height="16" rx="7" fill="#9b59b6" opacity="0.4" stroke="#8e44ad" strokeWidth="2.5"/><line x1="0" y1={glassesY-2} x2="0" y2={glassesY+14} stroke="#8e44ad" strokeWidth="2"/></>);
  const ElephantManto=()=>T(<><rect x="-17" y={bodyY} width="34" height="6" rx="3" fill="#c0392b"/><path d={`M-17 ${bodyY+6} Q-23 ${bodyY+20} -21 ${bodyY+42} L0 ${bodyY+33} L21 ${bodyY+42} Q23 ${bodyY+20} 17 ${bodyY+6} Z`} fill="#9b59b6"/><circle cx="0" cy={bodyY+18} r="3" fill="#FFD700"/><circle cx="-8" cy={bodyY+26} r="2.5" fill="#FFD700"/><circle cx="8" cy={bodyY+26} r="2.5" fill="#FFD700"/></>);
  const ElephantCetro=()=>T(<><line x1={sideX+4} y1="-18" x2={sideX+4} y2="28" stroke="#8e44ad" strokeWidth="4"/><circle cx={sideX+4} cy="-22" r="7" fill="#FFD700"/><path d={`M${sideX-2} -22 L${sideX+4} -28 L${sideX+10} -22 L${sideX+8} -15 L${sideX} -15 Z`} fill="#e74c3c"/></>);

  // JIRAFA - elegante, artista
  const GiraffeSombrero=()=>T(<><ellipse cx="0" cy={hatY+10} rx="26" ry="6" fill="#e67e22"/><path d={`M-20 ${hatY+10} Q-18 ${hatY-8} 0 ${hatY-10} Q18 ${hatY-8} 20 ${hatY+10}`} fill="#d35400"/><circle cx="-8" cy={hatY} r="2.5" fill="#ecf0f1"/><circle cx="8" cy={hatY} r="2.5" fill="#ecf0f1"/></>);
  const GiraffeGafas=()=>T(<><circle cx="-12" cy={glassesY} r="8" fill="none" stroke="#e67e22" strokeWidth="2.5"/><circle cx="12" cy={glassesY} r="8" fill="none" stroke="#e67e22" strokeWidth="2.5"/><line x1="-4" y1={glassesY} x2="4" y2={glassesY} stroke="#e67e22" strokeWidth="2.5"/><circle cx="-12" cy={glassesY} r="5" fill="#f39c12" opacity="0.3"/><circle cx="12" cy={glassesY} r="5" fill="#f39c12" opacity="0.3"/></>);
  const GiraffePañuelo=()=>T(<><path d={`M-14 ${bodyY} L-10 ${bodyY+16} L0 ${bodyY+14} L10 ${bodyY+16} L14 ${bodyY} Q8 ${bodyY+4} 0 ${bodyY+3} Q-8 ${bodyY+4} -14 ${bodyY} Z`} fill="#e74c3c"/><path d={`M-10 ${bodyY+6} L-5 ${bodyY+12} L0 ${bodyY+10} L5 ${bodyY+12} L10 ${bodyY+6}`} fill="#c0392b" opacity="0.6"/></>);
  const GiraffePaleta=()=>T(<><circle cx={sideX} cy="-8" r="12" fill="#e67e22"/><circle cx={sideX-4} cy="-12" r="5" fill="#f39c12"/><circle cx={sideX+4} cy="-4" r="5" fill="#d35400"/><line x1={sideX} y1="4" x2={sideX} y2="24" stroke="#8d6e63" strokeWidth="3" strokeLinecap="round"/></>);

  // LEÓN - majestuoso, líder
  const LionCorona=()=>T(<><path d={`M-24 ${hatY+10} L-24 ${hatY-4} L-12 ${hatY+4} L0 ${hatY-10} L12 ${hatY+4} L24 ${hatY-4} L24 ${hatY+10} Z`} fill="#FFD700"/><line x1="-24" y1={hatY+10} x2="24" y2={hatY+10} stroke="#f39c12" strokeWidth="2.5"/><circle cx="0" cy={hatY-9} r="4" fill="#e74c3c"/><circle cx="-23" cy={hatY-3} r="3" fill="#27ae60"/><circle cx="23" cy={hatY-3} r="3" fill="#27ae60"/></>);
  const LionGafas=()=>T(<><circle cx="-15" cy={glassesY+1} r="10" fill="none" stroke="#d35400" strokeWidth="2.8"/><circle cx="15" cy={glassesY+1} r="10" fill="none" stroke="#d35400" strokeWidth="2.8"/><line x1="-5" y1={glassesY+1} x2="5" y2={glassesY+1} stroke="#d35400" strokeWidth="2.8"/></>);
  const LionCapa=()=>T(<><rect x="-16" y={bodyY} width="32" height="6" rx="3" fill="#FFD700"/><path d={`M-16 ${bodyY+6} Q-20 ${bodyY+18} -18 ${bodyY+36} L0 ${bodyY+29} L18 ${bodyY+36} Q20 ${bodyY+18} 16 ${bodyY+6} Z`} fill="#c0392b"/><path d={`M-16 ${bodyY+6} Q-8 ${bodyY+12} 0 ${bodyY+10} Q8 ${bodyY+12} 16 ${bodyY+6}`} fill="#e74c3c" opacity="0.6"/></>);
  const LionEspada=()=>T(<><line x1={sideX+3} y1="-20" x2={sideX+3} y2="22" stroke="#95a5a6" strokeWidth="4"/><rect x={sideX-2} y="-24" width="10" height="8" rx="2" fill="#FFD700"/><path d={`M${sideX-4} 22 L${sideX+3} 30 L${sideX+10} 22 Z`} fill="#ecf0f1"/></>);

  // ZORRO - astuto, inteligente  
  const FoxSombrero=()=>T(<><ellipse cx="0" cy={hatY+8} rx="27" ry="6" fill="#e67e22"/><path d={`M-21 ${hatY+8} Q-19 ${hatY-10} 0 ${hatY-12} Q19 ${hatY-10} 21 ${hatY+8}`} fill="#d35400"/><ellipse cx="0" cy={hatY+7} rx="15" ry="3" fill="#f39c12" opacity="0.4"/></>);
  const FoxGafas=()=>T(<><circle cx="-13" cy={glassesY} r="9" fill="none" stroke="#c0392b" strokeWidth="2.3"/><circle cx="13" cy={glassesY} r="9" fill="none" stroke="#c0392b" strokeWidth="2.3"/><line x1="-4" y1={glassesY} x2="4" y2={glassesY} stroke="#c0392b" strokeWidth="2.3"/><circle cx="-13" cy={glassesY} r="6" fill="#e67e22" opacity="0.3"/><circle cx="13" cy={glassesY} r="6" fill="#e67e22" opacity="0.3"/></>);
  const FoxBufanda=()=>T(<><rect x="-16" y={bodyY} width="32" height="8" rx="2" fill="#e74c3c"/><path d={`M16 ${bodyY} L22 ${bodyY+16} L18 ${bodyY+32}`} stroke="#c0392b" strokeWidth="4" fill="none" strokeLinecap="round"/><path d={`M20 ${bodyY+8} L24 ${bodyY+8} M20 ${bodyY+16} L24 ${bodyY+16}`} stroke="#ecf0f1" strokeWidth="2"/></>);
  const FoxLupa=()=>T(<><circle cx={sideX+2} cy="0" r="13" fill="none" stroke="#34495e" strokeWidth="2.8"/><circle cx={sideX+2} cy="0" r="10" fill="#e3f2fd" opacity="0.5"/><line x1={sideX-8} y1="11" x2={sideX-18} y2="22" stroke="#34495e" strokeWidth="3.5" strokeLinecap="round"/></>);

  // CONEJO - tierno, rápido
  const RabbitMonito=()=>T(<><ellipse cx="0" cy={hatY+12} rx="18" ry="4" fill="#e91e63"/><path d="M-8 ${hatY+12} L-10 ${hatY+20} L-6 ${hatY+28}" stroke="#c2185b" strokeWidth="3" fill="none" strokeLinecap="round"/><path d="M8 ${hatY+12} L10 ${hatY+20} L6 ${hatY+28}" stroke="#c2185b" strokeWidth="3" fill="none" strokeLinecap="round"/></>);
  const RabbitGafas=()=>T(<><circle cx="-12" cy={glassesY+2} r="8" fill="none" stroke="#e91e63" strokeWidth="2.2"/><circle cx="12" cy={glassesY+2} r="8" fill="none" stroke="#e91e63" strokeWidth="2.2"/><line x1="-4" y1={glassesY+2} x2="4" y2={glassesY+2} stroke="#e91e63" strokeWidth="2.2"/><circle cx="-12" cy={glassesY+2} r="5" fill="#f8bbd0" opacity="0.4"/><circle cx="12" cy={glassesY+2} r="5" fill="#f8bbd0" opacity="0.4"/></>);
  const RabbitChaleco=()=>T(<><path d="M-14 ${bodyY} L-18 ${bodyY+28} L18 ${bodyY+28} L14 ${bodyY} Q6 ${bodyY+4} 0 ${bodyY+3} Q-6 ${bodyY+4} -14 ${bodyY}Z" fill="#f06292" stroke="#e91e63" strokeWidth="1.5"/><circle cx="-6" cy={bodyY+10} r="2" fill="#fff"/><circle cx="6" cy={bodyY+10} r="2" fill="#fff"/><circle cx="-6" cy={bodyY+18} r="2" fill="#fff"/><circle cx="6" cy={bodyY+18} r="2" fill="#fff"/></>);
  const RabbitZanahoria=()=>T(<><path d="M${sideX-2} -8 L${sideX+2} 18" fill="#ff6f00" stroke="#e65100" strokeWidth="3.5" strokeLinecap="round"/><path d="M${sideX-4} -12 L${sideX-2} -8 M${sideX} -14 L${sideX} -8 M${sideX+4} -12 L${sideX+2} -8" stroke="#4caf50" strokeWidth="2.5" strokeLinecap="round"/></>);

  // DRAGÓN - místico, poderoso
  const DragonCasco=()=>T(<><path d="M-28 ${hatY+2} Q-30 ${hatY-22} 0 ${hatY-24} Q30 ${hatY-22} 28 ${hatY+2}" fill="#5e35b1"/><path d="M-12 ${hatY-24} L-8 ${hatY-32} L-4 ${hatY-24}" fill="#7e57c2"/><path d="M-4 ${hatY-24} L0 ${hatY-34} L4 ${hatY-24}" fill="#9575cd"/><path d="M4 ${hatY-24} L8 ${hatY-32} L12 ${hatY-24}" fill="#7e57c2"/></>);
  const DragonGafas=()=>T(<><rect x="-28" y={glassesY-2} width="56" height="15" rx="7" fill="#7e57c2" opacity="0.4" stroke="#5e35b1" strokeWidth="2.5"/><line x1="0" y1={glassesY-2} x2="0" y2={glassesY+13} stroke="#5e35b1" strokeWidth="2"/><path d="M-14 ${glassesY+5} L-10 ${glassesY+9} M14 ${glassesY+5} L10 ${glassesY+9}" stroke="#9575cd" strokeWidth="2"/></>);
  const DragonCapa=()=>T(<><rect x="-16" y={bodyY} width="32" height="6" rx="3" fill="#5e35b1"/><path d="M-16 ${bodyY+6} Q-20 ${bodyY+18} -18 ${bodyY+36} L0 ${bodyY+29} L18 ${bodyY+36} Q20 ${bodyY+18} 16 ${bodyY+6} Z" fill="#7e57c2"/><path d="M-14 ${bodyY+12} L-8 ${bodyY+18} L-2 ${bodyY+12} M2 ${bodyY+12} L8 ${bodyY+18} L14 ${bodyY+12}" fill="#9575cd" opacity="0.6"/></>);
  const DragonOrbe=()=>T(<><circle cx={sideX+2} cy="-4" r="11" fill="#9575cd" opacity="0.6"/><circle cx={sideX+2} cy="-4" r="8" fill="#ce93d8"/><circle cx={sideX-1} cy="-7" r="3" fill="#fff" opacity="0.7"/><line x1={sideX+2} y1="7" x2={sideX+2} y2="22" stroke="#5e35b1" strokeWidth="3.5"/></>);

  // GATO - elegante, misterioso
  const CatSombrero=()=>T(<><ellipse cx="0" cy={hatY+8} rx="26" ry="6" fill="#9c27b0"/><path d="M-20 ${hatY+8} Q-18 ${hatY-10} 0 ${hatY-12} Q18 ${hatY-10} 20 ${hatY+8}" fill="#7b1fa2"/><ellipse cx="0" cy={hatY+7} rx="14" ry="3" fill="#ba68c8" opacity="0.4"/></>);
  const CatGafas=()=>T(<><circle cx="-13" cy={glassesY} r="9" fill="none" stroke="#7b1fa2" strokeWidth="2.3"/><circle cx="13" cy={glassesY} r="9" fill="none" stroke="#7b1fa2" strokeWidth="2.3"/><line x1="-4" y1={glassesY} x2="4" y2={glassesY} stroke="#7b1fa2" strokeWidth="2.3"/><circle cx="-13" cy={glassesY} r="6" fill="#ce93d8" opacity="0.3"/><circle cx="13" cy={glassesY} r="6" fill="#ce93d8" opacity="0.3"/></>);
  const CatCollar=()=>T(<><ellipse cx="0" cy={bodyY+4} rx="18" ry="3" fill="#e91e63"/><circle cx="0" cy={bodyY+4} r="4" fill="#FFD700"/><path d="M-2 ${bodyY+2} L0 ${bodyY} L2 ${bodyY+2}" fill="#f39c12"/></>);
  const CatBola=()=>T(<><circle cx={sideX} cy="2" r="10" fill="#e91e63" opacity="0.6"/><circle cx={sideX-3} cy="-1" r="3" fill="#f48fb1" opacity="0.8"/><line x1={sideX-6} y1="8" x2={sideX-12} y2="18" stroke="#c2185b" strokeWidth="2.5" strokeLinecap="round"/><line x1={sideX+6} y1="8" x2={sideX+12} y2="18" stroke="#c2185b" strokeWidth="2.5" strokeLinecap="round"/></>);

  // Mapeo de accesorios por animal
  const animalOutfits={
    owl:{m1:<OwlBirrete/>,m2:<OwlGafas/>,m3:<OwlCapa/>,m4:<OwlLibro/>},
    bear:{m1:<BearCasco/>,m2:<BearGafas/>,m3:<BearCapa/>,m4:<BearHacha/>},
    whale:{m1:<WhaleSombrero/>,m2:<WhaleGafas/>,m3:<WhaleChaleco/>,m4:<WhaleAncla/>},
    elephant:{m1:<ElephantCorona/>,m2:<ElephantGafas/>,m3:<ElephantManto/>,m4:<ElephantCetro/>},
    giraffe:{m1:<GiraffeSombrero/>,m2:<GiraffeGafas/>,m3:<GiraffePañuelo/>,m4:<GiraffePaleta/>},
    lion:{m1:<LionCorona/>,m2:<LionGafas/>,m3:<LionCapa/>,m4:<LionEspada/>},
    fox:{m1:<FoxSombrero/>,m2:<FoxGafas/>,m3:<FoxBufanda/>,m4:<FoxLupa/>},
    rabbit:{m1:<RabbitMonito/>,m2:<RabbitGafas/>,m3:<RabbitChaleco/>,m4:<RabbitZanahoria/>},
    dragon:{m1:<DragonCasco/>,m2:<DragonGafas/>,m3:<DragonCapa/>,m4:<DragonOrbe/>},
    cat:{m1:<CatSombrero/>,m2:<CatGafas/>,m3:<CatCollar/>,m4:<CatBola/>},
  };

  // Accesorios genéricos para materias (sin animal específico)
  const T_default=(children)=><g transform={`translate(50,26)`}>{children}</g>;
  const DefaultBirrete=()=>T_default(<><rect x="-19" y="-34" width="38" height="7" rx="2" fill="#1a1a2e"/><rect x="-13" y="-42" width="26" height="9" rx="2" fill="#1a1a2e"/><line x1="11" y1="-40" x2="21" y2="-31" stroke="#FFD700" strokeWidth="2.5" strokeLinecap="round"/><circle cx="22" cy="-30" r="3.5" fill="#FFD700"/></>);
  const DefaultGafas=()=>T_default(<><circle cx="-13" cy="4" r="9" fill="none" stroke="#222" strokeWidth="2.2"/><circle cx="13" cy="4" r="9" fill="none" stroke="#222" strokeWidth="2.2"/><line x1="-4" y1="4" x2="4" y2="4" stroke="#222" strokeWidth="2.2"/></>);
  const DefaultCapa=()=>T_default(<><rect x="-14" y="22" width="28" height="5" rx="2" fill="#FFD700"/><path d="M-14 27 Q-18 38 -16 54 L0 47 L16 54 Q18 38 14 27 Z" fill="#E53935"/></>);
  const DefaultCorona=()=>T_default(<><path d="M-20 -26 L-20 -38 L-8 -30 L0 -44 L8 -30 L20 -38 L20 -26 Z" fill="#FFD700"/><circle cx="0" cy="-43" r="3.5" fill="#E53935"/></>);

  const outfitMap = animalOutfits[animalKey] || {
    m1:<DefaultBirrete/>,m2:<DefaultGafas/>,m3:<DefaultCapa/>,m4:<DefaultCorona/>,
    c1:<DefaultGafas/>,c2:<DefaultCapa/>,c3:<DefaultBirrete/>,c4:<DefaultCorona/>,
    h1:<DefaultBirrete/>,h2:<DefaultGafas/>,h3:<DefaultCapa/>,h4:<DefaultCorona/>,
    e1:<DefaultGafas/>,e2:<DefaultCapa/>,e3:<DefaultBirrete/>,e4:<DefaultCorona/>,
    d1:<DefaultBirrete/>,d2:<DefaultGafas/>,d3:<DefaultCapa/>,d4:<DefaultCorona/>,
  };
...
const SchoolMSFooter=()=>(
  <div style={{
    position:"fixed",bottom:0,left:0,right:0,zIndex:299,
    background:"rgba(255,255,255,0.96)",
    backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
    borderTop:"0.5px solid rgba(60,60,67,0.1)",
    padding:"5px 0 max(6px,env(safe-area-inset-bottom,6px))",
    display:"flex",alignItems:"center",justifyContent:"center",
    gap:6,pointerEvents:"none",
  }}>
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <rect x="1" y="8" width="14" height="7" rx="1.5" fill="#007AFF" opacity="0.9"/>
      <path d="M8 1L15 7H1L8 1Z" fill="#007AFF"/>
      <rect x="6" y="9.5" width="4" height="5.5" rx="1" fill="#fff"/>
    </svg>
    <span style={{
...
const TabBar=({tabs,active,onChange,accent=C.blue})=>{
  ...
  if (isDesktop) {
    return (
      <div style={{display:"flex",gap:2,padding:"6px 12px",background:"rgba(255,255,255,0.97)",
        ...
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>{SFX.play("click");onChange(t.id);}}
            style={{display:"flex",alignItems:"center",gap:7,
              ...
              fontSize:15,fontWeight:active===t.id?700:400,fontFamily:SF,
  ...
  // Mobile: big icons only, no labels — SCHOOLMS footer shows below
  return(
    <>
      <div style={{
          ...
          return(
            <button key={t.id} onClick={()=>{SFX.play("click");onChange(t.id);}}
              style={{flex:1,display:"flex",flexDirection:"column",
                ...
                padding:"4px 2px",
                transition:"transform 0.12s",transform:isActive?"scale(1.08)":"scale(1)"}}>
              <div style={{
                ...
                <span style={{fontSize:24,lineHeight:1,filter:isActive?"none":"grayscale(0.55)",
                  opacity:isActive?1:0.6,transition:"all 0.15s"}}>{t.icon}</span>
              </div>
              <span style={{fontSize:10,fontWeight:isActive?700:400,
                color:isActive?accent:"rgba(60,60,67,0.55)",fontFamily:SF,
                letterSpacing:"-0.1px",lineHeight:1}}>{t.label}</span>
            </button>
          );
        })}
      </div>
...
const Feed=({state,setState,userId,userName,userAvatar,userColor,userRole,accent=C.blue,newsItems=[],urgentCount=0})=>{
  ...
  const deletePost=(id)=>{setState(s=>({...s,posts:s.posts.filter(p=>p.id!==id)}));setDeleteConfirmId(null);setMenuPostId(null);};
  const startEdit=(post)=>{setEditingPostId(post.id);setEditForm({title:post.title,body:post.body||""});};
  const saveEdit=(id)=>{setState(s=>({...s,posts:s.posts.map(p=>p.id===id?{...p,title:editForm.title,body:editForm.body}:p)}));setEditingPostId(null);};
  const canModify=(post)=>post.authorId===userId||userRole==="director";
  ...
  return(
    <div style={{background:C.bg2,minHeight:"100vh"}}>
      <div style={{position:"relative"}}>
        ...
        {userRole==="student"&&<LionTitleDeco/>}
      </div>
          ...
          return(
            <Card key={post.id} style={{marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"13px 14px 8px"}}>
                ...
                {canModify(post)&&(
                  <div style={{position:"relative"}}>
                    <button onClick={e=>{e.stopPropagation();setMenuPostId(menuPostId===post.id?null:post.id);}}
                      style={{background:"none",border:"none",cursor:"pointer",padding:"4px 7px",borderRadius:8,
                        color:C.lbl3,fontSize:18,lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      ⋯
                    </button>
                    {menuPostId===post.id&&(
                      <>
                        <div onClick={()=>setMenuPostId(null)} style={{position:"fixed",inset:0,zIndex:800}}/>
                        <div style={{position:"absolute",top:32,right:0,background:"#fff",borderRadius:12,
                          boxShadow:"0 4px 24px rgba(0,0,0,0.18)",border:`1px solid ${C.sep}`,
                          zIndex:810,minWidth:140,overflow:"hidden",animation:"fadeUp 0.12s ease"}}>
                          <button onClick={()=>{startEdit(post);setMenuPostId(null);}}
                            style={{width:"100%",padding:"11px 16px",background:"none",border:"none",cursor:"pointer",
                              textAlign:"left",fontSize:14,color:C.lbl,fontFamily:SF,display:"flex",gap:10,alignItems:"center"}}
                            onMouseEnter={e=>e.currentTarget.style.background=C.fill4}
                            onMouseLeave={e=>e.currentTarget.style.background="none"}>
                            <span>✏️</span> Editar
                          </button>
                          <div style={{height:"0.5px",background:C.sep}}/>
                          <button onClick={()=>{setDeleteConfirmId(post.id);setMenuPostId(null);}}
                            style={{width:"100%",padding:"11px 16px",background:"none",border:"none",cursor:"pointer",
                              textAlign:"left",fontSize:14,color:C.red,fontFamily:SF,display:"flex",gap:10,alignItems:"center"}}
                ...
                {deleteConfirmId===post.id&&(
                  <>
                    <div onClick={()=>setDeleteConfirmId(null)} style={{position:"fixed",inset:0,zIndex:800,background:"rgba(0,0,0,0.4)"}}/>
                    <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",
                      background:"#fff",borderRadius:18,padding:24,zIndex:810,width:280,textAlign:"center",
                      boxShadow:"0 12px 48px rgba(0,0,0,0.22)"}}>
                      <div style={{fontSize:36,marginBottom:10}}>🗑️</div>
                      <div style={{fontSize:16,fontWeight:700,color:C.lbl,fontFamily:SFD,marginBottom:6}}>¿Eliminar publicación?</div>
                      <div style={{fontSize:13,color:C.lbl2,fontFamily:SF,marginBottom:20,lineHeight:1.5}}>Esta acción no se puede deshacer.</div>
                      <div style={{display:"flex",gap:10}}>
                        <button onClick={()=>setDeleteConfirmId(null)}
                          style={{flex:1,padding:"11px",borderRadius:12,background:C.fill4,border:"none",cursor:"pointer",
                            fontSize:14,fontWeight:600,color:C.lbl,fontFamily:SF}}>Cancelar</button>
                        <button onClick={()=>deletePost(post.id)}
                          style={{flex:1,padding:"11px",borderRadius:12,background:C.red,border:"none",cursor:"pointer",
                            fontSize:14,fontWeight:600,color:"#fff",fontFamily:SF}}>Eliminar</button>
                      </div>
                    </div>
                  </>
                )}
              ...
              {editingPostId===post.id?(
                <div style={{padding:"8px 14px 12px"}}>
                  <input value={editForm.title} onChange={e=>setEditForm(f=>({...f,title:e.target.value}))}
                    style={{width:"100%",background:C.fill4,border:`1px solid ${accent}40`,borderRadius:10,padding:"9px 13px",fontSize:15,fontWeight:600,color:C.lbl,fontFamily:SF,outline:"none",boxSizing:"border-box",marginBottom:8}}/>
                  <textarea value={editForm.body} onChange={e=>setEditForm(f=>({...f,body:e.target.value}))} rows={3}
                    style={{width:"100%",background:C.fill4,border:`1px solid ${accent}40`,borderRadius:10,padding:"9px 13px",fontSize:14,color:C.lbl,fontFamily:SF,resize:"none",outline:"none",boxSizing:"border-box",marginBottom:10}}/>
                  <div style={{display:"flex",gap:8}}>
                    <Btn onPress={()=>saveEdit(post.id)} color={accent} size="sm" style={{flex:1}}>Guardar</Btn>
                    <Btn onPress={()=>setEditingPostId(null)} variant="ghost" size="sm" style={{flex:1}}>Cancelar</Btn>
                  </div>
                </div>
              ):(
...
const AppSidebar=({open,onToggle,gradient,accent=C.blue,logoEmoji="🏫",logoLine1="Instituto",logoLine2="Educativo",
  userEmoji,userName,userSub,navItems=[],onLogout,profileInitials=null,profilePic=null,profileColor=null,onLogoClick=null,bellNode=null})=>{
  ...
  if(isMobile){
    ...
    // Avatar helper
    const AvatarBubble=()=>profileInitials?(
      <div style={{width:42,height:42,borderRadius:"50%",overflow:"hidden",flexShrink:0,
        border:"2px solid rgba(255,255,255,0.4)",boxShadow:"0 1px 6px rgba(0,0,0,0.18)"}}>
        {profilePic
          ?<img src={profilePic} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          :<div style={{width:"100%",height:"100%",
            background:`linear-gradient(135deg,${profileColor||"#fff"},${profileColor||"#fff"}88)`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:15,fontWeight:800,color:"#fff",fontFamily:SF}}>
            {profileInitials}
          </div>}
      </div>
    ):null;
    return(
      <>
        ...
        <div style={{
          ...
          {/* Hamburger — sin badge roja */}
          <button onClick={()=>{SFX.play("click");setDrawerOpen(true);}}
            style={{
            ...
            <span style={{display:"block",width:24,height:2.8,background:accent,borderRadius:3}}/>
            <span style={{display:"block",width:24,height:2.8,background:accent,borderRadius:3}}/>
            <span style={{display:"block",width:24,height:2.8,background:accent,borderRadius:3}}/>
          </button>

          {/* SCHOOLMS logo + nombre */}
          <div onClick={()=>{SFX.play("click");onLogoClick&&onLogoClick();}}
            style={{flex:1,display:"flex",alignItems:"center",gap:9,marginLeft:8,cursor:onLogoClick?"pointer":"default"}}>
            <svg width="34" height="34" viewBox="0 0 16 16" fill="none">
              <rect x="1" y="8" width="14" height="7" rx="1.5" fill={accent} opacity="0.9"/>
              <path d="M8 1L15 7H1L8 1Z" fill={accent}/>
              <rect x="6" y="9.5" width="4" height="5.5" rx="1" fill="#fff"/>
            </svg>
            <span style={{
              fontFamily:SFD,fontSize:28,fontWeight:800,letterSpacing:"-0.5px",
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            {bellNode&&<div style={{position:"relative"}}>{bellNode}</div>}
            <AvatarBubble/>
          </div>
        </div>
        ...
        {/* ── SLIDE-OUT DRAWER ── */}
        <div style={{
          position:"fixed",top:0,left:0,bottom:0,width:285,zIndex:9999,
          background:gradient,
          transform:drawerOpen?"translateX(0)":"translateX(-100%)",
          transition:"transform 0.27s cubic-bezier(0.4,0,0.2,1)",
          display:"flex",flexDirection:"column",overflowY:"auto",
          boxShadow:drawerOpen?"5px 0 32px rgba(0,0,0,0.3)":"none",
        }}>
          {/* Drawer header with profile */}
          <div style={{paddingTop:"max(env(safe-area-inset-top,0px),16px)",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"0 16px 14px",borderBottom:"1px solid rgba(255,255,255,0.12)"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{minWidth:0}}>
                  <div style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:SF2,
                    overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{userName}</div>
                  {userSub&&<div style={{fontSize:11,color:"rgba(255,255,255,0.65)",fontFamily:SF2}}>{userSub}</div>}
                </div>
              </div>
          ...
          <div style={{flex:1,padding:"8px 10px",overflowY:"auto"}}>
            {navItems.map(item=>{
              ...
              return(
                <div key={item.id}>
                  <button onClick={()=>{SFX.play("click");item.onClick();closeDrawer();}}
                    ...
                    {isActive&&<div style={{position:"absolute",left:0,top:"50%",
                      transform:"translateY(-50%)",width:4,height:26,
                      background:"rgba(255,255,255,0.9)",borderRadius:"0 3px 3px 0"}}/>}
                    ...
                    <span style={{fontSize:15,fontWeight:isActive?700:500,color:"#fff",
                      fontFamily:SF2,flex:1,textAlign:"left",letterSpacing:"-0.2px"}}>{item.label}</span>
                    {item.badge>0&&(
                      <span style={{background:"#ef4444",color:"#fff",fontSize:11,fontWeight:800,
                        borderRadius:10,padding:"2px 7px",fontFamily:SF2,flexShrink:0,
                        minWidth:20,textAlign:"center"}}>{item.badge}</span>
                    )}
                  </button>
                  {isActive&&item.subItems?.length>0&&(
                    <div style={{paddingLeft:16,marginBottom:4,
                      borderLeft:"2px solid rgba(255,255,255,0.2)",marginLeft:24}}>
                      {item.subItems.map(sub=>(
                        <button key={sub.id} onClick={()=>{SFX.play("click");sub.onClick();closeDrawer();}}
                          style={{width:"100%",display:"flex",alignItems:"center",gap:10,
                            padding:"9px 12px",borderRadius:10,border:"none",cursor:"pointer",
                            marginBottom:2,
                            background:sub.active?"rgba(255,255,255,0.18)":"transparent",
                            transition:"background 0.15s"}}
                          onMouseEnter={e=>{ if(!sub.active) e.currentTarget.style.background="rgba(255,255,255,0.08)"; }}
                          onMouseLeave={e=>{ if(!sub.active) e.currentTarget.style.background="transparent"; }}>
                          <span style={{fontSize:15,width:18,textAlign:"center",flexShrink:0}}>{sub.icon}</span>
                          <span style={{fontSize:13,fontWeight:sub.active?700:400,flex:1,textAlign:"left",
                            color:sub.active?"#fff":"rgba(255,255,255,0.7)",fontFamily:SF2}}>{sub.label}</span>
                          {sub.badge>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:10,
                            fontWeight:700,borderRadius:7,padding:"1px 5px",fontFamily:SF2}}>{sub.badge}</span>}
                        </button>
                      ))}
                    </div>
                  )}
          ...
          {/* Logout */}
          <div style={{padding:"10px 10px 24px",borderTop:"1px solid rgba(255,255,255,0.12)",flexShrink:0}}>
            <button onClick={()=>{SFX.play("click");onLogout();closeDrawer();}}
              style={{width:"100%",background:"rgba(255,255,255,0.1)",
                ...
                fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:SF2,
                display:"flex",alignItems:"center",gap:12,transition:"background 0.15s"}}
              onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.22)"}
              onMouseLeave={e=>e.currentTarget.style.background="rgba(255,255,255,0.1)"}>
              <div style={{width:38,height:38,borderRadius:11,background:"rgba(255,80,80,0.25)",
...
const ChatPanel=({state,setState,myUserId,myName,myAvatar,myColor,role,accent=C.blue})=>{
    ...
    setState(s=>{
      const msgs=(s.chatMessages?.[selectedChatId]||[]).map(m=>{
        if((m.id||m._id)===msgId){
          ...
          const liked=hearts.includes(myUserId);
          return {...m,hearts:liked?hearts.filter(u=>u!==myUserId):[...hearts,myUserId]};
        }
        return m;
      });
  ...
  // ── Chat list view (Messenger style) ────────────────────────────────────────
  if(view==="list"||view==="new"){
    ...
    return(
      <div style={{background:"#fff",
        position:"fixed",top:68,left:0,right:0,bottom:"calc(var(--tab-h,84px) + 20px)",
        fontFamily:SF_local,display:"flex",flexDirection:"column",overflowY:"auto",zIndex:10}}>
        {/* Desktop layout hint */}
        <style>{`
          @media(min-width:1024px){
            .chat-list-container{max-width:420px!important;border-right:1px solid rgba(0,0,0,0.08)!important;}
          }
        `}</style>
        <div className="chat-list-container" style={{width:"100%",display:"flex",flexDirection:"column",height:"100%"}}>
        <div style={{background:"#fff",borderBottom:"1px solid rgba(0,0,0,0.08)",
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {view==="new"&&(
                <button onClick={()=>{setView("list");setNewChatSearch("");setShowGroupForm(false);setSelectedMembers([]);setGroupName("");}}
                  style={{background:"#f0f2f5",border:"none",borderRadius:"50%",width:36,height:36,cursor:"pointer",
                    fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",color:"#000"}}>←</button>
              )}
              <div style={{fontSize:26,fontWeight:900,color:"#050505",fontFamily:"-apple-system,system-ui,sans-serif",
                letterSpacing:"-0.5px"}}>{view==="new"?showGroupForm?"Nuevo grupo":"Nueva conversación":"Chats"}</div>
            </div>
            {view==="list"&&(
              <button onClick={()=>{setView("new");setNewChatSearch("");setShowGroupForm(false);}}
                style={{width:36,height:36,borderRadius:"50%",background:"#f0f2f5",border:"none",
                  cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>✏️</button>
            )}
          </div>
          <div style={{background:"#f0f2f5",borderRadius:20,display:"flex",alignItems:"center",gap:8,
            ...
            <input value={newChatSearch} onChange={e=>setNewChatSearch(e.target.value)}
              ...
              style={{flex:1,background:"transparent",border:"none",outline:"none",
                fontSize:15,color:"#050505",fontFamily:SF_local}}/>
          </div>
        </div>

        <div style={{flex:1,overflowY:"auto",paddingBottom:120}}>
          {view==="new"&&showGroupForm&&(
            <div style={{background:"#fff",margin:"8px 12px",borderRadius:16,padding:16,
              boxShadow:"0 1px 6px rgba(0,0,0,0.08)"}}>
              <input value={groupName} onChange={e=>setGroupName(e.target.value)}
                placeholder="Nombre del grupo…"
                style={{width:"100%",background:"#f0f2f5",border:"none",borderRadius:12,padding:"10px 14px",
                  fontSize:15,fontWeight:600,color:"#050505",fontFamily:SF_local,outline:"none",boxSizing:"border-box",marginBottom:10}}/>
              {selectedMembers.length>0&&(
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
                  {selectedMembers.map(id=>{
                    const c=contacts.find(x=>x.id===id);
                    return c?(<div key={id} style={{display:"flex",alignItems:"center",gap:5,
                      background:`${c.color}18`,borderRadius:20,padding:"4px 10px"}}>
                      <span style={{fontSize:12,fontWeight:600,color:c.color}}>{c.name.split(" ")[0]}</span>
                      <button onClick={()=>setSelectedMembers(m=>m.filter(x=>x!==id))}
                        style={{background:"none",border:"none",color:c.color,cursor:"pointer",fontSize:12,padding:0}}>✕</button>
                    </div>):null;
                  })}
                </div>
              )}
              <button onClick={createGroup} disabled={!groupName.trim()||selectedMembers.length===0}
                style={{width:"100%",background:selectedMembers.length>0&&groupName.trim()?MSN_BLUE:"#e4e6eb",
                  border:"none",borderRadius:12,padding:12,fontSize:15,fontWeight:700,cursor:"pointer",
                  color:selectedMembers.length>0&&groupName.trim()?"#fff":"#bcc0c4",fontFamily:SF_local}}>
                Crear grupo ({selectedMembers.length} personas)
              </button>
            </div>
          )}
          ...
          {view==="list"&&(
            <div>
              {myChats.length===0&&(
                <div style={{textAlign:"center",padding:"60px 24px",color:"#65676b"}}>
                  <div style={{fontSize:56,marginBottom:12}}>💬</div>
                  <div style={{fontSize:20,fontWeight:700,color:"#050505",marginBottom:6}}>Sin mensajes</div>
                  <div style={{fontSize:15}}>Toca ✏️ para iniciar una conversación</div>
                </div>
              )}
              {myChats.map((c)=>{
                  ...
                  <div key={c.id} onClick={()=>{
                    ...
                    <div style={{position:"relative",flexShrink:0}}>
                      <div style={{width:56,height:56,borderRadius:"50%",
                        background:`linear-gradient(135deg,${clr},${clr}88)`,
                        display:"flex",alignItems:"center",justifyContent:"center",
                        fontSize:c.type==="group"?24:18,fontWeight:700,color:"#fff",fontFamily:SF_local}}>
                        {c.type==="group"?"👥":initials}
                      </div>
                      <div style={{position:"absolute",bottom:2,right:2,width:13,height:13,
                        borderRadius:"50%",background:"#31a24c",border:"2.5px solid #fff"}}/>
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:2}}>
                        <div style={{fontSize:16,fontWeight:unread>0?700:600,color:"#050505",
                          overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>{displayName}</div>
                        <div style={{fontSize:12,color:unread>0?MSN_BLUE:"#65676b",
                          flexShrink:0,marginLeft:8,fontWeight:unread>0?600:400}}>{c.lastTime||""}</div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <div style={{fontSize:14,color:unread>0?"#050505":"#65676b",
                          overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1,
                          fontWeight:unread>0?600:400}}>{c.lastMsg||"Toca para chatear"}</div>
                        {unread>0&&<div style={{background:MSN_BLUE,color:"#fff",fontSize:11,fontWeight:800,
                          borderRadius:"50%",minWidth:20,height:20,display:"flex",alignItems:"center",
                          justifyContent:"center",flexShrink:0,padding:"0 4px"}}>{unread}</div>}
                      </div>
                    </div>
                  </div>
                );
            ...
            <div>
              {!showGroupForm&&(
                <button onClick={()=>setShowGroupForm(true)}
                  style={{display:"flex",alignItems:"center",gap:14,padding:"10px 16px",width:"100%",
                    background:"transparent",border:"none",cursor:"pointer",textAlign:"left"}}
                  onMouseEnter={e=>e.currentTarget.style.background="#f2f2f2"}
                  onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{width:52,height:52,borderRadius:"50%",background:"#e4e6eb",
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>👥</div>
                  <div style={{fontSize:15,fontWeight:600,color:"#050505"}}>Crear grupo</div>
                </button>
              )}
              <div style={{padding:"8px 16px 4px"}}>
                <div style={{fontSize:13,fontWeight:600,color:"#65676b",textTransform:"uppercase",
                  letterSpacing:"0.05em"}}>{showGroupForm?"Añadir participantes":"Contactos"}</div>
              </div>
              {filteredContacts.length===0&&(
                <div style={{padding:"20px 16px",color:"#65676b",fontSize:15,textAlign:"center"}}>Sin resultados</div>
              )}
              {filteredContacts.map((contact)=>{
                ...
                return(
                  <div key={contact.id} onClick={()=>{
                    ...
                    background:isSel?"#f0f8ff":"transparent",transition:"background 0.1s"}}
                    onMouseEnter={e=>!isSel&&(e.currentTarget.style.background="#f2f2f2")}
                    onMouseLeave={e=>e.currentTarget.style.background=isSel?"#f0f8ff":"transparent"}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontSize:15,fontWeight:600,color:"#050505"}}>{contact.name}</div>
                      <div style={{fontSize:13,color:"#65676b"}}>{contact.role}</div>
                    </div>
                    {showGroupForm&&(
                      <div style={{width:24,height:24,borderRadius:"50%",
                        background:isSel?MSN_BLUE:"transparent",
                        border:`2px solid ${isSel?MSN_BLUE:"#ccd0d5"}`,
                        display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
  ...
  // ── Chat view (full Messenger style) ──────────────────────────────────────
  if(view==="chat"&&selectedChat){
    ...
    return(
      <>
      {/* Desktop: overlay background para cerrar chat */}
      <style>{`
        @media(min-width:1024px){
          .chat-view-desktop{position:fixed!important;top:68px!important;left:0!important;right:0!important;
            bottom:calc(var(--tab-h,84px) + 20px)!important;display:flex!important;}
          .chat-view-desktop .back-btn-mobile{display:none!important;}
        }
        @media(max-width:1023px){
          .chat-view-desktop{position:fixed!important;top:68px!important;left:0!important;right:0!important;
            bottom:calc(var(--tab-h,84px) + 20px)!important;}
        }
      `}</style>
      <div className="chat-view-desktop" style={{background:"#f0f2f5",fontFamily:SF_local,
        display:"flex",flexDirection:"column",overflow:"hidden",zIndex:10}}>
        {/* Messenger Header */}
        <div style={{background:"#fff",borderBottom:"1px solid rgba(0,0,0,0.08)",flexShrink:0,zIndex:10}}>
          <div style={{display:"flex",alignItems:"center",padding:"10px 12px",gap:10}}>
            <button onClick={()=>setView("list")} className="back-btn-mobile"
              style={{background:"none",border:"none",borderRadius:"50%",width:36,height:36,
                cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center",
                color:MSN_BLUE,flexShrink:0}}>←</button>
            <div style={{position:"relative",flexShrink:0}}>
              <div style={{width:42,height:42,borderRadius:"50%",
                background:`linear-gradient(135deg,${clr},${clr}88)`,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:selectedChat.type==="group"?20:14,fontWeight:700,color:"#fff"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:12,color:"#31a24c",fontWeight:500}}>
                {selectedChat.type==="group"?`${selectedChat.participants.length} participantes`:"En línea"}
              </div>
            </div>
            <div style={{display:"flex",gap:4,flexShrink:0}}>
              {[{icon:"📞"},{icon:"📹"},{icon:"ℹ️"}].map((btn,bi)=>(
                <button key={bi}
                  style={{width:36,height:36,borderRadius:"50%",background:"#e4e6eb",border:"none",
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px 12px",display:"flex",
          flexDirection:"column",gap:1,WebkitOverflowScrolling:"touch",minHeight:0}}>
          {selectedMessages.length===0&&(
            <div style={{textAlign:"center",padding:"40px 20px"}}>
              <div style={{width:72,height:72,borderRadius:"50%",
                background:`linear-gradient(135deg,${clr},${clr}88)`,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:30,fontWeight:700,color:"#fff",margin:"0 auto 14px"}}>
                {selectedChat.type==="group"?"👥":(otherContact.avatar||getInitials(displayName))}
              </div>
              <div style={{fontSize:19,fontWeight:700,color:"#050505",marginBottom:4}}>{displayName}</div>
              <div style={{fontSize:14,color:"#65676b"}}>Di hola 👋</div>
            </div>
          )}
          {selectedMessages.map((msg,i)=>{
            ...
            const nextMsg=i<selectedMessages.length-1?selectedMessages[i+1]:null;
            const showDate=!prevMsg||prevMsg.date!==msg.date;
            const isLastInGroup=!nextMsg||nextMsg.from!==msg.from||nextMsg.date!==msg.date;
            const isFirstInGroup=!prevMsg||prevMsg.from!==msg.from||prevMsg.date!==msg.date;
            const br=(tl,tr,br2,bl)=>`${tl}px ${tr}px ${br2}px ${bl}px`;
            const bubbleRadius=isMe
              ?(isFirstInGroup&&isLastInGroup?br(18,18,4,18):isFirstInGroup?br(18,18,4,18):isLastInGroup?br(18,4,18,18):br(18,4,4,18))
              :(isFirstInGroup&&isLastInGroup?br(18,18,18,4):isFirstInGroup?br(18,18,18,4):isLastInGroup?br(4,18,18,18):br(4,18,18,4));
            return(
              <React.Fragment key={msg.id||i}>
                {showDate&&(
                  <div style={{textAlign:"center",margin:"14px 0 6px"}}>
                    <span style={{fontSize:12,color:"#65676b",padding:"2px 10px"}}>
                      {msg.date===new Date().toISOString().slice(0,10)?"Hoy":msg.date}
                    </span>
                  </div>
                )}
                <div style={{display:"flex",flexDirection:isMe?"row-reverse":"row",
                  ...
                  {!isMe&&(
                    <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,
                      background:isLastInGroup?`linear-gradient(135deg,${clr},${clr}88)`:"transparent",
                      display:"flex",alignItems:"center",justifyContent:"center",
                      fontSize:10,fontWeight:700,color:"#fff",marginBottom:2,alignSelf:"flex-end"}}>
                      {isLastInGroup?(selectedChat.type==="group"
                        ?(msg.fromName||"?").split(" ").map(p=>p[0]).slice(0,2).join("").toUpperCase()
                        :(otherContact.avatar||getInitials(displayName)))
                        :""}
                    </div>
                  )}
                  <div style={{maxWidth:"72%",display:"flex",flexDirection:"column",
                    ...
                    {showName&&isFirstInGroup&&(
                      <div style={{fontSize:11,fontWeight:600,color:"#65676b",
                        marginBottom:3,paddingLeft:12}}>{msg.fromName}</div>
                    )}
                    {msg.type==="poll"?(
                      ...
                      <div>
                        {msg.images?.length>0&&(
                          <div style={{display:"flex",gap:2,flexWrap:"wrap",marginBottom:msg.text?3:0,
                            justifyContent:isMe?"flex-end":"flex-start"}}>
                            {msg.images.map(img=>(
                              <img key={img.id} src={img.src}
                                style={{width:msg.images.length===1?"100%":80,
                                  height:msg.images.length===1?220:80,
                        ...
                        {msg.files?.length>0&&msg.files.map(f=>(
                          <div key={f.id} style={{display:"flex",alignItems:"center",gap:10,
                            background:isMe?MSN_BLUE:"#e4e6eb",borderRadius:bubbleRadius,
                            padding:"10px 14px",marginBottom:msg.text?2:0}}>
                            <span style={{fontSize:22}}>📎</span>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:13,fontWeight:600,color:isMe?"#fff":"#050505",
                                overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</div>
                              {f.size&&<div style={{fontSize:11,color:isMe?"rgba(255,255,255,0.7)":"#65676b"}}>{f.size}</div>}
                            </div>
                          </div>
                        ))}
                        {msg.text&&(
                          <div style={{
                            background:isMe?MSN_BLUE:"#e4e6eb",
                        ...
                        {isLastInGroup&&(
                          <div style={{fontSize:11,color:"#65676b",marginTop:3,
                            paddingLeft:isMe?0:10,paddingRight:isMe?10:0,
                        ...
                        {(()=>{
                          ...
                          return(
                            <div style={{display:"flex",alignItems:"center",gap:4,marginTop:2,
                              justifyContent:isMe?"flex-end":"flex-start"}}>
                              <button onClick={()=>toggleHeart(msgId)}
                                style={{background:liked?"#ffe4e4":"transparent",border:`1px solid ${liked?"#ffb3b3":"#e4e6eb"}`,
                                  borderRadius:20,padding:"2px 8px",cursor:"pointer",
                                  display:"flex",alignItems:"center",gap:3,
                                  fontSize:13,transition:"all 0.15s",
                                  color:liked?"#e11d48":"#65676b"}}>
                                <span style={{fontSize:14,transition:"transform 0.15s",
                                  transform:liked?"scale(1.2)":"scale(1)",display:"inline-block"}}>
                                  {liked?"❤️":"🤍"}
                                </span>
        ...
        {/* Poll composer */}
        {showPoll&&(
          <div style={{flexShrink:0,background:"#fff",borderTop:"1px solid #e4e6eb",padding:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:15,fontWeight:700,color:"#050505"}}>📊 Nueva Encuesta</div>
              <button onClick={()=>setShowPoll(false)}
                style={{background:"#e4e6eb",border:"none",borderRadius:"50%",width:28,height:28,
                  cursor:"pointer",color:"#050505",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
            </div>
            <input value={pollQ} onChange={e=>setPollQ(e.target.value)} placeholder="Pregunta…"
              style={{width:"100%",background:"#f0f2f5",border:"none",borderRadius:10,padding:"9px 12px",
                fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:8}}/>
            {pollOpts.map((opt,i)=>(
              <div key={i} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}>
                <input value={opt} onChange={e=>setPollOpts(o=>{const n=[...o];n[i]=e.target.value;return n;})}
                  placeholder={`Opción ${i+1}…`}
                  style={{flex:1,background:"#f0f2f5",border:"none",borderRadius:10,padding:"8px 12px",
                    fontSize:13,outline:"none"}}/>
                {i>=2&&<button onClick={()=>setPollOpts(o=>o.filter((_,j)=>j!==i))}
                  style={{background:"none",border:"none",color:"#65676b",cursor:"pointer",fontSize:16}}>✕</button>}
              </div>
            ))}
            {pollOpts.length<5&&<button onClick={()=>setPollOpts(o=>[...o,""])}
              style={{fontSize:13,color:MSN_BLUE,background:"none",border:"none",cursor:"pointer",marginBottom:8}}>
              + Opción</button>}
            <button onClick={sendPoll} disabled={!pollQ.trim()||pollOpts.filter(o=>o.trim()).length<2}
              style={{width:"100%",background:pollQ.trim()&&pollOpts.filter(o=>o.trim()).length>=2?MSN_BLUE:"#e4e6eb",
                border:"none",borderRadius:12,padding:11,fontSize:15,fontWeight:700,cursor:"pointer",
                color:pollQ.trim()&&pollOpts.filter(o=>o.trim()).length>=2?"#fff":"#bcc0c4"}}>
              Enviar Encuesta
            </button>
          </div>
        )}
        ...
        {(attachImages.length>0||attachFiles.length>0)&&(
          <div style={{flexShrink:0,padding:"8px 12px",background:"#fff",borderTop:"1px solid #e4e6eb"}}>
            <div style={{display:"flex",gap:6,overflowX:"auto",scrollbarWidth:"none"}}>
              {attachImages.map(img=>(
                <div key={img.id} style={{position:"relative",flexShrink:0}}>
                  <img src={img.src} style={{width:64,height:64,borderRadius:10,objectFit:"cover"}}/>
                  <button onClick={()=>setAttachImages(a=>a.filter(x=>x.id!==img.id))}
                    style={{position:"absolute",top:-4,right:-4,width:18,height:18,borderRadius:"50%",
                      background:"#e41e3f",border:"2px solid #fff",cursor:"pointer",color:"#fff",
                      fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>✕</button>
                </div>
              ))}
              {attachFiles.map(f=>(
                <div key={f.id} style={{display:"flex",alignItems:"center",gap:6,background:"#f0f2f5",
                  borderRadius:10,padding:"6px 10px",flexShrink:0}}>
                  <span>📎</span>
                  <span style={{fontSize:12,whiteSpace:"nowrap"}}>{f.name.length>12?f.name.slice(0,12)+"…":f.name}</span>
                  <button onClick={()=>setAttachFiles(a=>a.filter(x=>x.id!==f.id))}
                    style={{background:"none",border:"none",color:"#65676b",cursor:"pointer",fontSize:13}}>✕</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{flexShrink:0,background:"#fff",borderTop:"1px solid #e4e6eb",
          padding:"6px 8px",paddingBottom:"max(6px,env(safe-area-inset-bottom,6px))",
          display:"flex",gap:3,alignItems:"flex-end",zIndex:20}}>
          <input ref={imgRef} type="file" accept="image/*" multiple style={{display:"none"}} onChange={addImg}/>
          <input ref={fileRef} type="file" multiple style={{display:"none"}} onChange={addFile}/>
          {[
            {icon:"📷",fn:()=>imgRef.current?.click()},
            {icon:"📁",fn:()=>fileRef.current?.click()},
          ].map((btn,bi)=>(
            <button key={bi} onClick={btn.fn}
              style={{width:32,height:32,borderRadius:"50%",background:"none",border:"none",
                cursor:"pointer",fontSize:18,flexShrink:0,display:"flex",alignItems:"center",
                justifyContent:"center",color:MSN_BLUE}}>
              {btn.icon}
            </button>
          ))}
          <div style={{flex:1,background:"#f0f2f5",borderRadius:18,display:"flex",
            alignItems:"flex-end",padding:"0 6px 0 12px",minHeight:32}}>
            <textarea value={msgText} onChange={e=>setMsgText(e.target.value)}
              onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}}
              placeholder="Aa" rows={1}
              style={{flex:1,background:"transparent",border:"none",padding:"7px 0",fontSize:14,
                color:"#050505",fontFamily:SF_local,outline:"none",resize:"none",lineHeight:"18px",
                maxHeight:80,overflowY:"auto"}}/>
          </div>
          <button onClick={()=>setShowPoll(s=>!s)}
            style={{width:32,height:32,borderRadius:"50%",background:"none",border:"none",
              cursor:"pointer",fontSize:16,flexShrink:0,display:"flex",alignItems:"center",
              justifyContent:"center",color:showPoll?MSN_BLUE:"#65676b"}}>📊</button>
          <button
            onClick={sendMessage}
            disabled={!msgText.trim()&&attachImages.length===0&&attachFiles.length===0}
            style={{width:34,height:34,borderRadius:"50%",
              background:(msgText.trim()||attachImages.length>0||attachFiles.length>0)?MSN_BLUE:"#e4e6eb",
              border:"none",cursor:"pointer",
              flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",
              boxShadow:(msgText.trim()||attachImages.length>0||attachFiles.length>0)?"0 2px 8px rgba(0,132,255,0.4)":"none",
              transition:"all 0.15s",touchAction:"manipulation",
              transform:"scale(1)"}}>
            <svg width="15" height="15" viewBox="0 0 14 14" fill="none">
              <path d="M12.5 1.5L7 7M12.5 1.5L8.5 12.5L7 7M12.5 1.5L1.5 5.5L7 7"
                stroke={(msgText.trim()||attachImages.length>0||attachFiles.length>0)?"#fff":"#999"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        </div>
      </div>
...
const DirGruposPanel=({state})=>{
  const [selGrp,setSelGrp]=useState(null);
  const [selStu,setSelStu]=useState(null);
  if(selStu){
    const s=selStu;
    const vals=Object.entries(s.subjects||{});
    return(
      <div>
        <button onClick={()=>setSelStu(null)} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:C.indigo,cursor:"pointer",fontFamily:SF,fontSize:14,fontWeight:600,padding:"4px 0",marginBottom:12}}>← Regresar</button>
        <Card style={{padding:16,marginBottom:12}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,marginBottom:16}}>
            <Ava initials={s.avatar} color={s.color} size={72} img={s.photo}/>
            <div style={{fontSize:18,fontWeight:700,color:C.lbl,fontFamily:SFD}}>{s.name}</div>
            <div style={{fontSize:13,color:C.lbl2,fontFamily:SF}}>Grupo {s.group}</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            {vals.map(([subj,data])=>{
              const g=data?.grade;
              const col=g>=9?C.green:g>=7?C.blue:g>=6?C.orange:C.red;
              return(
                <div key={subj} style={{background:`${col}10`,borderRadius:12,padding:"10px 12px",textAlign:"center"}}>
                  <div style={{fontSize:11,color:C.lbl2,fontFamily:SF,marginBottom:4}}>{subj}</div>
                  <div style={{fontSize:22,fontWeight:800,color:col,fontFamily:SFD}}>{g!=null?g.toFixed(1):"–"}</div>
                </div>
              );
            })}
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"space-around",padding:"10px 0",borderTop:`0.5px solid ${C.sep}`}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:16,fontWeight:700,color:C.blue,fontFamily:SF}}>{s.participation||0}</div>
              <div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>Participaciones</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:16,fontWeight:700,color:C.green,fontFamily:SF}}>{(s.attendance||[]).filter(a=>a.s==="present").length}</div>
              <div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>Asistencias</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:16,fontWeight:700,color:C.red,fontFamily:SF}}>{(s.attendance||[]).filter(a=>a.s==="absent").length}</div>
              <div style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>Faltas</div>
            </div>
          </div>
        </Card>
      </div>
  ...
  if(selGrp){
    ...
    return(
      <div>
        <button onClick={()=>setSelGrp(null)} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:C.indigo,cursor:"pointer",fontFamily:SF,fontSize:14,fontWeight:600,padding:"4px 0",marginBottom:12}}>← Todos los grupos</button>
        <div style={{...fmt.title3,fontWeight:700,color:C.lbl,fontFamily:SFD,marginBottom:4}}>Grupo {g.name}</div>
        <div style={{fontSize:13,color:C.lbl2,fontFamily:SF,marginBottom:14}}>{studs.length} alumnos inscritos</div>
        {studs.length===0&&<div style={{textAlign:"center",padding:"40px 16px",color:C.lbl2,fontSize:15,fontFamily:SF}}>Sin alumnos en este grupo.</div>}
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10}}>
          {studs.map(s=>{
            const vals=Object.values(s.subjects||{});
            const lastGrade=vals.length>0?vals[vals.length-1]?.grade:null;
            const gradeCol=lastGrade>=9?C.green:lastGrade>=7?C.blue:lastGrade>=6?C.orange:lastGrade!=null?C.red:C.g3;
            return(
              <div key={s.id} onClick={()=>setSelStu(s)}
                style={{background:"#fff",borderRadius:16,padding:"14px 12px",textAlign:"center",cursor:"pointer",
                  boxShadow:"0 1px 8px rgba(0,0,0,0.07)",border:`1px solid ${C.sep}`,transition:"transform 0.15s"}}
                onMouseEnter={e=>e.currentTarget.style.transform="scale(1.03)"}
                onMouseLeave={e=>e.currentTarget.style.transform=""}>
                <div style={{display:"flex",justifyContent:"center",marginBottom:8}}>
                  <Ava initials={s.avatar} color={s.color} size={52} img={s.photo}/>
                </div>
                <div style={{fontSize:12,fontWeight:700,color:C.lbl,fontFamily:SF,lineHeight:1.3,marginBottom:6}}>{s.name.split(" ").slice(0,2).join(" ")}</div>
                {lastGrade!=null?(
                  <div style={{background:`${gradeCol}15`,borderRadius:10,padding:"4px 10px",display:"inline-block"}}>
                    <span style={{fontSize:16,fontWeight:800,color:gradeCol,fontFamily:SFD}}>{lastGrade.toFixed(1)}</span>
                  </div>
                ):<span style={{fontSize:11,color:C.lbl3,fontFamily:SF}}>Sin cal.</span>}
              </div>
            );
          })}
        </div>
  ...
  return(
    <Sec title={`Grupos — ${state.groups.length}`}>
      {state.groups.map((g,i)=>{
        const studs=state.students.filter(s=>(g.students||[]).some(id=>String(id)===String(s.id))||s.group===g.name);
        const teacherOfGrp=state.teachers.find(t=>String(t.id)===String(g.teacherId));
        return(
          <div key={g.id}>
            <div onClick={()=>setSelGrp(g)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",cursor:"pointer"}}
              onMouseEnter={e=>e.currentTarget.style.background=C.fill4}
              onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              ...
              <div style={{flex:1}}>
                <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>Grupo {g.name}</div>
                <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF,marginTop:1}}>{teacherOfGrp?.name||"Sin docente"} · {studs.length} alumnos</div>
              </div>
...
const DirectorApp=({state,setState,onLogout})=>{
  ...
  // Director theme & profile
  const DIR_THEMES=[
    {key:"indigo",label:"Índigo",gradient:"linear-gradient(180deg,#312e81 0%,#4338ca 60%,#6366f1 100%)",accent:"#4338ca",btn:"#4338ca"},
    {key:"slate",label:"Pizarra",gradient:"linear-gradient(180deg,#1e293b 0%,#334155 60%,#475569 100%)",accent:"#334155",btn:"#334155"},
    {key:"teal",label:"Teal",gradient:"linear-gradient(180deg,#134e4a 0%,#0f766e 60%,#14b8a6 100%)",accent:"#0f766e",btn:"#0f766e"},
    {key:"rose",label:"Granate",gradient:"linear-gradient(180deg,#881337 0%,#be123c 60%,#e11d48 100%)",accent:"#be123c",btn:"#be123c"},
    {key:"gray",label:"Grafito",gradient:"linear-gradient(180deg,#111827 0%,#1f2937 60%,#374151 100%)",accent:"#1f2937",btn:"#374151"},
    {key:"forest",label:"Bosque",gradient:"linear-gradient(180deg,#14532d 0%,#15803d 60%,#22c55e 100%)",accent:"#15803d",btn:"#15803d"},
  ];
  const [dirThemeKey,setDirThemeKey]=useState(()=>localStorage.getItem("dir_theme")||"indigo");
  const [dirProfilePic,setDirProfilePic]=useState(()=>localStorage.getItem("dir_profile_pic")||null);
  const dirPhotoRef=useRef();
  const dirTheme=DIR_THEMES.find(t=>t.key===dirThemeKey)||DIR_THEMES[0];
  const DIR_ACCENT=dirTheme.accent;
  const DIR_GRAD=dirTheme.gradient;
  ...
  const [dirBellOpen,setDirBellOpen]=useState(false);
  const [dirBellReadIds,setDirBellReadIds]=useState(()=>{try{return JSON.parse(localStorage.getItem("lms_bell_dir")||"[]");}catch{return[];}});
  const dirUnreadCount=dirNewsItems.filter(n=>!dirBellReadIds.includes(n.id)).length;
  const markAllDirBellRead=()=>{const ids=dirNewsItems.map(n=>n.id);setDirBellReadIds(ids);localStorage.setItem("lms_bell_dir",JSON.stringify(ids));};
  const markOneDirBellRead=(id)=>{if(!dirBellReadIds.includes(id)){const ids=[...dirBellReadIds,id];setDirBellReadIds(ids);localStorage.setItem("lms_bell_dir",JSON.stringify(ids));}};
  const [dirSelBell,setDirSelBell]=useState(null);
  const DirBellBtn=()=>(
    <div style={{position:"relative"}}>
      <button onClick={()=>setDirBellOpen(o=>!o)}
        style={{width:26,height:26,borderRadius:"50%",
          background:dirBellOpen?C.fill3:`linear-gradient(135deg,${DIR_ACCENT},${DIR_ACCENT}bb)`,
          border:"none",cursor:"pointer",display:"flex",alignItems:"center",
          justifyContent:"center",fontSize:13,boxShadow:"0 1px 5px rgba(0,0,0,0.15)",
          transition:"all 0.15s",position:"relative"}}>
        🔔
        {dirUnreadCount>0&&<span style={{position:"absolute",top:-3,right:-3,
          background:dirUrgentCount>0?C.red:DIR_ACCENT,color:"#fff",fontSize:9,fontWeight:800,
          borderRadius:"50%",minWidth:15,height:15,display:"flex",alignItems:"center",
          justifyContent:"center",border:"2px solid #fff",fontFamily:SF,padding:"0 3px"}}>
          {dirUnreadCount}
        </span>}
      </button>
      {dirBellOpen&&(
        <>
          <div onClick={()=>setDirBellOpen(false)} style={{position:"fixed",inset:0,zIndex:650}}/>
          <div style={{position:"fixed",top:72,right:10,width:300,maxHeight:"70vh",
            background:"#fff",borderRadius:16,boxShadow:"0 10px 40px rgba(0,0,0,0.2)",
            border:`1px solid ${C.g5}`,overflow:"hidden",zIndex:700,animation:"fadeUp 0.18s ease"}}>
            <div style={{background:`linear-gradient(135deg,${C.indigo},${C.indigo}bb)`,padding:"11px 13px",
              display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SF}}>📣 Avisos {dirUnreadCount>0?`(${dirUnreadCount})`:""}</div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                {dirUnreadCount>0&&<button onClick={e=>{e.stopPropagation();markAllDirBellRead();}} style={{background:"rgba(255,255,255,0.25)",border:"none",borderRadius:8,padding:"3px 8px",cursor:"pointer",color:"#fff",fontSize:11,fontWeight:600,fontFamily:SF}}>Leer todas</button>}
                <button onClick={()=>setDirBellOpen(false)} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:"50%",width:22,height:22,cursor:"pointer",color:"#fff",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
              </div>
            </div>
            <div style={{overflowY:"auto",maxHeight:"calc(70vh - 50px)"}}>
              {dirNewsItems.filter(n=>!dirBellReadIds.includes(n.id)).length===0
                ?<div style={{padding:20,textAlign:"center",color:C.lbl2,fontSize:13,fontFamily:SF}}>¡Todo al día! 🎉</div>
                :dirNewsItems.filter(n=>!dirBellReadIds.includes(n.id)).map((n,i,arr)=>(
                <div key={n.id}>
                  <div onClick={()=>{markOneDirBellRead(n.id);setDirSelBell(dirSelBell===n.id?null:n.id);}}
                    style={{padding:"9px 12px",cursor:"pointer",borderLeft:`4px solid ${n.color}`,background:`${n.color}08`}}>
                    <div style={{display:"flex",gap:7,alignItems:"flex-start"}}>
                      <span style={{fontSize:17,flexShrink:0}}>{n.icon}</span>
                      <div style={{flex:1,minWidth:0}}>
                        {n.urgent&&<div style={{fontSize:9,fontWeight:800,color:C.red,fontFamily:SF}}>🚨 URGENTE</div>}
                        <div style={{fontSize:12,fontWeight:700,color:C.lbl,fontFamily:SF,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:dirSelBell===n.id?"normal":"nowrap"}}>{n.title}</div>
                        ...
                        {dirSelBell===n.id&&n.body&&<div style={{fontSize:11,color:C.lbl2,fontFamily:SF,lineHeight:1.5,marginTop:5,padding:"5px 7px",background:C.fill4,borderRadius:6}}>{n.body}</div>}
                      </div>
                    </div>
                  </div>
                  {i<arr.length-1&&<div style={{height:"0.5px",background:C.sep,marginLeft:44}}/>}
                </div>
              ))}
            </div>
          </div>
  ...
  return(
    <div className="app-layout" style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
      <AppSidebar
        ...
        onLogoClick={()=>{setTab("feed");localStorage.setItem("dir_tab","feed");}}
        userEmoji="👩‍💼" userName="Directora Gómez" userSub={activeCycle?.name||"Ciclo activo"}
        onLogout={onLogout}
        profileInitials="DG" profilePic={dirProfilePic}
        bellNode={<DirBellBtn/>}
        navItems={[
          ...
          {id:"management",icon:"📊",label:"Gestión",active:tab==="management",onClick:()=>{setTab("management");localStorage.setItem("dir_tab","management");},
            subItems:[
              {id:"resumen",icon:"🗺️",label:"Resumen",active:mgmtTab==="resumen",onClick:()=>{setMgmtTab("resumen");localStorage.setItem("dir_mgmtTab","resumen");}},
      ...
      <div style={{flex:1,minWidth:0,minHeight:"100vh",overflowX:"hidden"}}>
        ...
        {tab==="management"&&(
          <div>
                ...
                <>
                  <Card style={{marginBottom:14,overflow:"hidden"}}>
                    <div style={{padding:"12px 14px 6px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      <span style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>Toca un salón para ver detalles</span>
                    </div>
                    {(()=>{
                      ...
                      const RoomRect=({label,x,y,stroke,fill})=>{
                        ...
                        const occupants=grp?state.students.filter(s=>grp.students.includes(s.id)||grp.students.includes(String(s.id))):[];
                        const todayAtts=occupants.map(s=>{
                          // try live attendance from Firebase first, then local
                          const rec=s.attendance?.find(a=>a.date===todayStr);
                          return rec?.s||"unknown";
                        });
                        const presentCount=todayAtts.filter(s=>s==="present").length;
                        const absentCount=todayAtts.filter(s=>s==="absent").length;
                        const dotColor=absentCount>0?C.red:presentCount>0?C.green:C.g3;
                        return(
                          <g key={label} style={{cursor:clickable?"pointer":"default"}}
                            ...
                            {clickable&&grp&&(
                              <circle cx={x+54} cy={y-4} r="3.5" fill={dotColor}/>
                            )}
                          </g>
                        );
                      };
                      return(
                        <svg width="100%" viewBox="0 0 320 280" style={{display:"block",background:"#F8F9FC",minHeight:220}}>
                          ...
                          {/* Legend */}
                          <circle cx="90" cy="276" r="3.5" fill={C.green}/>
                          <text x="97" y="279" fill="#666" fontSize="6.5">Presentes</text>
                          <circle cx="150" cy="276" r="3.5" fill={C.red}/>
                          <text x="157" y="279" fill="#666" fontSize="6.5">Ausentes</text>
                          <circle cx="210" cy="276" r="3.5" fill={C.g3}/>
                          <text x="217" y="279" fill="#666" fontSize="6.5">Sin registro</text>
                        </svg>
                      );
                    })()}
                    {/* Room detail panel */}
                    {selectedRoom&&(()=>{
                      ...
                      const grp=state.groups.find(g=>g.name===grpName);
                      const teacher=grp?state.teachers.find(t=>String(t.id)===String(grp.teacherId)||state.groups.find(gg=>gg.id===grp.id)?.subjects?.some(s=>String(s.teacherId)===String(t.id))):null;
                      const studs=grp?state.students.filter(s=>(grp.students||[]).some(id=>String(id)===String(s.id))||s.group===grpName):[];
                        ...
                        <div style={{borderTop:`0.5px solid ${C.sep}`,background:"#fff"}}>
                          <div style={{padding:"12px 14px 8px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                            <div>
                              ...
                              <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF}}>{grp?`Grupo ${grpName}`:"Sin grupo asignado"}</div>
                            </div>
                            <button onClick={()=>setSelectedRoom(null)}
                              style={{background:C.fill3,border:"none",borderRadius:"50%",width:26,height:26,cursor:"pointer",color:C.lbl2,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                          </div>
                          {teacher?(
                            <div style={{padding:"8px 14px",borderBottom:`0.5px solid ${C.sep}`}}>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                <div style={{position:"relative",flexShrink:0}}>
                                  ...
                                  <div style={{position:"absolute",bottom:-1,right:-1,width:11,height:11,borderRadius:"50%",background:teacherPresent?C.green:C.g3,border:"2px solid #fff"}}/>
                                </div>
                                <div style={{flex:1}}>
                                  <div style={{...fmt.callout,fontWeight:600,color:C.lbl,fontFamily:SF}}>{teacher.name}</div>
                                  <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF}}>{(teacher.subjects||[]).join(", ")||"Sin materias"}</div>
                                </div>
                                <span style={{fontSize:11,fontWeight:600,borderRadius:6,padding:"2px 8px",fontFamily:SF,color:teacherPresent?C.green:C.g1,background:teacherPresent?`${C.green}15`:C.fill4}}>
                                  {teacherPresent?`Presente ${teacherAtt.time}`:"Sin registro"}
                                </span>
                              </div>
                            </div>
                          ):(
                            <div style={{padding:"8px 14px",borderBottom:`0.5px solid ${C.sep}`,color:C.lbl3,fontSize:13,fontFamily:SF}}>Sin docente asignado.</div>
                          )}
                          <div style={{padding:"8px 0 4px"}}>
                            ...
                            {studs.length===0&&<div style={{padding:"12px 14px",color:C.lbl3,fontSize:13,fontFamily:SF}}>Sin alumnos inscritos.</div>}
                            {studs.map((s,i)=>{
                                ...
                                <div key={s.id}>
                                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"7px 14px"}}>
                                    <div style={{position:"relative",flexShrink:0}}>
                                      ...
                                      <div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:dotColor,border:"2px solid #fff"}}/>
                                    </div>
                                    <div style={{flex:1}}>
                                      <div style={{fontSize:13,fontWeight:600,color:C.lbl,fontFamily:SF}}>{s.name}</div>
                                    </div>
                                    <span style={{fontSize:10,fontWeight:600,borderRadius:5,padding:"2px 7px",fontFamily:SF,color:dotColor,background:`${dotColor}15`}}>{statusLabel}</span>
                                  </div>
            ...
            {mgmtTab==="actividades"&&(()=>{
              ...
              // Send activity as a chat message to all relevant teachers
              const sendActToChat=()=>{
                if(!newAct.title.trim()||!newAct.date) return;
                const a={id:Date.now(),title:newAct.title,type:newAct.type,date:newAct.date,dueDate:newAct.dueDate||"",teacherId:newAct.teacherId,description:newAct.description,status:"pendiente",_createdAt:Date.now()};
                // Persist to Firestore
                addDoc(collection(db,"actividades"),{...a,_createdAt:serverTimestamp()}).catch(()=>{});
                setState(s=>({...s,actividades:[a,...(s.actividades||[])]}));
                // Determine recipients
                const targets=newAct.teacherId==="all"
                  ?state.teachers
                  :state.teachers.filter(t=>String(t.id)===String(newAct.teacherId));
                const typeLabel={trabajo:"Trabajo",exposicion:"Exposición",efemeride:"Efeméride",examen:"Examen",evento:"Evento Escolar",otro:"Otro"}[newAct.type]||newAct.type;
                const msgText=`📋 *Actividad asignada: ${newAct.title}*\nTipo: ${typeLabel}\nFecha evento: ${newAct.date}${newAct.dueDate?`\nFecha entrega: ${newAct.dueDate}`:""}${newAct.description?`\n\n${newAct.description}`:""}`;
                targets.forEach(t=>{
                  const participants=["dir",String(t.id)];
                  const existing=(state.chats||[]).find(c=>(c.participants||[]).includes("dir")&&(c.participants||[]).includes(String(t.id))&&c.participants.length===2);
                  const chatId=existing?.id||`chat_dir_${t.id}_${Date.now()}`;
                  const now=new Date();
                  const msg={from:"dir",fromName:"Directora",text:msgText,type:"text",time:now.toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"}),date:now.toISOString().slice(0,10)};
                  // Persist message to Firestore
                  addDoc(collection(db,"chatMsgs"),{...msg,chatId,_createdAt:serverTimestamp()}).catch(()=>{});
                  const chatMeta={lastMsg:msgText,lastTime:msg.time,unread:{}};
                  setState(s=>{
                    const chats=s.chats||[];
                    const msgs=s.chatMessages||{};
                    const chatExists=chats.some(c=>c.id===chatId);
                    const unread={...((chats.find(c=>c.id===chatId)||{}).unread||{}),[String(t.id)]:((chats.find(c=>c.id===chatId)||{}).unread?.[String(t.id)]||0)+1};
                    const fullMeta={...chatMeta,unread};
                    if(!chatExists){
                      setDoc(doc(db,"chats",chatId),{id:chatId,participants,name:t.name,...fullMeta},{merge:true}).catch(()=>{});
                    } else {
                      setDoc(doc(db,"chats",chatId),fullMeta,{merge:true}).catch(()=>{});
                    }
                    return{
                      ...s,
                      chats:chatExists
                        ?chats.map(c=>c.id===chatId?{...c,...fullMeta}:c)
                        :[{id:chatId,participants,name:t.name,...fullMeta},...chats],
                      chatMessages:{...msgs,[chatId]:[...(msgs[chatId]||[]),msg]}
                    };
                  });
                });
                SFX.play("success");
                pushNotification({title:"📋 Actividad asignada",text:`"${newAct.title}" enviada al chat`});
                setNewAct({title:"",type:"trabajo",date:"",dueDate:"",teacherId:"all",description:""});
              ...
              return(
                <>
                    ...
                    <Card style={{padding:14,marginBottom:12}}>
                      ...
                      <div style={{background:C.fill4,borderRadius:12,padding:"10px 14px",marginBottom:10}}>
                        ...
                        <input type="date" value={newAct.date} onChange={e=>setNewAct(a=>({...a,date:e.target.value}))}
                          style={{width:"100%",background:"transparent",border:"none",fontSize:15,color:C.lbl,outline:"none",boxSizing:"border-box",cursor:"pointer",appearance:"auto",WebkitAppearance:"auto"}}/>
                      </div>
                      ...
                      <Btn onPress={sendActToChat} full disabled={!newAct.title.trim()||!newAct.date} color={C.indigo}>
                        💬 Asignar y enviar al chat
                      </Btn>
                    </Card>
                  )}
                  {acts.length>0&&(
                  ...
                  {acts.map((a)=>{
                    ...
                    const assignedTeacher=a.teacherId==="all"?"Todos los docentes":state.teachers.find(t=>String(t.id)===String(a.teacherId))?.name||"—";
                    return(
                      <Card key={a.id} style={{marginBottom:10,padding:14}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                          <div style={{flex:1,marginRight:10}}>
                            ...
                            <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF,marginBottom:2}}>{assignedTeacher}</div>
                            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4}}>
                              {a.date&&<span style={{fontSize:10,color:C.lbl3,fontFamily:SF}}>📅 Evento: {a.date}</span>}
                              {a.dueDate&&<span style={{fontSize:10,color:C.orange,fontFamily:SF}}>📬 Entrega: {a.dueDate}</span>}
                            </div>
        ...
        {tab==="settings"&&(
          <div>
            <NavBar title="Ajustes" large accent={DIR_ACCENT}/>
            <div className="page-pad">
              {/* ── Foto de perfil ── */}
              <Card style={{padding:20,marginBottom:16,display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
                <input ref={dirPhotoRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
                  const f=e.target.files?.[0]; if(!f) return;
                  const r=new FileReader(); r.onload=ev=>{const d=ev.target.result;setDirProfilePic(d);localStorage.setItem("dir_profile_pic",d);};
                  r.readAsDataURL(f); e.target.value="";
                }}/>
                <div style={{position:"relative",cursor:"pointer"}} onClick={()=>dirPhotoRef.current?.click()}>
                  {dirProfilePic
                    ?<img src={dirProfilePic} style={{width:88,height:88,borderRadius:"50%",objectFit:"cover",border:`3px solid ${DIR_ACCENT}`}}/>
                    :<div style={{width:88,height:88,borderRadius:"50%",background:`${DIR_ACCENT}20`,border:`3px solid ${DIR_ACCENT}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,fontWeight:700,color:DIR_ACCENT,fontFamily:SFD}}>DG</div>
                  }
                  <div style={{position:"absolute",bottom:2,right:2,width:26,height:26,borderRadius:"50%",background:DIR_ACCENT,border:"2px solid #fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>📷</div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{...fmt.title3,color:C.lbl,fontFamily:SFD}}>Directora Gómez</div>
                  <div style={{...fmt.caption,color:C.lbl2,fontFamily:SF,marginTop:2}}>Ciclo Escolar: {activeCycle?.name}</div>
                </div>
                <button onClick={()=>dirPhotoRef.current?.click()}
                  style={{background:`${DIR_ACCENT}15`,border:`1px solid ${DIR_ACCENT}40`,borderRadius:10,padding:"7px 18px",
                    fontSize:13,fontWeight:600,color:DIR_ACCENT,cursor:"pointer",fontFamily:SF}}>
                  {dirProfilePic?"Cambiar foto":"Subir foto de perfil"}
                </button>
                {dirProfilePic&&<button onClick={()=>{setDirProfilePic(null);localStorage.removeItem("dir_profile_pic");}}
                  style={{background:"none",border:"none",fontSize:12,color:C.lbl3,cursor:"pointer",fontFamily:SF}}>Quitar foto</button>}
              </Card>

              {/* ── Tema de color ── */}
              <Sec title="Tema de Color">
                <div style={{padding:"12px 14px"}}>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                    {DIR_THEMES.map(t=>(
                      <button key={t.key} onClick={()=>{setDirThemeKey(t.key);localStorage.setItem("dir_theme",t.key);}}
                        style={{border:`2.5px solid ${dirThemeKey===t.key?t.accent:"transparent"}`,borderRadius:14,padding:0,
                          cursor:"pointer",overflow:"hidden",outline:"none",
                          boxShadow:dirThemeKey===t.key?`0 0 0 2px #fff,0 0 0 4px ${t.accent}`:"0 1px 4px rgba(0,0,0,0.1)",
                          transition:"all 0.15s"}}>
                        <div style={{background:t.gradient,height:44,display:"flex",alignItems:"center",justifyContent:"center"}}>
                          {dirThemeKey===t.key&&<span style={{fontSize:16}}>✓</span>}
                        </div>
                        <div style={{background:"#fff",padding:"5px 4px",textAlign:"center",fontSize:11,fontWeight:600,color:C.lbl,fontFamily:SF}}>{t.label}</div>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <Sec title="Cuenta">
                <Row label="Notificaciones" icon="🔔" iconBg={`${DIR_ACCENT}20`} chevron onPress={()=>{}}/>
              </Sec>
              <Sec>
        ...
        {/* ── Chat IA Flotante ── (oculto en pestaña de chat) */}
        {tab!=="chat"&&<div style={{position:"fixed",bottom:86,right:16,zIndex:400}}>
        {aiChatOpen&&(
...
const TeacherApp=({state,setState,teacherId,onLogout})=>{
  ...
  const [sidebarOpen,setSidebarOpen]=useState(true);
  const [themeKey,setThemeKey]=useState(()=>{ try{return localStorage.getItem(`lms_tea_theme_${teacherId}`)||"default";}catch{return "default";} });
  const [profilePic,setProfilePic]=useState(()=>{ try{return localStorage.getItem(`lms_tea_pic_${teacherId}`)||null;}catch{return null;} });
  const fileRef=useRef();
  // Serious teacher color themes
  const TEA_THEMES={
    default:{accent:"#1d4ed8",name:"Marino",bg:"#F0F4FF"},
    slate:  {accent:"#334155",name:"Pizarra",bg:"#F1F5F9"},
    forest: {accent:"#166534",name:"Bosque",bg:"#F0FDF4"},
    burgundy:{accent:"#881337",name:"Vino",bg:"#FFF1F2"},
    charcoal:{accent:"#374151",name:"Carbón",bg:"#F9FAFB"},
    teal:   {accent:"#0f766e",name:"Teal",bg:"#F0FDFA"},
  };
  const TT=TEA_THEMES[themeKey]||TEA_THEMES.default;
  const ACCENT=TT.accent;
  ...
  // Find teacher BEFORE bellDragTea (which uses teacher?.id) to avoid TDZ error
  const teacher=state.teachers.find(t=>t.key&&String(t.key)===String(teacherId))
    ||state.teachers.find(t=>String(t.id)===String(teacherId))
    ||state.teachers.find(t=>!isNaN(Number(teacherId))&&Number(t.id)===Number(teacherId))
    ||state.teachers[0];
  ...
  const BellBtn=()=>{
    ...
    <div style={{position:"relative"}}>
      <button onClick={()=>setBellOpen(o=>!o)}
        style={{width:26,height:26,borderRadius:"50%",
          background:bellOpen?C.fill3:`linear-gradient(135deg,${ACCENT},${ACCENT}cc)`,
          border:"none",cursor:"pointer",display:"flex",alignItems:"center",
          justifyContent:"center",fontSize:13,boxShadow:"0 1px 5px rgba(0,0,0,0.12)",
          transition:"all 0.15s",position:"relative"}}>
        🔔
        {unreadBellCount>0&&<span style={{position:"absolute",top:-3,right:-3,background:C.red,
          color:"#fff",fontSize:9,fontWeight:800,borderRadius:"50%",minWidth:15,height:15,
          display:"flex",alignItems:"center",justifyContent:"center",border:"2px solid #fff",fontFamily:SF}}>
          {unreadBellCount}
        </span>}
      </button>
      {bellOpen&&(
        <>
          ...
          <div style={{position:"fixed",top:72,right:10,width:300,maxHeight:"70vh",
            background:"#fff",borderRadius:16,boxShadow:"0 10px 40px rgba(0,0,0,0.2)",
            border:`1px solid ${C.g5}`,overflow:"hidden",zIndex:700,animation:"fadeUp 0.18s ease"}}>
            <div style={{background:`linear-gradient(135deg,${ACCENT},${ACCENT}bb)`,padding:"11px 13px",
              display:"flex",alignItems:"center",justifyContent:"space-between"}}>
  ...
  if(selectedActivity && selectedActivity.id){
    ...
    if(!act.title){ setSelectedActivity(null); }
  ...
  if(selectedStudent){
    return(
      <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
        <NavBar title={selectedStudent.name.split(" ")[0]} back="← Clase" onBack={()=>setSelectedStudent(null)} accent={C.blue} />
        <div className="page-pad">
          <Card style={{padding:20,marginBottom:16,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
  ...
  if(selectedGroup && selectedGroup.name && !selectedGroupSubject){
  ...
  // ── Main tabs ──────────────────────────────────────────────────────────────
  // Safety: if teacher still not found, show loading/error with logout
  if(!teacher) return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.bg2,gap:16,fontFamily:SF}}>
      <div style={{fontSize:48}}>👩‍🏫</div>
      <div style={{fontSize:17,fontWeight:700,color:C.lbl}}>Cargando datos del docente…</div>
      <div style={{fontSize:13,color:C.lbl2,textAlign:"center",padding:"0 24px"}}>Si el problema persiste, verifica que la directora haya creado tu cuenta correctamente.</div>
      <button onClick={onLogout} style={{marginTop:8,background:C.red,color:"#fff",border:"none",borderRadius:12,padding:"12px 28px",fontSize:15,fontWeight:700,cursor:"pointer"}}>
        Cerrar sesión
      </button>
    </div>
  );
  return(
    <div className="app-layout" style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
      <AppSidebar
        open={sidebarOpen} onToggle={()=>setSidebarOpen(o=>!o)}
        gradient={`linear-gradient(180deg,${ACCENT}dd 0%,${ACCENT} 60%,${ACCENT}bb 100%)`}
        accent={ACCENT}
        logoEmoji="👩‍🏫" logoLine1="Portal" logoLine2="Docente"
        onLogoClick={()=>{setTab("feed");localStorage.setItem("tea_tab","feed");}}
        userEmoji={teacher?.avatar?"":"👨‍🏫"} userName={teacher?.name||"Docente"} userSub={`Grupos: ${teacher?.groups?.join(", ")||"-"}`}
        onLogout={onLogout}
        profileInitials={teacher?.avatar} profileColor={teacher?.color} profilePic={profilePic}
        bellNode={<BellBtn/>}
        navItems={[
          ...
          {id:"attendance",icon:"📅",label:"Asistencia",active:tab==="attendance",onClick:()=>{setTab("attendance");localStorage.setItem("tea_tab","attendance");}},
          {id:"ai",icon:"🤖",label:"Asist. IA",active:tab==="ai",onClick:()=>{setTab("ai");localStorage.setItem("tea_tab","ai");}},
      ...
      {tab==="feed"&&<Feed state={state} setState={setState}
        ...
        userAvatar={teacher?.avatar||"T"} userColor={teacher?.color||ACCENT}
        userRole="teacher" accent={ACCENT}
        newsItems={newsItems} urgentCount={urgentCount}/>}

      {tab==="chat"&&(
        <ChatPanel state={state} setState={setState}
          myUserId={`t${teacher?.id}`} myName={teacher?.name||"Maestro"}
          myAvatar={teacher?.avatar||"T"} myColor={teacher?.color||ACCENT}
          role="teacher" accent={ACCENT}/>
      )}

      {tab==="classes"&&(
        <div>
          <NavBar title="Mis Clases" large accent={ACCENT} />
          <div style={{padding:"0 16px 130px"}}>
            {myGroups.length===0&&(
      ...
      {tab==="ai"&&(
        <div>
          <NavBar title="Generador IA" large sub="Powered by Claude" accent={C.purple} />
          <div className="page-pad">
            <Card style={{padding:16,marginBottom:14}}>
      ...
      {tab==="attendance"&&(
        <div>
          <NavBar title="Asistencia" large accent={ACCENT} />
          <div className="page-pad">
            {myGroups.length===0?(
              <div style={{textAlign:"center",padding:"48px 20px",color:C.lbl2,fontFamily:SF}}>
                <div style={{fontSize:44,marginBottom:10}}>📅</div>
                <div style={{fontSize:16,fontWeight:700,color:C.lbl}}>Sin grupos asignados</div>
              </div>
            ):(
              myGroups.map(g=>(
                <div key={g.id} style={{marginBottom:20}}>
                  <div style={{fontSize:13,fontWeight:700,color:ACCENT,fontFamily:SF,
                    textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>
                    Grupo {g.name}
                  </div>
                  <TeacherAttendanceTab state={state} setState={setState} selectedGroup={g} teacher={teacher} todayStr={todayStr} COLOR={ACCENT}/>
                </div>
              ))
            )}
          </div>
      ...
      {tab==="settings"&&(
        <div>
          <NavBar title="Ajustes" large accent={ACCENT} />
          <div className="page-pad">
            {/* Profile photo */}
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:28}}>
              <div style={{position:"relative"}}>
                <Ava initials={teacher?.avatar||"T"} color={ACCENT} size={90} img={profilePic}/>
                <button onClick={()=>fileRef.current?.click()} style={{
                  ...
                  boxShadow:`0 2px 10px ${ACCENT}50`,fontSize:14}}>
                  📷
                </button>
                  ...
                  onChange={e=>{
                    ...
                    if(f){
                      ...
                      r.onload=ev=>{
                        ...
                        setProfilePic(data);
                        try{localStorage.setItem(`lms_tea_pic_${teacherId}`,data);}catch{}
                      };
                      r.readAsDataURL(f);
                    }
                  }}/>
              ...
              <div style={{fontFamily:"'SF Mono','Menlo',monospace",fontSize:13,color:ACCENT}}>{teacher?.key}</div>
              <div style={{fontSize:12,color:C.lbl2,fontFamily:SF,textAlign:"center"}}>
                {[...new Set(myGroups.flatMap(g=>myGroupSubjects(g).map(s=>s.subject)))].join(" · ")||"Sin materias"}
              </div>
            </div>

            {/* Color themes */}
            <Sec title="Tema de Color" footer="Se aplica en tu interfaz del portal docente.">
              <div style={{padding:"16px"}}>
                <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
                  {Object.entries(TEA_THEMES).map(([key,val])=>(
                    <button key={key} onClick={()=>{
                      setThemeKey(key);
                      try{localStorage.setItem(`lms_tea_theme_${teacherId}`,key);}catch{}
                    }} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,
                      background:"none",border:"none",cursor:"pointer",
                      transform:themeKey===key?"scale(1.15)":"scale(1)",transition:"transform 0.15s"}}>
                      <div style={{width:46,height:46,borderRadius:"50%",background:val.accent,
                        boxShadow:themeKey===key?`0 0 0 3px white,0 0 0 5px ${val.accent}`:"none",
            ...
            <Sec title="Cuenta">
              ...
              <Div indent={46}/>
              <Row label="Notificaciones" icon="🔔" iconBg={`${ACCENT}20`} chevron onPress={()=>{
                if(Notification?.permission==="default"){
                  Notification.requestPermission().then(p=>{
                    if(p==="granted") pushNotification({title:"🔔 Notificaciones activadas",text:"Recibirás avisos importantes."});
                  });
                } else {
                  pushNotification({title:"🔔 Notificaciones",text:Notification?.permission==="granted"?"Ya están activas.":"Desactivadas en el navegador."});
                }
              }}/>
            </Sec>
      ...
      <TabBar
        tabs={[
          {id:"feed",icon:"📋",label:"Tablón",badge:0},
          {id:"classes",icon:"🏫",label:"Clases",badge:urgentCount||0},
          {id:"chat",icon:"💬",label:"Mensajes",badge:totalChatUnread||0},
          {id:"attendance",icon:"📅",label:"Asistencia",badge:0},
          {id:"ai",icon:"🤖",label:"IA",badge:0},
          {id:"settings",icon:"⚙️",label:"Ajustes",badge:0},
        ]}
        active={tab}
        onChange={id=>{setTab(id);localStorage.setItem("tea_tab",id);setSelectedGroup(null);setSelectedGroupSubject(null);}}
        accent={ACCENT}
      />
      </div>
    </div>
...
// ─── ANIMATED COLORFUL ANIMALS (decorative for student feed) ──────────────────
const AnimatedAnimal=({type,size=100,style={}})=>{
  const W=type==="giraffe"?size*0.6:type==="rabbit"?size*0.75:type==="whale"?size*1.2:size;
  const H=type==="giraffe"?size*1.3:type==="whale"?size*0.6:size;
  
  const animals={
    owl:(
      <svg width={W} height={H} viewBox="0 0 100 100" style={{filter:"drop-shadow(0 4px 12px rgba(0,118,255,0.3))"}}>
        <g style={{animation:"mascotBob 3s ease-in-out infinite"}}>
          <ellipse cx="50" cy="55" rx="28" ry="30" fill="#4A90E2"/>
          <ellipse cx="50" cy="60" rx="24" ry="26" fill="#5CA3F5"/>
          <circle cx="38" cy="48" r="12" fill="white"/>
          <circle cx="62" cy="48" r="12" fill="white"/>
          <circle cx="40" cy="50" r="7" fill="#1a1a2e"/>
          <circle cx="64" cy="50" r="7" fill="#1a1a2e"/>
          <circle cx="42" cy="48" r="3" fill="white"/>
          <circle cx="66" cy="48" r="3" fill="white"/>
          <path d="M45 62 Q50 66 55 62" stroke="#FF9500" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <ellipse cx="28" cy="28" rx="10" ry="14" fill="#4A90E2" transform="rotate(-20 28 28)"/>
          <ellipse cx="72" cy="28" rx="10" ry="14" fill="#4A90E2" transform="rotate(20 72 28)"/>
          <ellipse cx="32" cy="85" rx="8" ry="3" fill="#5CA3F5" opacity="0.5"/>
          <ellipse cx="68" cy="85" rx="8" ry="3" fill="#5CA3F5" opacity="0.5"/>
        </g>
      </svg>
    ),
    rabbit:(
      <svg width={W} height={H} viewBox="0 0 100 120" style={{filter:"drop-shadow(0 4px 12px rgba(244,67,54,0.25))"}}>
        <g style={{animation:"mascotBob 2.8s ease-in-out infinite"}}>
          <ellipse cx="50" cy="75" rx="26" ry="30" fill="#FF9EB5"/>
          <ellipse cx="50" cy="78" rx="22" ry="26" fill="#FFB3C6"/>
          <ellipse cx="32" cy="28" rx="8" ry="28" fill="#FF9EB5" transform="rotate(-12 32 28)"/>
          <ellipse cx="34" cy="32" rx="4" ry="22" fill="#FFC1D3" transform="rotate(-12 34 32)"/>
          <ellipse cx="68" cy="28" rx="8" ry="28" fill="#FF9EB5" transform="rotate(12 68 28)"/>
          <ellipse cx="66" cy="32" rx="4" ry="22" fill="#FFC1D3" transform="rotate(12 66 32)"/>
          <circle cx="42" cy="70" r="5" fill="#1a1a2e"/>
          <circle cx="58" cy="70" r="5" fill="#1a1a2e"/>
          <circle cx="43" cy="69" r="2" fill="white"/>
          <circle cx="59" cy="69" r="2" fill="white"/>
          <circle cx="50" cy="77" r="4" fill="#FF6B9D"/>
          <path d="M48 80 L50 83 L52 80" stroke="#1a1a2e" strokeWidth="1.5" fill="none" strokeLinecap="round"/>
          <circle cx="50" cy="95" r="8" fill="white" opacity="0.8"/>
          <ellipse cx="38" cy="102" rx="9" ry="5" fill="#FFB3C6"/>
          <ellipse cx="62" cy="102" rx="9" ry="5" fill="#FFB3C6"/>
        </g>
      </svg>
    ),
    fox:(
      <svg width={W} height={H} viewBox="0 0 100 100" style={{filter:"drop-shadow(0 4px 12px rgba(255,149,0,0.3))"}}>
        <g style={{animation:"mascotBob 3.2s ease-in-out infinite"}}>
          <ellipse cx="50" cy="62" rx="24" ry="28" fill="#FF9500"/>
          <ellipse cx="50" cy="66" rx="20" ry="24" fill="#FFB84D"/>
          <ellipse cx="50" cy="75" rx="14" ry="16" fill="white"/>
          <path d="M26 35 L18 18 L32 42Z" fill="#FF9500"/>
          <path d="M74 35 L82 18 L68 42Z" fill="#FF9500"/>
          <path d="M28 38 L24 28 L34 42Z" fill="#FFD699"/>
          <path d="M72 38 L76 28 L66 42Z" fill="#FFD699"/>
          <circle cx="42" cy="58" r="5" fill="#1a1a2e"/>
          <circle cx="58" cy="58" r="5" fill="#1a1a2e"/>
          <circle cx="43" cy="57" r="2" fill="white"/>
          <circle cx="59" cy="57" r="2" fill="white"/>
          <circle cx="50" cy="66" r="4" fill="#1a1a2e"/>
          <path d="M46 70 Q50 73 54 70" stroke="#1a1a2e" strokeWidth="1.5" fill="none"/>
          <path d="M45 90 Q50 88 55 90 Q52 94 50 95 Q48 94 45 90Z" fill="#FF9500"/>
        </g>
      </svg>
    ),
    cat:(
      <svg width={W} height={H} viewBox="0 0 100 100" style={{filter:"drop-shadow(0 4px 12px rgba(156,39,176,0.25))"}}>
        <g style={{animation:"mascotBob 3.5s ease-in-out infinite"}}>
          <ellipse cx="50" cy="60" rx="26" ry="28" fill="#BA68C8"/>
          <ellipse cx="50" cy="64" rx="22" ry="24" fill="#CE93D8"/>
          <path d="M28 32 L22 18 L34 38Z" fill="#BA68C8"/>
          <path d="M72 32 L78 18 L66 38Z" fill="#BA68C8"/>
          <circle cx="40" cy="56" r="6" fill="#1a1a2e"/>
          <circle cx="60" cy="56" r="6" fill="#1a1a2e"/>
          <circle cx="42" cy="55" r="2" fill="white"/>
          <circle cx="62" cy="55" r="2" fill="white"/>
          <circle cx="50" cy="64" r="3" fill="#E1BEE7"/>
          <path d="M46 68 Q50 70 54 68" stroke="#1a1a2e" strokeWidth="1.5" fill="none"/>
          <path d="M36 64 L28 66 M36 68 L28 70" stroke="#9C27B0" strokeWidth="1.5" strokeLinecap="round"/>
          <path d="M64 64 L72 66 M64 68 L72 70" stroke="#9C27B0" strokeWidth="1.5" strokeLinecap="round"/>
          <ellipse cx="38" cy="86" rx="8" ry="4" fill="#CE93D8"/>
          <ellipse cx="62" cy="86" rx="8" ry="4" fill="#CE93D8"/>
        </g>
      </svg>
    ),
    bear:(
      <svg width={W} height={H} viewBox="0 0 110 110" style={{filter:"drop-shadow(0 4px 12px rgba(121,85,72,0.3))"}}>
        <g style={{animation:"mascotBob 3.1s ease-in-out infinite"}}>
          <ellipse cx="55" cy="68" rx="30" ry="32" fill="#8D6E63"/>
          <ellipse cx="55" cy="72" rx="26" ry="28" fill="#A1887F"/>
          <circle cx="55" cy="45" r="28" fill="#8D6E63"/>
          <circle cx="55" cy="48" r="24" fill="#A1887F"/>
          <circle cx="30" cy="26" r="12" fill="#8D6E63"/>
          <circle cx="30" cy="26" r="7" fill="#BCAAA4" opacity="0.6"/>
          <circle cx="80" cy="26" r="12" fill="#8D6E63"/>
          <circle cx="80" cy="26" r="7" fill="#BCAAA4" opacity="0.6"/>
          <circle cx="45" cy="42" r="6" fill="#1a1a2e"/>
          <circle cx="65" cy="42" r="6" fill="#1a1a2e"/>
          <circle cx="47" cy="41" r="2" fill="white"/>
          <circle cx="67" cy="41" r="2" fill="white"/>
          <ellipse cx="55" cy="52" rx="8" ry="6" fill="#5D4037"/>
          <path d="M50 56 Q55 60 60 56" stroke="#5D4037" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <ellipse cx="55" cy="86" rx="20" ry="16" fill="#A1887F"/>
          <ellipse cx="42" cy="95" rx="10" ry="6" fill="#8D6E63"/>
          <ellipse cx="68" cy="95" rx="10" ry="6" fill="#8D6E63"/>
        </g>
      </svg>
    ),
    dragon:(
      <svg width={W} height={H} viewBox="0 0 110 100" style={{filter:"drop-shadow(0 4px 12px rgba(94,53,177,0.35))"}}>
        <g style={{animation:"mascotBob 2.9s ease-in-out infinite"}}>
          <ellipse cx="55" cy="60" rx="28" ry="30" fill="#7E57C2"/>
          <ellipse cx="55" cy="64" rx="24" ry="26" fill="#9575CD"/>
          <path d="M32 35 L28 22 L26 20 L30 36Z" fill="#7E57C2"/>
          <path d="M78 35 L82 22 L84 20 L80 36Z" fill="#7E57C2"/>
          <path d="M30 36 L34 40 L38 38Z" fill="#B39DDB"/>
          <path d="M80 36 L76 40 L72 38Z" fill="#B39DDB"/>
          <circle cx="44" cy="56" r="7" fill="#FFD700"/>
          <circle cx="66" cy="56" r="7" fill="#FFD700"/>
          <circle cx="45" cy="56" r="4" fill="#FF6F00"/>
          <circle cx="67" cy="56" r="4" fill="#FF6F00"/>
          <path d="M48 66 Q55 70 62 66" stroke="#B39DDB" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <path d="M75 58 Q88 62 92 72 Q90 76 84 70 Q82 64 78 60" fill="#7E57C2"/>
          <ellipse cx="42" cy="86" rx="9" ry="5" fill="#9575CD"/>
          <ellipse cx="68" cy="86" rx="9" ry="5" fill="#9575CD"/>
          <circle cx="30" cy="48" r="2" fill="#FFD700" opacity="0.7"/>
          <circle cx="80" cy="52" r="2" fill="#FFD700" opacity="0.7"/>
        </g>
      </svg>
    ),
    lion:(
      <svg width={W} height={H} viewBox="0 0 110 110" style={{filter:"drop-shadow(0 4px 16px rgba(251,140,0,0.4))"}}>
        <g style={{animation:"mascotBob 3.3s ease-in-out infinite"}}>
          <circle cx="55" cy="55" r="38" fill="#FFB74D" opacity="0.5"/>
          {[0,45,90,135,180,225,270,315].map((angle,i)=>{
            const rad=angle*Math.PI/180;
            const x=55+Math.cos(rad)*35;
            const y=55+Math.sin(rad)*35;
            return <ellipse key={i} cx={x} cy={y} rx="8" ry="14" fill="#FB8C00" transform={`rotate(${angle} ${x} ${y})`}/>;
          })}
          <circle cx="55" cy="55" r="26" fill="#FFB74D"/>
          <circle cx="55" cy="58" r="22" fill="#FFCC80"/>
          <circle cx="45" cy="50" r="6" fill="#1a1a2e"/>
          <circle cx="65" cy="50" r="6" fill="#1a1a2e"/>
          <circle cx="47" cy="49" r="2" fill="white"/>
          <circle cx="67" cy="49" r="2" fill="white"/>
          <ellipse cx="55" cy="60" rx="7" ry="5" fill="#E65100"/>
          <path d="M50 64 Q55 68 60 64" stroke="#E65100" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <ellipse cx="55" cy="82" rx="18" ry="14" fill="#FFCC80"/>
          <ellipse cx="44" cy="92" rx="8" ry="5" fill="#FFB74D"/>
          <ellipse cx="66" cy="92" rx="8" ry="5" fill="#FFB74D"/>
        </g>
      </svg>
    ),
    whale:(
      <svg width={W} height={H} viewBox="0 0 140 80" style={{filter:"drop-shadow(0 4px 12px rgba(74,158,255,0.35))"}}>
        <g style={{animation:"mascotBob 3.4s ease-in-out infinite"}}>
          <ellipse cx="65" cy="45" rx="55" ry="28" fill="#4A9EFF"/>
          <ellipse cx="68" cy="48" rx="58" ry="30" fill="#6CB8FF" opacity="0.8"/>
          <path d="M120 48 Q135 38 138 32 Q136 42 138 52 Q135 46 120 48Z" fill="#4A9EFF"/>
          <circle cx="28" cy="42" r="5" fill="white"/>
          <circle cx="29" cy="43" r="3" fill="#1a1a2e"/>
          <circle cx="30" cy="42" r="1" fill="white"/>
          <path d="M18 50 Q24 54 30 50" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round"/>
          <path d="M55 24 Q57 16 59 12 M59 24 Q62 16 64 11 M63 25 Q67 17 69 13" stroke="#A0D8FF" strokeWidth="1.8" fill="none" strokeLinecap="round"/>
          <ellipse cx="58" cy="62" rx="38" ry="12" fill="#E8F4FF" opacity="0.6"/>
          <circle cx="85" cy="38" r="3" fill="#A0D8FF" opacity="0.5"/>
          <circle cx="95" cy="44" r="2" fill="#A0D8FF" opacity="0.5"/>
        </g>
      </svg>
    ),
    giraffe:(
      <svg width={W} height={H} viewBox="0 0 80 140" style={{filter:"drop-shadow(0 4px 12px rgba(255,152,0,0.3))"}}>
        <g style={{animation:"mascotBob 3.6s ease-in-out infinite"}}>
          <path d="M36 80 Q38 40 40 18 Q42 40 44 80Z" fill="#FFB74D"/>
          <ellipse cx="40" cy="95" rx="26" ry="32" fill="#FFB74D"/>
          <ellipse cx="40" cy="18" rx="14" ry="12" fill="#FFB74D"/>
          <rect x="36" y="6" width="3" height="10" rx="1.5" fill="#F57C00"/>
          <rect x="41" y="7" width="3" height="9" rx="1.5" fill="#F57C00"/>
          <circle cx="37" cy="6" r="2.5" fill="#F57C00"/>
          <circle cx="42" cy="7" r="2.5" fill="#F57C00"/>
          <ellipse cx="30" cy="17" rx="5" ry="4" fill="#FFB74D" transform="rotate(-18 30 17)"/>
          <ellipse cx="50" cy="17" rx="5" ry="4" fill="#FFB74D" transform="rotate(18 50 17)"/>
          <circle cx="35" cy="17" r="4" fill="white"/>
          <circle cx="36" cy="17" r="2.5" fill="#1a1a2e"/>
          <circle cx="37" cy="16" r="1" fill="white"/>
          <ellipse cx="40" cy="24" rx="4" ry="2.5" fill="#FB8C00"/>
          {[[37,58,5],[46,68,4],[35,82,6],[47,90,5],[36,100,5],[48,108,4]].map(([x,y,r],i)=>(
            <ellipse key={i} cx={x} cy={y} rx={r} ry={r*0.9} fill="#F57C00" opacity="0.5"/>
          ))}
          <rect x="26" y="122" width="7" height="18" rx="3.5" fill="#FFB74D"/>
          <rect x="36" y="122" width="7" height="18" rx="3.5" fill="#FB8C00"/>
          <rect x="42" y="122" width="7" height="18" rx="3.5" fill="#FFB74D"/>
          <rect x="52" y="122" width="7" height="18" rx="3.5" fill="#FB8C00"/>
        </g>
      </svg>
    ),
  };
  ...
  return(
    <div style={{display:"inline-block",pointerEvents:"none",userSelect:"none",...style}}>
      {animals[type]||null}
    </div>
  );
};

// ─── LION TITLE DECORATION ─────────────────────────────────────────────────────
const LionTitleDeco=()=>(
  <div style={{position:"absolute",top:-8,left:-45,zIndex:5,pointerEvents:"none"}}>
    <svg width="42" height="42" viewBox="0 0 42 42" style={{filter:"drop-shadow(0 2px 8px rgba(251,140,0,0.4))"}}>
      <g style={{animation:"mascotBob 3s ease-in-out infinite"}}>
        <circle cx="21" cy="21" r="16" fill="#FFB74D" opacity="0.4"/>
        {[0,60,120,180,240,300].map((angle,i)=>{
          const rad=angle*Math.PI/180;
          const x=21+Math.cos(rad)*15;
          const y=21+Math.sin(rad)*15;
          return <ellipse key={i} cx={x} cy={y} rx="3.5" ry="6" fill="#FB8C00" transform={`rotate(${angle} ${x} ${y})`}/>;
        })}
        <circle cx="21" cy="21" r="11" fill="#FFB74D"/>
        <circle cx="21" cy="22" r="9" fill="#FFCC80"/>
        <circle cx="17" cy="19" r="2.5" fill="#1a1a2e"/>
        <circle cx="25" cy="19" r="2.5" fill="#1a1a2e"/>
        <circle cx="17.7" cy="18.5" r="0.8" fill="white"/>
        <circle cx="25.7" cy="18.5" r="0.8" fill="white"/>
        <ellipse cx="21" cy="23" rx="3" ry="2" fill="#E65100"/>
        <path d="M19 25 Q21 27 23 25" stroke="#E65100" strokeWidth="1.2" fill="none" strokeLinecap="round"/>
      </g>
    </svg>
  </div>
);
    ...
    <div style={{position:"relative"}}>
      <button onClick={()=>setNewsOpen(o=>!o)}
        style={{width:26,height:26,borderRadius:"50%",
          background:newsOpen?"#fff":`linear-gradient(135deg,${T.accent},${T.accent}bb)`,
          border:newsOpen?`1.5px solid ${T.accent}`:"none",
          cursor:"pointer",boxShadow:"0 1px 5px rgba(0,0,0,0.12)",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,
          transition:"all 0.2s",position:"relative"}}>
        <span style={{color:newsOpen?T.accent:"#fff"}}>🔔</span>
        {stuUnreadBellCount>0&&(
          <span style={{position:"absolute",top:-3,right:-3,background:urgentCount>0?C.red:T.accent,
            color:"#fff",fontSize:9,fontWeight:800,borderRadius:"50%",
      ...
      {newsOpen&&(
        <>
          <div onClick={()=>setNewsOpen(false)} style={{position:"fixed",inset:0,zIndex:650}}/>
          <div style={{position:"fixed",top:72,right:10,width:300,maxHeight:"70vh",
            background:"#fff",borderRadius:16,boxShadow:"0 10px 40px rgba(0,0,0,0.2)",
            border:`1px solid ${C.g5}`,overflow:"hidden",zIndex:700,animation:"fadeUp 0.18s ease"}}>
            <div style={{background:`linear-gradient(135deg,${T.accent},${T.accent}bb)`,padding:"11px 13px",
              display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SF}}>📣 Avisos {stuUnreadBellCount>0?`(${stuUnreadBellCount})`:""}</div>
              </div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                {stuUnreadBellCount>0&&<button onClick={e=>{e.stopPropagation();markAllStuBellRead();}} style={{background:"rgba(255,255,255,0.25)",border:"none",borderRadius:8,padding:"3px 8px",cursor:"pointer",color:"#fff",fontSize:11,fontWeight:600,fontFamily:SF}}>Leer todas</button>}
                <button onClick={()=>setNewsOpen(false)} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:"50%",width:22,height:22,cursor:"pointer",color:"#fff",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
              </div>
            </div>
            <div style={{overflowY:"auto",maxHeight:"calc(70vh - 50px)"}}>
              {newsItems.filter(n=>!stuBellReadIds.includes(n.id)).length===0
                ?<div style={{padding:20,textAlign:"center",color:C.lbl2,fontSize:13,fontFamily:SF}}>¡Todo al día! 🎉</div>
                :newsItems.filter(n=>!stuBellReadIds.includes(n.id)).map((n,i,arr)=>(
                <div key={n.id}>
                  <div onClick={()=>{markOneStuBellRead(n.id);setSelectedNewsId(selectedNewsId===n.id?null:n.id);}}
                    style={{padding:"9px 12px",cursor:"pointer",borderLeft:`4px solid ${n.color}`,background:`${n.color}08`}}>
                    <div style={{display:"flex",gap:7,alignItems:"flex-start"}}>
                      <span style={{fontSize:17,flexShrink:0}}>{n.icon}</span>
                      <div style={{flex:1,minWidth:0}}>
                        {n.urgent&&<div style={{fontSize:9,fontWeight:800,color:C.red,fontFamily:SF}}>🚨 URGENTE</div>}
                        <div style={{fontSize:12,fontWeight:700,color:C.lbl,fontFamily:SF,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:selectedNewsId===n.id?"normal":"nowrap"}}>{n.title}</div>
                        <div style={{display:"flex",gap:5,marginTop:2}}>
                          <span style={{fontSize:9,fontWeight:600,color:n.color,background:`${n.color}15`,borderRadius:4,padding:"1px 5px",fontFamily:SF}}>{n.badge}</span>
                          <span style={{fontSize:9,color:C.lbl3,fontFamily:SF}}>{n.time}</span>
                        </div>
                        {selectedNewsId===n.id&&n.body&&<div style={{fontSize:11,color:C.lbl2,fontFamily:SF,lineHeight:1.5,marginTop:5,padding:"5px 7px",background:C.fill4,borderRadius:6}}>{n.body}</div>}
                      </div>
                    </div>
                  </div>
                  {i<arr.length-1&&<div style={{height:"0.5px",background:C.sep,marginLeft:44}}/>}
  ...
  return(
    <div className="app-layout" style={{background:T.bg,minHeight:"100vh",fontFamily:SF}}>
      <AppSidebar
        ...
        accent={T.accent||C.blue}
        logoEmoji="🎒" logoLine1="Portal" logoLine2="Alumno"
        onLogoClick={()=>{setTab("feed");localStorage.setItem("stu_tab","feed");}}
        userEmoji="👤" userName={student?.name||"Alumno"} userSub={`Grupo ${student?.group||"-"}`}
        onLogout={onLogout}
        profileInitials={student?.avatar} profilePic={profilePic} profileColor={student?.color}
        bellNode={<StuBellBtn/>}
        navItems={[
      ...
      {tab==="feed"&&(
        <div style={{position:"relative",overflow:"hidden",minHeight:"100vh"}}>
          {/* Decorative colorful animals — scattered around tablón */}
          <div style={{position:"fixed",top:140,left:8,zIndex:1,pointerEvents:"none",opacity:0.85}}>
            <AnimatedAnimal type="owl" size={85}/>
          </div>
          <div style={{position:"fixed",top:320,right:12,zIndex:1,pointerEvents:"none",opacity:0.9}}>
            <AnimatedAnimal type="rabbit" size={95}/>
          </div>
          <div style={{position:"fixed",bottom:220,left:16,zIndex:1,pointerEvents:"none",opacity:0.8}}>
            <AnimatedAnimal type="fox" size={75}/>
          </div>
          <div style={{position:"fixed",bottom:80,right:8,zIndex:1,pointerEvents:"none",opacity:0.88}}>
            <AnimatedAnimal type="cat" size={88}/>
          </div>
          <div style={{position:"fixed",top:500,left:6,zIndex:1,pointerEvents:"none",opacity:0.82}}>
            <AnimatedAnimal type="bear" size={105}/>
          </div>
          <div style={{position:"fixed",top:680,right:10,zIndex:1,pointerEvents:"none",opacity:0.85}}>
            <AnimatedAnimal type="dragon" size={92}/>
          </div>
          <div style={{position:"fixed",top:200,right:-20,zIndex:1,pointerEvents:"none",opacity:0.75}}>
            <AnimatedAnimal type="whale" size={120}/>
          </div>
          <div style={{position:"fixed",bottom:360,left:-8,zIndex:1,pointerEvents:"none",opacity:0.78}}>
            <AnimatedAnimal type="giraffe" size={110}/>
          </div>
          <div style={{position:"fixed",top:850,left:14,zIndex:1,pointerEvents:"none",opacity:0.83}}>
            <AnimatedAnimal type="lion" size={98}/>
          </div>
...
const Login=({state,onLogin})=>{
  ...
  const submit=async()=>{
    ...
    if(role==="developer"){
      ...
      if(state.teachers.length===0){setErr("Los datos aún están cargando. Espera un momento e intenta de nuevo.");return;}
      const t=state.teachers.find(t=>t.key&&t.key.trim()===trimmedKey);
      if(!t){
        const keys=state.teachers.map(x=>x.key).filter(Boolean).join(", ");
        setErr(`Clave incorrecta. Claves válidas: ${keys||"ninguna registrada"}`);
        return;
      }
      ...
      // Clear stale navigation state to prevent blank group view after re-login
      localStorage.removeItem("tea_grp");
      localStorage.removeItem("tea_gsubj");
      // Store teacher key — TeacherApp finds teacher by key (works across Firestore ID migrations)
      onLogin("teacher", t.key||String(t.id));
    } else {
      if(mode==="login"){
...
const AIButton = ({ role, state }) => {
  ...
  // Hide when user is on chat tab
  const getActiveTab = () => {
    if(role==="director") return localStorage.getItem("dir_tab")||"";
    if(role==="teacher") return localStorage.getItem("tea_tab")||"";
    if(role==="student") return localStorage.getItem("stu_tab")||"";
    return "";
  };
  const [activeTab, setActiveTab] = useState(getActiveTab);
  useEffect(() => {
    const handler = () => setActiveTab(getActiveTab());
    window.addEventListener("storage", handler);
    // Poll every 300ms to catch tab changes (localStorage doesn't fire in same tab)
