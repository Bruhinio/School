        ...
        <div>
          ...
          <div style={{padding:"0 16px 100px"}}>
            ...
            {mgmtTab==="asistencia"&&(()=>{
              ...
              const [attView, setAttView] = useState("tomar"); // tomar | historial

              // Build attendance map for selected date
              const staffAttMap = {};
              const staffAtt = state.teacherAttendance?.[attDate] || {};
              state.teachers.forEach(t => {
                staffAttMap[t.id] = staffAtt[t.id]?.status || null;
              });
              ...
              const markStaff = async (personId, date, status) => {
                ...
                try {
                  await setDoc(doc(db,"teacherAttendance",docId),{teacherId:personId,date,status,time});
                } catch {}
                setState(s=>({...s,teacherAttendance:{...s.teacherAttendance,[date]:{...(s.teacherAttendance?.[date]||{}),
                  [personId]:{status,time}}}}));
              };
      ...
      <div style={{background:C.bg2,minHeight:"100vh",fontFamily:SF}}>
        ...
        <div style={{padding:"14px 16px 100px"}}>
          ...
          {classTab==="attendance"&&(()=>{
            ...
            const groupStudents = state.students.filter(s => selectedGroup?.students?.includes(s.id) || s.group === selectedGroup?.name);
                ...
                {attView==="tomar"&&(
                  <>
                    ...
                    {groupStudents.length === 0 ? (
                      <div style={{textAlign:"center",padding:32,color:C.lbl2,fontSize:15,fontFamily:SF}}>
                        No hay alumnos en este grupo
                      </div>
                    ) : (
                      <AttendancePanel
                        people={groupStudents}
                        attendanceMap={studentAttMap}
                        date={attDate}
                        onMark={markStudent}
                        accentColor={COLOR}
                      />
