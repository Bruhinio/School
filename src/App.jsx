        ...
        <div className="page-pad">
          ...
          {classTab==="grades"&&(
            (()=>{
              ...
              const parcialLabels=Array.from({length:np},(_,i)=>`${i+1}er Parcial`.replace("1er","1er").replace("2er","2do").replace("3er","3er").replace("4er","4to").replace("5er","5to"));
              // Fix ordinals
              const ordinal=(n)=>["1er","2do","3er","4to","5to"][n-1]||`${n}°`;
              const [selParcial,setSelParcial]=React.useState(1);
              return(
                <>
                  {/* Parcial selector */}
                  <Card style={{padding:14,marginBottom:12}}>
                  ...
                  <Card style={{padding:14,marginBottom:14,background:`linear-gradient(135deg,${COLOR}10,${COLOR}04)`}}>
                    ...
                    <Btn onPress={()=>{
                      ...
                      setPublishedGrades(newPub);
                      // Persist parcial grade to each student in Firebase
                      groupStudents.forEach(s=>{
                        const key=`p${selParcial}_${selectedGroupSubject||selectedGroup.id}_${s.id}`;
