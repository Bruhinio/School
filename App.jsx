...
const DeveloperApp=({state,setState,onLogout})=>{
  ...
  const addTeacher=()=>{
    ...
    const t={id:Date.now(),name:newT.name,contact:newT.contact,email:newT.email,
  ...
  const addStudent=()=>{
    ...
    const s={id:Date.now(),name:newS.name,group:newS.group,
  ...
  const addGroup=()=>{
    ...
    const g={id:Date.now(),name:newG.name,grade:parseInt(newG.grade)||0,
      section:newG.section,teacherId:parseInt(newG.teacherId)||null,
  ...
  const addCycle=()=>{
    ...
    setState(s=>({...s,cycles:[...s.cycles,{id:Date.now(),name:newCycleName.trim(),active:false}]}));
            ...
            <div style={{position:"fixed",inset:0,zIndex:800,background:"rgba(0,0,0,0.45)",backdropFilter:"blur(4px)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
              <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:600,padding:"20px 20px 40px",animation:"slideUp 0.28s cubic-bezier(0.34,1.56,0.64,1)"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                  ...
                  <button onClick={()=>setGradeModal(null)} style={{background:C.fill3,border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",color:C.lbl2,fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                </div>
                ...
                <Btn onPress={()=>{saveGrade(gradeModal.actId,gradeModal.studentId,localGrade,localFeedback);setGradeModal(null);}} full color={C.blue}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:0}}>
              ...
              {role!=="developer"&&role!=="teacher"&&mode==="register"&&role==="student"&&(
                <>
                  ...
                  <Input label="Grupo *" placeholder="3°A" value={form.group||""} onChange={v=>setForm(f=>({...f,group:v}))}/>
                </>
              )}
        ...
        {step==="key"&&(
          <div style={{textAlign:"center"}}>
            <div style={{background:"#F0FFF4",border:`1.5px solid ${C.green}40`,borderRadius:16,padding:24,marginBottom:20}}>
              <div style={{fontSize:12,fontWeight:600,color:C.green,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:12,fontFamily:SF}}>Clave de Acceso del Alumno</div>
              ...
              <div style={{fontSize:12,color:C.green,marginTop:12,lineHeight:1.6,fontFamily:SF}}>
                ⚠️ El alumno usará esta clave para ingresar.<br/>Guárdala en un lugar seguro.
              </div>
            </div>
            <Btn onPress={()=>onLogin("student",state.students[0]?.id||1)} full size="lg">Entrar al Sistema →</Btn>
          </div>
        )}
      </div>
    </div>
...
export default function App(){
  ...
  const logout=()=>{setRole(null);setUserId(null);setScreen("login");};
  ...
  return(
    <>
      ...
      {screen==="app"&&role==="developer"&&<DeveloperApp state={state} setState={setState} onLogout={logout}/>}
      {screen==="app"&&role==="director"&&<DirectorApp state={state} setState={setState} onLogout={logout}/>}
      {screen==="app"&&role==="teacher"&&<TeacherApp state={state} setState={setState} teacherId={userId||1} onLogout={logout}/>}
      {screen==="app"&&role==="student"&&<StudentApp state={state} setState={setState} studentId={userId||1} onLogout={logout}/>}
    </>
  );
}
